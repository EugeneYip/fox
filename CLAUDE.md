...
- **直排的軸向是反的。** `writing-mode: vertical-rl` 之下，
  ...
  **容器單位也一樣，而且它跟的是元素自己、不是容器。** 2026-09-11 實測：
  容器 `.poem` 是橫排、寬 736px，而在 `vertical-rl` 的子元素上量到
  **`100cqi` ＝ 900px**（視窗的高），`100cqw` 才是 736px。
  直排裡要「容器有多寬」就用 `cqw`，不要用 `cqi`。

  這個錯很難看出來：算出來的字級剛好卡在 `clamp()` 的上限，
  跟「整條沒有生效」長得一模一樣。是拿 `calc(100cqi / 10)` 印出 90px 才抓到的。

  **還有 margin。** 一個直排的元素放進橫排的 grid／flex 裡（兩邊軸向交錯），
  它 inline 軸上的 margin 在 WebKit 是從**它自己的內容尺寸**裡扣，
  不是從可用空間裡扣。2026-09-11 在影片封面的題字上量到：
...
**2026-09-11 起這一步有檢查在守了**：`npm run icons -- --check` 在
`check:generated` 裡，它比對 `public/favicon.svg` 跟 `fox-mark.ts` 對不對得上。
只比那一個檔案是刻意的 —— 它是這裡唯一**逐位元組確定**的產物（一串字）；
PNG 由 sharp／libvips 編碼、分享圖上的中文要靠系統字型，
拿它們去比，CI 會紅在跟狐狸無關的事情上。理由寫在那支腳本的註解裡。

**圖示的網址帶著內容雜湊**（`src/lib/icons.ts` 的 `iconHref()`）。
分頁圖示存在瀏覽器自己的 favicon 資料庫裡、鍵是網址，
所以換掉舊圖的唯一可靠方法是換網址 —— 站主 2026-09-11 就是踩到這個
（伺服器供應的是新狐狸，瀏覽器顯示的是舊的）。檔案沒變雜湊就不變。
...
第 6 輪〔第二十八圈〕量到：這一份**曾經**整份被豁免，就是為了上面那一行；
換成放進框裡之後那條豁免就撤掉了，現在 `SKIP` 裡只剩 `docs/REVIEW-LOG.md`。
所以這一份跟其他文件一樣會被掃 —— 反例一定要放在框裡。）
...
**用臺灣的詞、臺灣的字**，不要用大陸的說法（`mainland-wording`）：
...
站主 2026-09-11 定的。加這條的時候實測過語料：63 頁的可見文字加上
所有文件，**一個命中都沒有** —— 它不是為了修什麼加的，是為了讓現在
這個狀態留得住。清單刻意很短，臺灣有正當用法的（質量＝mass、用戶名稱、
文言的信息、水平方向、刷新紀錄、國家代碼、餐廳的菜單、〈靜夜思〉的牀）
都不在裡面；理由逐條寫在 `scripts/lib/copy-rules.mjs` 那條規則上面。
