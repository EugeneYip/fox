...
- **commit 完就推。** 站主 2026-09-06 授權可以直接 `git push`，
  ...
  **推之前一定要先 rebase。** 排程的 `sync-feeds` 一天跑兩次，
  而**一天可能 commit 一次，也可能兩次**。

  第 7 輪（第二十三圈）加的判準要**兩個條件同時成立**才不寫檔
  （函式名字就寫著：`sameAndAlreadyToday`）——「內容沒變」**而且**
  「上一次的 `generatedAt` 已經是臺北的同一天」。
  這裡本來寫的是「最多 commit 一次」，那是把兩個條件說成一個。
  第 7 輪（第五十二圈）數了版控裡每一筆 `chore(sync)`：

  | 臺北日 | 幾筆 |
  |---|---|
  | 2026-09-05 | **2**（12:08、23:00） |
  | 2026-09-06 | 1 |
  | 2026-09-07 | 1 |
  | 2026-09-08 | **2**（01:40、12:15） |

  **四天裡有兩天是兩筆。** 兩個原因都遇得到：
  內容真的變了（09-08 那次是 feed 壞掉，`status` 翻 `error`），
  以及 **GitHub 的排程會遲到**，遲到夠久就跨到下一個臺北日 ——
  cron 是 `0 0,12 * * *`（臺北 08:00 與 20:00），而實際落在 12:08–12:15
  與 23:00／01:40。晚上那一跑遲了五個多小時，於是「同一天」不成立。
