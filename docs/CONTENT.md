...
**在那一句前面加一個 `>`**，它就會被框起來，跟前後的散文分開：

```markdown
意思全對，可是原句裡那種一格一格對上去的快感，全沒了。
...
- **前後都要空一行**，不然 markdown 會把它跟上一段黏在一起。
- **不用再加引號。** 那一圈框就是引號了，`「」` 留著會變成雙重引用。
- **只有「整段就是一句詩」才這樣寫。** 句子中間引的詞或半句
  （像「水窮處」、「白日依山盡」）照原樣用 `「」` 就好 ——
  把它們框起來會把一段話切得七零八落。

（這個框是詩頁那一方版框的輕量版：一圈細線、方角、貼著字。
以後如果要引的是別人說的話而不是詩，再來想要不要分成兩種樣子。）
...
| `draft-leaked` | 這篇是草稿，但它的字出現在產出裡（站上別的地方也寫著同一句話時不算 —— 那證明不了是這篇洩漏的） |
...
把 `draft: true` 加回去（或者刪掉整個檔案），再跑一次那四個指令。
下一次部署完那一頁就會變成 404。
