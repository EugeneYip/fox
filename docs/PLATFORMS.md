...
|---|---|---|---|---|---|---|---|
| `medium` | Medium | 美國 | 文章 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `substack` | Substack | 美國 | 文章 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `note` | note（日本） | 日本 | 文章 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `hatena` | はてなブログ | 日本 | 文章 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `pixnet` | 痞客邦 | 臺灣 | 文章 | ✅ 官方 RSS | 帳號名 | 需自行查 | — |
| `blogger` | Blogger | 國際 | 文章 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `wordpress` | WordPress | 國際 | 文章 | ✅ 官方 RSS | 完整網域 | 已實測（2026-09-06） | — |
| `ghost` | Ghost | 國際 | 文章 | ✅ 官方 RSS | 完整網域 | 已實測（2026-09-06） | — |
...
| `bluesky` | Bluesky | 國際 | 社群 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
| `mastodon` | Mastodon | 國際 | 社群 | ✅ 官方 RSS | 站台/帳號 | 已實測（2026-09-06） | — |
| `xiaohongshu` | 小紅書 | 中國 | 圖像 | 🔀 需橋接 | 帳號名 | 需自行查 | — |
| `zhihu` | 知乎 | 中國 | 文章 | 🔀 需橋接 | 帳號名 | 需自行查 | — |
| `douban` | 豆瓣 | 中國 | 文章 | 🔀 需橋接 | 帳號名 | 需自行查 | — |
| `weixin` | 微信公眾號 | 中國 | 文章 | ✍️ 手動 | 帳號名 | 依文件 | — |
| `youtube` | YouTube | 國際 | 影音 | ✅ 官方 RSS（+ API 備援） | 頻道 ID | 已實測（2026-09-06） | **已啟用** |
| `podcast` | Podcast | 國際 | 聲音 | ✅ 官方 RSS | 帳號名 | 依文件 | — |
| `github` | GitHub | 國際 | 程式 | ✅ 官方 RSS | 帳號名 | 已實測（2026-09-06） | — |
...
官方 Atom feed 可用，不需要金鑰，最近 15 支。但它會間歇性掛掉，而且不只一種樣子：2026-09-02 上午實測所有頻道（含 Google 自家的）都回 404，2026-09-06 則是一分鐘內回 404、500、404，而且只壞這一個頻道（同時間 Google 自家的回 200）——所以「別的頻道通不通」不是判準，要看頻道頁本身還在不在。同日中午再測就恢復 200。所以同步腳本是「先 RSS，掛了才退回 Data API v3」。YOUTUBE_API_KEY 是選填的備援，也是想往回抓 15 支以外的歷史時才需要。
