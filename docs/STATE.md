...
第三十八圈第 1 輪（無障礙）：`docs/A11Y.md` 點名三件**只能靠人**的事 ——
Tab 順序、目標尺寸、螢幕閱讀器。前兩件有探針，但那支探針**要人手貼進 console**；
第三件那份文件自己寫著「現況：**沒有人做過**」。而在這之前，
沒有任何地方提醒你它們在變舊。

先把躺很久的那條待辦做掉：**真的重驗一次**。375px、照探針自己的選取器跑五頁 ——
**25 個數字一個都沒有變**。（第一次我用自己寫的離線選取器數，得到 47/23/32
而基準是 45/20/30，差的是「看不見的元素」—— 沒有拿那個數字充數。）
現在 `check:a11y` 每次都印「上一次量是哪一天、幾天前、螢幕閱讀器做過沒有」，
只印不擋。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 299 筆逐輪紀錄都在
