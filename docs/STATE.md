...
第四十二圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪發現：最顯眼的手寫清單 `PAIRS` **其實有人守**（`gaps` 每次從 CSS 重算）。
沒有人守的是**決定語料的那一份** —— `FG_PROPS` 列的是長寫法
（`border-color`⋯），而 CSS 用的是簡寫（`border:`、`outline:`），
於是 `--c-rule`、`--c-edge`、`--c-focus` 三個 token **只經由清單外的屬性用到**。

實測：把 `--c-edge` 的兩組從 `PAIRS` 整個刪掉 —— 改之前輸出**還是印
「都在 PAIRS 裡 ✓」、離開碼 0**；改之後點名並紅。改成照 CSS 自己的命名認，
涵蓋率從前景 9 種變成 **12 種**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 338 筆逐輪紀錄都在
...
第四十二圈第 8 輪指出了這件事：「涵蓋率：前景 **9** 種⋯都在 PAIRS 裡 ✓」
看起來是個量測，其實那個 9 是 `FG_PROPS` 決定的 —— 改成照 CSS 命名認之後
同一句話變成 **12**。**數字沒有變準，是分母本來就少了三個。**
站上這種句子還有很多（44 頁、31 條規則、108 組文案 100%、11 條預算、
901 個連結⋯），下一圈逐句問那個分母是誰決定的、它漏掉了什麼。
