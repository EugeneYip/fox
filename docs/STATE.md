...
第 2 輪（效能）把 `check:perf` 的 11 條預算**一條一條歸零**
（預算的安靜失效不是上限被調高，是量到的值塌成 0）：**11／11 都會讓測試變紅**，
而且抓到它們的是不同格測試。

**量法整個壞掉也會被抓到，而抓到它的是「說明裡的現值」那一條。**
把 `gz()` 改成回傳常數，11 條預算沒有一條會超標（0 或 1 KB 都遠低於上限），
唯一出聲的是「說明裡寫的現值跟量到的對不上」——
那條看起來像文件整潔的檢查，實際上是對量法本身的健全性檢查。

**但那把尺的解析度是 10%。** 把 gzip 壓縮等級從 9 換成 1（一個合理的人為改動），
每個數字漲 8.8%、最大單頁從 75% 走到 82%，**兩套關卡一句話都沒說**。
那個 10% 是刻意訂的，所以不是 bug，是這把尺的極限 ——
而那個極限在預算上等於幾個百分點，這一輪之前沒有人量過。

**真正能抓到量法漂移的是線路，而它是手動的。** 跑了一次 `npm run probe:served`：
四頁的實際送出量比本機 level 9 少 **1.6%～0.4%**，跟 `MEASURED` 記著的
（`-1.7`～`-0.4`）一模一樣。日期更新成 2026-09-07，那是這一輪唯一的改動。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 388 筆逐輪紀錄都在
