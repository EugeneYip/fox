...
**第四十六圈問的是：這一段如果拿掉，輸出會差在哪裡？**
判準：**真的拿掉、跑一次、看誰說話。沒有人說話的，就是沒有人在守 ——
不管它看起來多重要。**
理由：四十五圈都在**加**東西，而從來沒有一圈問過「拿掉會怎樣」。
第四十五圈第 8 輪順手發現 `FG_PROPS` 裡有 10 個名字是三圈前就該刪沒刪的重複
—— 那是撞到的，不是問出來的。

第 1 輪（無障礙）把站上九項無障礙設施一個一個拿掉、重建、再跑關卡。
**九項裡有五項拿掉之後沒有任何一道關卡說話。** 這張表講的是同一件事：
關卡守的是「有人寫錯」（`outline: none`、`aria-live` 的值打錯），
不是「有人拿掉」—— 整段刪掉的話那些規則的主體變成 0，而 0 是「沒東西可判斷」。

只補了後果**可以證明**的那一項：搜尋頁狀態列的 `aria-live` 拿掉之後，
畫面照樣看得見，唯一分得出差別的是螢幕閱讀器使用者。
新規則 `status-not-live`（31 → 32 條），判準用站上自己的命名慣例
（`data-…status`），而且分得出 `role="status"` 本來就隱含 polite 的情況。

第一次量錯了一次：只看離開碼，於是把 `nav-label` 那兩格讀成「沒有人說話」——
**那條是 warn，它說了話但不擋。**

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 363 筆逐輪紀錄都在
