...
**下一輪：第四十八圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）在真的網站上量讀者看到的結構，**沒有改任何東西**。

**七個列表頁說的數字跟真的畫出來的完全一樣**（1／1、1／1、3／3、5／5、
14／14、9／9、9／9）。第一次量時 `/archive` 與 `/tags` 顯示 0 筆，
差點寫成「數字對不上」—— 那是我的選擇器只認 `article`／`.entry`，
而那兩頁用 `<ol><li>`。這一圈第三次靠「先確認工具答的是哪個問題」擋下假發現。

**英文那一側是空的，而它說得很好**：「Nothing here yet. There are 3 entries
in Chinese →」，單複數也對（1 篇時是「There **is 1 entry**」）——
那是 `list.otherLang_one` 這個動態查表的鍵在站上真的算繪出來了。
順帶量到 `list.count_one`（zh）也有，只有 `tags.count_one` 還是 0。

**五個跨語言連結裡有一個指到別的地方**：`/en/tags` 說「There are 5 entries」
並送到 `/archive`，而中文 `/tags` 有 14 個標籤。原始碼沒有註解說明 ——
沒有改（送英文讀者去看 14 個中文標籤名不見得比較好），記成站主的判斷。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 381 筆逐輪紀錄都在
