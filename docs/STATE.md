...
**下一輪：第四十四圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）問「日常發文誰來推」那一條。**答案是「真的要站主決定」**
—— 那是他跟她的工作方式，不是技術問題，這一輪沒有替他決定。

但問的時候看到另一件事：**在他決定之前，那幾個指令本身就得是對的，
而它們不是。** `docs/CONTENT.md` 教她 `git add` → `git commit` → `git push`，
**少了 `git pull --rebase`** —— 而排程每天早上 8 點與晚上 8 點會自己
commit 一次 `syndication.json`，所以她那一份每天落後兩次，
`git push` 會被擋（今天推 165 個 commit 時就是這樣被擋的）。

更難看的是：站主今天授權直接推，我當場把規則寫進 `CLAUDE.md`、`AGENTS.md`、
`REVIEW-LOG.md` —— **三份都是給 AI 看的**，而 `docs/CONTENT.md` 是這個 repo
裡唯一寫給她的，一個字都沒動。（`CLAUDE.md` 自己就講過這個形狀。）

改成四個指令，附上衝突的照抄行與「什麼時候不該照抄」；
部署時間從「71～80 秒」重量成 **71～108 秒**（原本那個範圍是拿排程部署量的，
而她推的是內容）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 349 筆逐輪紀錄都在
