...
**下一輪：第三十九圈第 6 輪 —— 文案與語氣。**

第三十九圈第 5 輪（隱私與安全）驗到一條**數字漂了**的：
「11 條 warn 沒說為什麼」—— 今天是 **13 條**。而「沒說為什麼」是真的：
`privacy-rules.mjs` 的 5 條 warn 一條都沒寫過理由，
旁邊那些長註解講的是**正則怎麼修的**，不是「為什麼這一條不擋」
（對照組 `check:a11y` 的 `SEVERITY` 表每一條都寫了）。
加了 `whyWarn` 欄位與計數（今天 **0／5**），但**刻意不填內容** ——
理由要由知道當初決定的人寫，我補的會是自己編的。
數出來是為了讓那個缺口有一個**會跟著程式走的數字**，而不是一句會過期的話。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 311 筆逐輪紀錄都在
