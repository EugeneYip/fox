...
**下一輪：第四十一圈第 7 輪 —— 建置與 CI。**

第四十一圈第 6 輪（文案與語氣）驗的一課是第十四圈的
「同一個約定有兩個地方在管，而兩邊的清單不一樣 ——
**照文件寫的人會在 CI 上被擋下來卻不知道為什麼**」。

那一課修了兩次：第十四圈要求每條規則的 **id** 出現在兩份文件裡，
第三十七圈要求文件裡的 `✗` 至少有一條規則抓得到、`✓` 一條都不能抓到。
中間那一塊沒有人守：**id 在、例子也對，但那條規則可能一個例子都沒有。**

拿 `check:copy` 自己的 `RULES` 去比兩份文件裡的 6 個 `✗` ——
**5 條規則各有 1～2 個反例，一條不漏。這一課修乾淨了。**

補守門的第一版做成「擋」，結果 `test:copy-rules` **三格當場紅了** ——
那三格的假文件為了讓 `rule-not-documented` 閉嘴而列出五個 id，
卻只放一個跟自己有關的例子。也就是說「擋」會讓每一份提到那五個 id 的文件
都得帶五個例子，換來的是一個今天 0 件的問題。改成只說不擋。
**那個代價是測試告訴我的** —— 這一輪本來差一點就變成「為了有東西交而製造改動」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 328 筆逐輪紀錄都在
