...
第四十圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪量了一件從來沒有人數過的事：**哪幾條 CSS 規則一個元素都沒配到。**
爬完全站 44 頁、335 條選擇器，**106 條（32%）一條都沒配到** ——
但那 32% 分三種，風險完全不一樣：還沒有內容 22、要跑起來才有 25、
**元件沒算繪過 59**。第三類那 14 個前綴跟這一圈前幾輪撞到的是同一批東西
（`.cover`、`.facade`、`.article__also`、`.note-source`⋯）。

留下 `scripts/probe-css-coverage.js`（第三支貼進 console 的探針），
`check:contrast` 的結尾指路。**而且真的跑過** —— 跑的時候修掉兩個自己的 bug：
`CSSStyleRule` 也有 `cssRules`（CSS 巢狀），第一版因此抽到 0 條還不報錯；
以及「只讀一頁的連結」抓到 33 頁，少掉的正好是整個 `/en` 那一半。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 322 筆逐輪紀錄都在
