...
第 2 輪（效能）往外看兩層：**檔案層沒發現問題**（61 個檔案逐個對照，
一條預算都沒 claim 的是 0 個）；**請求層有 176 個在邊界外** ——
「單頁請求數」只算 stylesheet／script src／img src，而 `rel="icon"` 那一類
132 個、`rel="manifest"` 44 個也會發出請求，平均每頁 4 個。
所以「單頁請求數 2／上限 4」不是那一頁請求的全部。
判準不動（圖示只抓一次、快取很久，要不要算是站主對這條預算意圖的判斷），
但加一行把數字說出來，並加 2 格測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 284 筆逐輪紀錄都在
