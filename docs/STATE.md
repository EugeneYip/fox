...
**下一輪：第四十五圈第 8 輪 —— 視覺與版面。**

第 7 輪（建置與 CI）挑了四條待辦。**全部還成立**，但其中一條的**判準是錯的**。

**「其餘六支關卡都以 `process.exit()` 收尾」第一次有數字。** 量了這台機器的
管線緩衝區（**65,536 bytes**）與每一支的輸出（最大 13,827，`check:perf --verbose`，
...
**判準錯的那一條**：「測試裡出現 `writeFile(resolve(ROOT, …))`」grep 出來
**0 筆**，而那件事確實存在兩處（都先把路徑存進變數）。真正找得到的判準是
結果不是寫法 —— 跑完之後 `git status --porcelain` 有沒有變（量過：沒有）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 361 筆逐輪紀錄都在
