...
**第二十九圈（要開始）：這件事，第一次做的人跟第一百次做的人，
看到的是同一份東西嗎？**
這個專案的檢查輸出愈來愈長（`verify:all` 加 `test:tools` 現在超過十分鐘、
上百行說明），而那些說明大多是寫給**已經知道脈絡的人**看的。
第一次跑的人看到的是一整面牆。第二十七圈問過「換一個人做得到嗎」，
這一圈問更窄的一件事：**同一份輸出，對新手與老手是不是兩種東西**，
以及如果是，該分開還是該收斂。
...
### 最近一圈（第二十八圈：這件事是誰決定的，那個人還在嗎）

| 輪 | 查的那個決定 | 理由還在嗎 |
|---|---|---|
| 1 無障礙 | 「刻意沒有瀏覽器相依套件」 | **過期一半**（磁碟緊已不成立） |
| 2 效能 | 11 條預算的門檻 | 理由都在；但關鍵路徑那條**用錯了框架** |
| 3 內容結構 | schema 的 8 個閒置欄位 | 都有人讀 —— 但**寫給她看的註解沒人在檢查** |
| 4 平臺 feed | 11 個 `confidence: 'verified'` | **說不出是哪一天**，git 壓縮之後也答不出來 |
| 5 隱私 | 9 條豁免 | 4 條在擋、1 條是值的家、**4 條什麼都沒擋** |
| 6 文案 | 2 條豁免 | 都在擋 —— 但 `CLAUDE.md` **整份為了一行反例** |
| 7 建置與 CI | 「下限來自 undici」 | 還精確；但 **CI 跑哪一版從來沒人查過**（v22.23.2） |
| 8 視覺 | 排版的實測數字 | **全部吻合**，沒發現問題 |

那一圈最有用的一句：**決定會傳播，理由不會。**
以及：做一個正確的決定時，也要記下它換掉了什麼
（壓縮 git 歷史是對的，代價是「這一行何時寫的」再也答不出來）。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第二十八圈走完了」。
