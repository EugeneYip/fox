...
**下一輪：第三十九圈第 4 輪 —— 平臺 feed 實測。**

第三十九圈第 3 輪（內容結構）**第一次驗到一條是錯的**：
「`PAGE_SIZE` 沒有呼叫者」—— 它是 `paginate()` 的預設參數，
而 **8 個頁面**呼叫時不帶 `size`，也就是天天在用。
而它當初怎麼寫錯的，我在驗它的時候**當場重演了一次**：
第一版探針只看「別的檔案有沒有提到」，於是報出 16 個沒人用的匯出，
`PAGE_SIZE` 就在裡面 —— 少看了同一個檔案裡的使用與 `src/` 以外的消費者。
所以把判準寫進 `check:content`（134 個匯出裡 4 個沒人用，判準印出來）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 309 筆逐輪紀錄都在
