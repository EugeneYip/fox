...
**下一輪：第四十六圈第 8 輪 —— 視覺與版面。**（第四十六圈的最後一輪）

第 7 輪（建置與 CI）量到三件事。

**第 3 輪那個 `paths` 改動有效**：連三輪的 commit 都只跑「檢查」（94～98 秒）、
**沒有部署** —— 在那之前那三次都會是完整部署並重新發佈網站。

**把 CI 的步驟一個一個拿掉：六個，六個都有人說話**
（`gate-not-on-deploy-path`、`gate-missing-in-check`、`deploy-without-cname-check`）。
這一半沒發現問題。

**「為什麼不讓 `check.yml` 直接跑三行」有答案了**：把 8 步併成 2 行，
`gate-missing-in-check` 就紅 —— 那條規則的判準就是「逐一列出」，
換到的是在 GitHub 介面上看得出哪一道紅。待辦結掉。

順手修掉三處已經不成立的說法（三個地方都寫著 `check.yml` 一次都沒跑過）——
那正是改觸發方式的代價：**文件講的事實沒有人在守**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 369 筆逐輪紀錄都在
