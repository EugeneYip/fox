...
第三十七圈第 2 輪（效能）在同一個問題上撈到一句**數得出來**的宣稱：
`docs/ARCHITECTURE.md` 在「決定性的因素是 0 KB JavaScript」下面兩行寫著
「全站的 JavaScript 只有**四小段**」，而產出裡**一直是五段**。
少掉的是 `Base.astro` 裡算繪前套用已存偏好的那一段 —— 641 B、每一頁都有、
**跑在畫面出現之前**，是五段裡唯一會擋住第一次算繪的。
講效能時漏掉的偏偏是那個會擋渲染的。兩邊都改了，並讓 `check:perf`
每次拿產出的段數去對那句話（對得上也要出聲）。
（11 條預算的上限仍然沒有任何一份文件寫過，其中 5 條是「挑的」——
那條記進待辦。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 292 筆逐輪紀錄都在
