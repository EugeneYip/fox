...
第三十八圈第 2 輪（效能）：那句「2026-09-04 實測 GitHub Pages 送出的位元組」
已經做對了大半（說得出什麼時候量的、怎麼重量）。**去跑了一次，0 頁量到** ——
五頁全部「線上跟本機不一樣」，而其中四頁位元組數一模一樣。追下去不是 bug：
**本機領先 origin 105 個 commit，其中 12 個動到 `src/`**，線上那份本來就比較舊。
也就是說那句宣稱**現在重量不出來**，而輸出不會透露。
現在探針自己去問 git 並說出來，`check:perf` 那一行也加上「（N 天前）」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 300 筆逐輪紀錄都在
