...
**下一輪：第四十八圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）問的是 `check:copy` 的**邊界**：讀者讀得到的字，
有多少落在語料外面。

**屬性那一半沒發現問題。** 44 頁裡含漢字的屬性值 517 次、相異 78 個，
其中 14 個只活在屬性裡（`aria-label`、`placeholder`、`<meta content>`）——
`toVisibleText()` 確實看不到它們，但它們的出處是 `ui.ts` 與 `site.ts`，
而那兩份另外有人掃。繞道，但蓋得到。

**真的在外面的是 `dist/` 裡不是 `.html` 的檔案。** 掃 `dist/` 的迴圈第一行
就是 `if (!f.endsWith('.html')) continue;`，而 `rss.xml`／`rss-all.xml`／
`robots.txt` 都是出貨的中文。突變證明：`rss-all.xml.ts` 的「臺」改成 `台`，
建置過、**`check:copy` 離開碼 0**，那一句就這樣出現在每個訂閱者
的閱讀器裡；同一個字改在 `ui.ts` 就當場紅。

**修的是重複的那一半**：feed 標題的「（含各平臺）」跟 `ui.ts` 的
`rss.allSuffix` 是同一個字，改成走 `ui.ts`，兩份 feed 逐行比對
**一個 byte 都沒差**。剩下 9 條（robots.txt 8、feed 說明 1）記進 TODO。

（第一版量出「49 段沒有人掃到」是我自己比對方式的問題 —— 其中 26 段是
正文、句中夾著 `<strong>` 比不到，9 段是她的 YouTube 字、本來就刻意排除。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 384 筆逐輪紀錄都在
