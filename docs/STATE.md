...
**下一輪：第四十一圈第 8 輪 —— 視覺與版面。**

第四十一圈第 7 輪（建置與 CI）驗的是第二十六圈那一課：
**「需要 `dist/` 的東西排在建置之前 —— 本機永遠有 dist 所以兩套關卡全綠，
乾淨的 runner 上才會壞」**。

同一個形狀還在，而且在更要緊的地方：`audit:privacy` 在**兩份 workflow 上
都排在建置之前**。把 `dist/` 移走再跑，它少判斷了 **10 條**（31 條裡）——
包括 `built-third-party-request`（產出裡有沒有第三方請求，**這個專案最硬的
那條承諾**）與四條 CSP 規則。

**為什麼二十六圈那次抓得到、這次抓不到**：那條規則守的是「沒有 dist 就會
失敗」的那幾支，而 `audit:privacy` **不會失敗** —— 它照跑，只是少驗 10 條。

改了三處：`verify:all` 與 `check.yml` 都把它移到建置之後；`NEEDS_DIST` 收進
`audit:privacy`，並且**分開兩種後果**（原本訊息寫「一定會失敗」，
對這一支是假的）。從乾淨的樹量：**閒置從 10 條變 1 條**，CI 上跑得到的
隱私規則從 21／31 變成 **30／31**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 329 筆逐輪紀錄都在
