...
**下一輪：第四十三圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed 實測）先照本來的工作打了一次：樣板 11 個全過、
她自己的來源 `youtube-foxpoetry` 200 Atom 9 筆、24 個平臺的 `confidence`
一個都不用改。**沒有平臺改版或下架。**

問題出在第三個綠勾：`sync:health` 說「**1 個來源**，全部都在 3 天內成功過 ✓」，
而那個 1 只讀 `syndication.json` —— **分母是「上一次同步記了誰」，
不是「設定檔宣告了誰」**。

同步有兩條路會跳過一個來源而且不留紀錄（`platform` id 不存在、
handle 還是 `CHANGE_ME`），後者連 `failures` 都不加；排程跑的又是沒有
`--strict` 的那一版。實測兩個開著的來源 → `syndication.json` 記下的是
「一個都沒有」。**填錯平臺 id 的來源，從頭到尾不會有任何一句話提到它。**

改成多收一份「設定檔宣告了誰」，用的是 `sources.mjs` 自己就有、
而第 3 輪剛被列為「沒有人用」的 `enabledSources()`。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 342 筆逐輪紀錄都在
...
前四輪的答案一樣：一支關卡可以有好幾個分母，而**推導出來的那幾個通常沒事，
出事的是決定語料的那一份** —— 第 1 輪是那句說明涵蓋範圍的話只認連結，
第 2 輪是決定「用哪一把尺量」的副檔名清單，第 3 輪是決定「看得到哪些檔案」的
另一份副檔名清單，第 4 輪則是**分母整個取自產出的檔案，而不是設定檔**。站上這種句子還有很多
