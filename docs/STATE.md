...
**下一輪：第四十五圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed 實測）挑了五條 feed 的待辦，**五條全部還成立**，補掉三條。

先量今天：她的來源 `200 Atom 9 筆`；11 個有 `probeHandle` 的平臺全部 200；
`sync:health` 綠。YouTube 這次沒有間歇性壞掉。

**補掉的三條。** `FLAKY_ENDPOINT` 那份手寫的 Set 沒有人比 —— id 打錯的話
「不要當成帳號沒了」那段提醒永遠不會印，而且不會有錯誤訊息。
五份對照表只驗正向，而註解裡「4／5」那組數字是寫下來就沒人再算的；
現在讓 `label()` 順手記，每次自己算（`KIND` 裡的 `api` 今天什麼都沒翻譯）。
第三條掛了最久：`note` 回 200、合法 RSS、**0 筆**，而那一列的符號跟旁邊
抓到 30 筆的 `hatena` 一模一樣 —— 現在 0 筆是 `·` 不是 `✓`，離開碼不動。

**這一輪自己犯了四個，每一個都是被別的東西擋下來的**：拿表名猜欄位名
（`KIND` → `kind`，真名 `feedKind`）、新訊息寫進 stderr 而其餘 28 句在 stdout、
圖例的用字讓一格既有的反面斷言變成永遠成立、JSDoc 放在 `/* */` 裡。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 358 筆逐輪紀錄都在
