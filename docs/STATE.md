...
**下一輪：第三十六圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）問同一件事，而 `check:copy` 的前半早就有了 ——
它說得出**什麼**不在範圍內（程式碼註解與內容正文）。這一輪補的是後半：
那外面現在有多少。用它自己匯出的那五條規則量：程式碼註解有 **8,619 行**
含漢字，而它掃的是 2,571 行 —— **邊界外面比裡面大 3.4 倍**，命中 67 處。
沒有去改那 67 處：約定寫的是「網站上的中文」，註解不在網站上，
擴上去會是自己發明的規矩；而且有一部分是刻意在引用違規本身。
更值得看的是另一半：**草稿正文是發佈那一刻才第一次被檢查的**
（正文不掃的理由是「發佈後會進 dist」，但草稿不會進 dist）。
走不到程式碼時它會說「這次沒有量到」，不印一個漂亮的 0。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 288 筆逐輪紀錄都在
