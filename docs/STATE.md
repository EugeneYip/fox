...
**下一輪：第四十九圈第 8 輪 —— 視覺與版面。**

**第四十九圈問的是：這道關卡如果安靜地失效了，我們會不會知道？**

第 7 輪（建置與 CI）往上一層問：前六輪問的是「一條規則不再報了會怎樣」，
這一輪問「**一整道關卡、或一整支測試，如果根本沒跑呢**」。四個實驗：

| 拿掉什麼 | 誰說話 |
|---|---|
| `check.yml` 少列一道關卡 | `gate-missing-in-check`，exit 1 |
| `verify:all` 少一道 | `gate-count-stale`，exit 1（那條看起來只是文件整潔的規則） |
| **`test:units` 少一步** | **沒有人說話** |
| 連 script 定義一起刪 | `test-file-not-run`，exit 1 |

第三、四格放在一起才看得出 `test-file-not-run` 真正在守什麼：
**「每支 `test-*.mjs` 有沒有對應的 npm script」**，不是「那個 script 有沒有被跑到」。

後果量過：把 `test:a11y-rules` 從 `test:units` 拿掉，`test:tools` 印
**「43 步全部通過」、exit 0** —— 而那一步正是第 1 輪證明「32 條無障礙規則
弄啞都會被抓到」的唯一那支測試。螢幕上唯一的差別是 44 變 43，沒有人在比。

**今天沒有真的漏掉的**：38 個 `test:*` 腳本逐一對照，38／38 都被套件列到。
所以是空著的洞不是漏水的洞；判準寫得出來而且今天零誤報，
但說不出過去十輪裡它會擋下哪一次（規則 10），記進 [TODO.md](TODO.md)。
