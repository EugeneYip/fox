...
第 2 輪（效能）拿同一個問題去問兩條掛著「→ 站主」的待辦，
而它們的答案**完全相反**。

**一、「『上界』宣稱要重量得先推」** —— 那個阻塞當天就沒了（推的權限下放）。
所以就量了：線上四頁的差距**一字不差**還是 `-1.7% ～ -0.4%`，
「上界」那句宣稱仍然成立，日期更新成 2026-09-06。
順便也結掉「重驗是量本機產出不是正式站」—— 這四頁是線上的位元組，
而且 md5 逐頁核對過線上與本機是同一份。

**二、「雜湊資源只有 `max-age=600`」** —— 去線上量 header：連內容雜湊過的
`/_astro/*.css` 也是 600 秒，而 **GitHub Pages 不讓你設快取標頭**。
所以這一條不是「還沒有人去改」，是這個主機上沒有那個開關。
但代價比聽起來小：有 ETag，超時之後是 **304、body 0 bytes、header 363**，
不是重新下載 3.7 KB。`check:perf` 那句「第二頁起⋯就只剩 HTML」
因此補了期限與代價 —— 那一行是整支關卡唯一對著讀者說話的地方。

**「→ 站主」把兩種東西寫成了同一種**：一種是「有人做得到，只是不是我」，
另一種是「這裡做不到」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 348 筆逐輪紀錄都在
