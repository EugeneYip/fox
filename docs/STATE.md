...
**下一輪：第四十五圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）挑了五條待辦。**一條是假的、四條還成立**，補掉三條。

**假的那一條正是這一圈存在的理由**：「`audit:privacy` 排在 `build` 前面，
讀 `dist/` 的規則在 CI 上一次都沒真的判斷過東西」—— `check.yml` 現在是
先 build（37 行）再稽核（47 行），第 7 輪（第四十一圈）就修好了。
待辦被一輪一輪抄下來，而抄的時候沒有人回去看它還成不成立。

**補掉的三條。** 兩支 walker 各寫一份的「跳過 node_modules⋯」清單併成一支。
新規則 `unscanned-tracked-file`（error）：拿 `filesToScan()` 自己走出來的
那一份跟 `git ls-files` 相減 —— 量到 202 個追蹤的檔案、視野涵蓋 195 個、
沒讀的 7 個全說得出理由。第三條是把 33 條規則的嚴重度釘進
`test-privacy-structural.mjs` 的 `SEVERITY`（`check:a11y` 那邊早就這樣做）；
在這之前把一條 error 一致地改成 warn，全部都是綠的。

**為什麼要擋是量出來的**：把根目錄那行的 dotfile 逃生門拿掉，
`.env.example` 從此不在視野裡，而稽核仍然印「必須修正 0」、離開碼 0。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 359 筆逐輪紀錄都在
