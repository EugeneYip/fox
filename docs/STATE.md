...
第 2 輪（效能）先查最明顯的重複 —— **壓縮**：全 repo 只有兩處在算 gzip，
**兩處都是 `level: 9`**，同一個檔案量出來 10752 對 10752，沒有分岔。

真正有兩份的是「**這是不是文字檔**」：`check-perf` 的 `isTextLike` **看內容**
（NUL ＋ 嚴格 UTF-8），`audit-privacy` 的 `SCAN_EXT` **看副檔名**（17 項名單）。
餵同一棵 `dist/`（61 個檔案）：51 個兩邊都說是、**4 個只有看內容說是**（四個 `.xml`）、
0 個只有名單說是、6 個都說不是（5 png ＋ 1 ico —— 看內容那邊不用名單就分對了）。
那 4 個 xml 是刻意排除的，稽核自己的輸出就講了理由。

**名單那一邊有結構性的洞，而且被咬過一次**（`.webmanifest`，第十圈：
同樣的內容、同樣的目錄，差別只有副檔名）。今天常見的純文字副檔名裡
**15 種不在名單上**。

**但那個洞今天被第三條規則守住**：拿假的針實測 —— 根目錄的 dotfile 抓得到、
`probe.toml` 沒進版控時靜悄悄、**`git add` 之後 `unscanned-tracked-file` 當場點名**。
代價是那條規則哪天鬆掉，缺口就跟著開，而它們是兩個地方。記進 [TODO.md](TODO.md)。
