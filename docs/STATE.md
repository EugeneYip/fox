...
**下一輪：第四十三圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）問 `check:copy` 那句「**108 組文案全部都有 en（100%）**」。
那個分母是一份**兩個檔案**的清單（`ui.ts`、`site.ts`），而站上的 L10n 物件
不只寫在那裡 —— 頁面自己也寫 `pick({ 'zh-TW': …, en: … }, locale)`。

逐個數：147 組裡有 39 組在清單外（`platforms.data.mjs` 25、三個頁面 11、
其他 3）。**那個 100% 涵蓋的是 73%。**

而 `pick()` 收的是 `Partial`，`en` 型別上選填。實測拿掉 `about.astro` 的
`en: 'About'`：型別 0、build 0、`check:copy` 0 並照樣印 100%，
而 `/en/about` 的 `<title>` 變成「關於 — Fox Says」、`<h1>` 是「關於」。

分母改成「`src/` 底下每一個有 `zh-TW` 的物件」（`.astro` 用文字括號配對抽，
並拿那兩個 import 得了的檔案自我驗證 108 = 108，對不上就說「不敢算」）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 344 筆逐輪紀錄都在
