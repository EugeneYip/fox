...
第 4 輪（平臺 feed 實測）有三條寫下來的做法，三條都照著走一次，**都走得完**：

- **CLAUDE.md 的「feed 掛了怎麼判斷」**（看她的頻道頁而不是別的頻道）——
  `@FoxPoetry` 回 200、canonical 跟 `sources.mjs` 的 channelId 一字不差。
- **「`--patterns` 那一格證明不了她的頻道沒事」** —— 照著跑，
  她的 channelId 在輸出裡出現 **0 次**，打的全是公開樣板頻道。那句警告仍然準確。
- **`npm run handle <帳號名>` —— 第一次有人跑。** 19 個平臺、4.8 秒，
  輸出正是文件說的「可貼進 `sources.mjs` 的片段」。

那一次跑出來的東西正好證明硬性限制為什麼要那樣寫：`FoxPoetry` 這個名字
**3 個平臺開得起來，兩個是同名的陌生人**（Blogger 標題「Walt & Emily」、
X 標題「Faerie Fox Poetry」），只有 YouTube 是「狐說八道」。
沒有標題那一欄的話，照著「有東西就貼」做會多兩筆假來源。
**這一輪沒有把任何東西加進 `sources.mjs`，清單仍然只有一筆。**

反貧化：拿一個不可能存在的名字跑，打勾 0 個、印「沒有明確的命中。」，
5 個天生查不準的平臺照樣標成 `?` 而不是假綠勾。
