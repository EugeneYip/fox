...
**下一輪：第四十六圈第 1 輪 —— 無障礙。**（第四十五圈八輪都走完了）

**第四十五圈問的是：這條待辦還在那裡，是因為它還成立嗎？**
八輪、四十筆待辦回去對照現在的程式。四句話：

**一、待辦會安靜地過期。** 四十筆裡**四筆已經不成立** ——
「`audit:privacy` 排在 `build` 前面」（第四十一圈就修好了）、
「`check:copy` 不拿掉行內反引號」、「`list.count_one` 沒算繪過」、
「還有沒有別的測試會動到版控裡的檔案」（判準 grep 出來 0 筆）。

**二、過期的待辦會擋住真正的問題。** 「行內反引號」那條前提是錯的，
但**問下去**找到真的沒人看的：反引號**配不成對**時，一段文案會安靜地
離開 `check:copy` 的視野，而「掃了 61 個檔案、13917 行」一點都不會變。
「`BG_PROPS` 三個裡只有一個」問下去是 **19 個裡只有 4 個**，
其中 10 個是三圈前就該刪沒刪的重複。

**三、「還成立」的待辦，缺的多半是一個算得出來的數字。**
這一圈補的守衛幾乎都是同一個形狀：把寫下來就沒人再算的東西改成每次自己算 ——
manifest 的四個圖示、Astro 自己的 schema、版控 202 個檔案的視野涵蓋、
33 條隱私規則的嚴重度、五份對照表的鍵、`EN_COVERAGE` 的組數與天數、
管線緩衝區的 21% 餘裕。

**四、我最常犯的錯是「不問系統本人」**（這一圈四次）：拿表名猜欄位名、
重寫一次 `SCAN_DIRS`／`SCAN_EXT` 而漏掉 dotfile 逃生門、
在檔案裡 grep `aria-current` 分不出是哪一個 nav、兩次把 JSDoc 寫進 `/* */`。
每一次都是被別的東西擋下來的，沒有一次是重讀自己的程式發現的。

**留下什麼**：規則數 `audit:privacy` 32 → 33、`check:copy` 9 → 10；
新守衛七個；拿掉三處重複；四個數字不再靠人記；
看得見的改動只有一處（頁尾「你在這裡」的顏色與底線）；
`REVIEW-LOG.md` 開頭那張圈末索引從第二十六圈補到第四十五圈（44 段全在）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 362 筆逐輪紀錄都在
