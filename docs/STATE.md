...
**下一輪：第五十圈第 7 輪 —— 建置與 CI。**

**第五十圈問的是：照著文件做，做得完嗎？**

第 6 輪（文案與語氣）照著走的是 `docs/CONTENT.md` 的「用字的幾個約定」——
五條規則、每條一組正反例，開頭寫著「這幾條是**會擋你的**」。
做法很明確：把五個反例各寫一句進一篇真的文章，發佈，跑檢查。

**五條裡只有三條停下來。** `straight-quotes` 與 `halfwidth-ellipsis` 一聲都沒出 ——
去看產出才知道原因：markdown 在進 `dist/` 之前就把那兩種字元換掉了，
而 `check:copy` 掃的是 `dist/`，所以那兩條在文章正文上永遠沒有主體可判斷。
（第一次只看到「兩條沒響」，差點寫成「規則壞了」。）

**兩條的後果不一樣**：刪節號換出來的字元本來就合格；
**引號換出來的是彎引號，而那不是文件說的那種，也沒有任何檢查會擋**。
量過今天沒有踩到（內容裡沒有人打過直引號，站上唯一有彎引號的是 `/en/privacy`，
那是英文本來就該用）—— 空著的洞，不是漏水的洞。

正例那一半乾淨：同一篇換成文件給的正例，`check:copy` 離開碼 0、0 命中。

**改的是文件不是規則**：那一節補了一段說明哪兩條會被自動換掉、
換出來的合不合格、以及「在 `docs/`、`README` 或介面字串裡五條全部都會擋」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 400 筆逐輪紀錄都在
