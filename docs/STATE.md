...
**下一輪：第四十五圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）挑了四條待辦：**一條前提是錯的、一條半錯、兩條還成立**。

前提錯的那條寫著「`check:copy` 掃之前會拿掉程式碼區塊，但**行內**的
反引號片段呢」—— `stripCode()` 的第二個 `replace` 就是行內那種，
註解第 45 行也白紙黑字寫著。那條是憑印象寫的。

**但問下去有一件真的沒有人在看：配不成對。** 少打一個反引號，
它會跟後面某個不相干的反引號配成一對，中間那段文案就被當成程式碼拿掉 ——
**不是誤報是漏報**，而且「掃了 61 個檔案、13917 行」一點都不會變。
新規則 `unbalanced-backtick`（規則數 9 → 10），圍欄用逐行狀態機數。
今天那 10 份文件語料全部成對，所以它是預防性的。

順手發現佔比那份手寫的排除清單漏了 `example-not-real`，改成問 `RULES` 本人。
`EN_COVERAGE` 的組數比對補上「變多」那一半，日期改成算得出來的天數。

半錯的那條：`list.count_one` 早就算繪過了（zh 15 頁、en 3 頁），
沒證據的只剩 `tags.count_one`。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 360 筆逐輪紀錄都在
