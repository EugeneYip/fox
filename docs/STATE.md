...
**下一輪：第四十六圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）拿掉兩個機制，兩個都守得住（`.gitignore` 那一行 →
`gitignore-weakened`；整段 CSP → `csp-missing`）。

**然後量到一條從來沒被說出來的邊界**：把 `privacy.ts` 的 `showRealName`
翻成 `true`，值真的進了 `dist/about/index.html`，而 `audit:privacy`
**離開碼 0、一個字都沒說**。那不是 bug 是分工 —— 身分規則掃的是 `SCAN_DIRS`，
`dist/` 不在裡面；這支守的是「不進 repo」，而值要不要上站是站主的開關。

所以沒有加規則（規則 10：它不該擋），加了一個**數字**：
「站上有幾個：0 個（讀了 dist/ 的 51 個文字檔）」。翻開開關就變成 2 個。
永遠只印數量與檔案數，不印值本身。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 367 筆逐輪紀錄都在
