...
**下一輪：第四十六圈第 4 輪 —— 平臺 feed 實測。**（但先看站主那幾個問題）

第 3 輪（內容結構）把 `lib/content.ts` 的零件一個一個拿掉再比 `dist/`：
草稿過濾與語言過濾都守得住，**而 `getEntries` 的 `sort` 拿掉會讓 dist 有
4 個檔案不一樣，卻沒有任何一道關卡說話**（列表順序與上一篇／下一篇的鄰居）。
補了 `listing-order`（規則 23 → 24），驗的是性質不是重寫排序。

兩條 `limit` 拿掉之後 dist 一個字都沒變 —— 站上只有 6 篇，切不到。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 365 筆逐輪紀錄都在
