...
第四十圈第 5 輪（隱私與安全）：規則那一端早就答完了（30 條每一條都有主體），
所以問到這個專案最硬的那條承諾 —— **「影片要按了才會載入」**。
扛它的 `VideoFacade` 在站上一次都沒算繪過，也就是說
**整條同意路徑從來沒有人走過**。

建一頁暫存的，起 preview，第一次真的按下去：載入時 **0 個 iframe**、
3 個請求全是 localhost、console 一句都沒有；按下去之後 iframe 建起來，
`frame-src` 讓它過了。**同意路徑是對的。**

按下去那條路上有兩個地方必須講同一個主機（handler 建的網址、每一頁的 CSP）。
只要有一邊改了，讀者按下去會得到**一個永遠不會出現的播放器** ——
畫面上沒有錯誤，只有 console 裡一行 CSP 訊息。而驗它今天要手動建頁再按一次。
所以加了第 31 條規則 `csp-frame-src-mismatch` 靜態地比一次，突變驗過。
暫存頁刪掉之後它的主體回到 0 —— 關卡自己會說，那正是這一圈在問的事。

**我差一點寫下一個假的資安發現**：在一個累積了半輪狀態的分頁裡看到
一行被擋掉的 `i.ytimg.com` 圖片請求。乾淨分頁重跑什麼都沒有、
`grep -rl ytimg dist` 整個產出一次都沒有、網路紀錄也沒送出去。
又是拿一個被污染的儀器去量。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 319 筆逐輪紀錄都在
