...
第 2 輪（效能）挑的是 `check:perf` 每次收尾自己講的那句
「11 條預算裡，**6 條說得出上限是怎麼推導的**」—— 照著那 6 條各推一次。

**六條都跟得上，但只有三條真的能「重新走一次」。** 另外三條的輸入是
「訂的時候的現值」，那個數字只存在於那一句話裡，照著做只能驗算術。

重新量得出來的兩條**跟文件一字不差**：feed 滿載 100 筆 × 273 B = 26.7 KB
（文件「約 27 KB」）、搜尋索引每筆 426 B → 144 筆（文件一模一樣）。

**第三條的註解自己寫著「那個數字一直沒有真的被量」——這一輪量了**：
一個雜湊 **41 B**（文件 43，量法是把首頁 CSP 拿掉一個雜湊再 gzip）、
`auto` 對 `never` 的領先 **323 B**（文件 294，量法是真的建一次 `never`）。
照新數字算翻轉點是 42.9 而不是 41.8 —— **上限訂在 41 仍然在安全側**，
所以沒有動它，只把量到的數字寫進那段註解。

那句自我描述可以更誠實（實際是 3 條可重推、3 條只能驗算術、5 條是挑的），
但要分成三類得多一個判準，規則 10 說不出過去十輪會擋下哪一次，記進 [TODO.md](TODO.md)。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 396 筆逐輪紀錄都在
