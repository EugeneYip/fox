...
**下一輪：第三十七圈第 8 輪 —— 視覺與排版。**

第三十七圈第 7 輪（建置與 CI）先把 `CLAUDE.md` 對建置流程的四個可查宣稱
逐條對過 —— **全中，沒發現問題**。問題不在對不對，在**寫了幾次**：
`verify:all` 有幾道關卡這件事，人會讀的文件裡寫了**四次**
（`CLAUDE.md`、`AGENTS.md`、`DEPLOY.md`、`STATE.md`）。加一道，四份同時錯。
而它**已經過期過一次** —— `CLAUDE.md` 還留著「原本這裡寫五道」那一行。
新增 `gate-count-stale`：拿 `package.json` 的 `verify:all` 真的有幾步去比。
實測把它加成 7 步 → 四份文件全部被點名。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 297 筆逐輪紀錄都在
