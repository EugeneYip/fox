...
**下一輪：第四十二圈第 7 輪 —— 建置與 CI。**

第四十二圈第 6 輪（文案與語氣）：這一塊的清單多數有人守
（`NOT_A_WRITING_RULE` 走 `documentationDuty` 的 `unknown`、
`EN_COVERAGE` 每次跟實際比）。沒有東西在比的是 **`EXTRA_RULE_IDS`** ——
而它跟上一輪那份 `STRUCTURAL_IDS` 是**同一個角色**：補 0 那一行用的名單，
而且結尾那句「N 條規則」也是 `RULES.length + EXTRA_RULE_IDS.length` 算的。

少登記一條有兩個後果，**兩個都不會紅**：那條規則在區塊沒跑到時整條消失、
結尾印的規則數少一條。**而這不是假想的** —— 上一圈第 5 輪就在
`audit:privacy` 上抓到三條沒登記的。

量出來今天兩個方向都是 0，補上跟上一輪同一格的檢查，突變兩邊都驗過。
（第二個突變順帶讓三格紅了 —— 那個幽靈 id 也流進文件義務的計數裡，
是額外的證據：這份名單不只餵一個地方。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 336 筆逐輪紀錄都在
...
**第四十二圈（進行中）：這份清單是誰維護的？漏一個會怎樣？**
判準：**找出一份人手維護的清單，問兩件事 —— 它有沒有辦法從真正的來源
推導出來？推導不出來的話，漏一項的時候誰會說話？**
第四十一圈驗了十三課，**沒有一課是學錯的** —— 漏掉的永遠是同一課的另一半，
而六條裡有四條的兇手是同一個：**一份人維護的清單少了一項**
（`ALL_USER_AGENTS`、`STRUCTURAL_IDS`、`NEEDS_DIST`、`docs/DEPLOY.md` 那張表）。
每一份當時都列對了，錯的是它們沒有跟真正的來源綁在一起。
所以下一圈逐份問：推導得出來的就改成推導；推導不出來的，
要有東西在漏項的時候出聲。

（第四十一圈最貴的一條：`audit:privacy` 在兩份 workflow 上都排在建置之前，
於是 31 條規則裡有 **10 條在 CI 上一次都沒跑過** —— 包括「產出裡有沒有
第三方請求」那條硬性限制。**它從來沒紅過，它只是少驗。**
改完 CI 上跑得到的隱私規則從 21／31 變成 30／31。）
