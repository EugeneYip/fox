...
第 1 輪（無障礙）拿這一圈的問題去問三條掛著「靠人」的事：
**「探針還是要人手貼」為什麼等於「要站主」？**

**不等於。** 它需要的是一個拿得到瀏覽器的人或代理人 —— 而這個工作階段就有。
所以就跑了：`npm run preview`、375px、把探針**原檔** `eval` 進去，
基準那五頁 **25 個數字一個都沒有變**，另外多量了三頁
（`/search`、`/writing/translating-poems`、`/en`），違規一樣是 0。

**英文首頁的餘裕比中文大**（25.9 vs 17.9）—— 最擠的一對是頁尾的
「搜尋 ↔ RSS」，英文寫 `Search`／`RSS` 字長框寬，中心離得遠。
**只量英文頁量不到最壞情況。**

順便把第四十三圈留下的那句「箭頭有 aria-hidden，但那是人寫對的，
不是檢查出來的」也結掉：在瀏覽器裡打「李白」讓結果畫出來，
外連的 rel、可及名稱、尺寸都量過了。

**第三件沒有變**：螢幕閱讀器還是沒有人做過 —— 那一件需要耳朵，不是瀏覽器。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 347 筆逐輪紀錄都在
