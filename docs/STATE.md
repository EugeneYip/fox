...
**下一輪：第三十五圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）數「掃了 176 個檔案」那個數字，**追到第三次才對上**
（整個 repo 191 → 加副檔名過濾 186 → 照 `filesToScan()` 的 `SCAN_DIRS` 走
再扣掉 9 個豁免 = 176 ✓）。中途拿 node 與 python 各走一次，兩邊都是 186 ——
所以錯的不是實作，是**我對語料的假設**。

那個數字其實是 `possible-secret` 這條規則的主體數，也就是**豁免之後**的。
185 走過、9 個豁免、176 真的看過 —— 而被跳過的那 9 個正是唯一可能藏著真值的。
已改成同一行說出豁免了幾個（現算的）。改那一行弄紅了兩格既有測試，
**那證明它們是真的在守那一行**，把樣式放寬即可。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 279 筆逐輪紀錄都在
