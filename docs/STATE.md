...
**下一輪：第四十七圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）：`privacy.ts`、`ExternalLink.astro` 都是 210 個 commit
沒動過，`docs/PRIVACY.md` 191 個。**四個前提，四個都還成立** ——
PRIVACY.md 說稽核在看的五件事今天全部有對應的規則（33 條）；
「⚠ 沒有接上」的標記有 `privacy-doc-unwired` 守著；
**沒有 `identity.local.ts` 時建置照樣過、`about` 頁上沒有 undefined**（那是 CI
上真正的情況，210 個 commit 沒人再走過）；產出裡 **58 個外連、58 個帶 `rel`，
58 個 `target="_blank"` 全部有 `noopener`＋`noreferrer`**。

**這一輪唯一的發現是我自己的錯**：我跑
`audit-privacy.mjs --list-rules` 把輸出當規則清單，報出「五件事全部少了對應規則」
—— 而真正的原因是**那支腳本根本沒有這個旗標**，不認得就當作沒看到、照常跑完，
我把它 34 行的輸出當成了 26 條 id。改成從原始碼抽再問一次：33 條，五件事全對得上。

**這是這一圈第三次「工具做的不是我以為的事」**（前兩次是 `grep -c` 數行、
`grep -rl` 配到樣式而不是標記）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 375 筆逐輪紀錄都在
