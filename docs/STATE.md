...
**下一輪：第四十九圈第 6 輪 —— 文案與語氣。**

**第四十九圈問的是：這道關卡如果安靜地失效了，我們會不會知道？**

第 5 輪（隱私與安全）的賭注最大 —— 它是「本名／信箱進不進得了一個公開 repo」
中間唯一那道門，而它被這種錯咬過一次（第二圈：規則把真值寫死在正則裡，
而那個檔案在自己的豁免名單上）。

**33 條規則一條一條弄啞：27 條讓 `test-privacy-structural.mjs` 變紅，6 條兩套都綠。**
那 6 條的正則其實有人守得很細（`test-privacy-rules.mjs` 每條都有 `catch`／`skip`），
但它 **import 庫、從來不執行 `audit-privacy.mjs`** —— 所以沒有人守的是**接線**：
稽核有沒有真的拿那些正則去掃、並且把掃到的報出來。
（`CASES` 涵蓋 33 條裡的 20 條。記進 [TODO.md](TODO.md)，規則 10。）

**賭注最大的那條路第一次在本機重現**：用 `git archive HEAD` 拉一棵乾淨的樹
（`identity.local.ts` 在 `.gitignore` 裡，不會被帶進去），沒有 `PRIVACY_NEEDLES` ——
本機 exit 0 但總結那一行自己帶著但書「**但身分規則這次沒有執行**」，
`CI=1` 則 **exit 1** 並點名 Environment secret 那個陷阱。
以前這條路只有在真的 CI 上被走過一次（2026-09-04 那次紅燈）。

（另外：稽核說 33 條而我抽出來 32，差的是動態登記的 `identity-value` ——
在寫下「數字對不上」之前先把它找出來了。）
