...
第三十九圈第 1 輪（無障礙）逐條驗無障礙那九條待辦 —— **全部還成立**，
但多數原本只是一句話，這一輪把今天的數字補上（`<details>`／`<summary>`／
`<time>` 是 44／44／82；沒有 href 的 `<a>` 是 0 個所以那條沒有主體⋯⋯）。
其中一條驗出來**有 46 個主體又完全沒人守**：`aria-live`／`role="status"`
（44 個主題狀態列每頁一個、搜尋頁 2 個）。值打錯**畫面上什麼都不會變**，
那一格從此不再朗讀 —— 而唯一會發現的人是正在用螢幕閱讀器的人，
而這個站的螢幕閱讀器測試「現況：沒有人做過」。補上 `live-region-value`。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 307 筆逐輪紀錄都在
