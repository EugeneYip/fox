...
**下一輪：第三十二圈第 5 輪 —— 隱私與安全。**

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 254 筆逐輪紀錄都在
...
**第三十二圈（進行中）：這件事第一次做對了嗎？如果不是，第幾次才對？**
第三十一圈八輪裡有六輪的第一版是錯的，而每一次都是突變掃描抓到的。
更值得注意的是那些第一版都**通過了全部測試** —— 它們不是寫壞了，
是寫成了一個測不出來的樣子。
判準：**這段程式碼如果第一版就寫錯，今天有沒有東西會說話？**

（第三十一圈問的是：是我們選的，還是它剛好長成這樣？
八輪八個，**沒有一個是「有人選錯了」** —— 全部是「沒有人選過，
而它剛好是對的」。最難發現的一種錯是「剛好相等」。）
...
### 最近一圈（第三十一圈：是選的，還是剛好長成這樣）

| 輪 | 查的那個「決定」 | 結果 |
|---|---|---|
| 1 無障礙 | 28 條規則的 error／warn | 只有 **3 條**被釘住；4 條 warn 沒說為什麼 |
| 2 效能 | 11 條預算的上限 | 餘裕 **1.50 到 17.14 倍**，四種訂法混在一起 |
| 3 內容結構 | 語言清單 | **四份**，沒有東西檢查它們一樣；外加兩個日文化石 |
| 4 平臺 feed | 「沒打到」的統計 | **8 解釋不了 9** —— 失效的樣板從統計裡消失 |
| 5 隱私 | 27 條規則的嚴重度 | 一個 id **兩種等級**，而沒人說為什麼不同 |
| 6 文案 | 排除清單 | **一條都沒濾掉** —— 理由跟程式做的事不是同一件 |
| 7 建置與 CI | 部署必跑清單 | 寫死的**剛好**等於推導的，所以沒人發現它沒在跟 |
| 8 視覺 | 65 個 token | 「沒人用」少算 2 個 —— 被一個死掉的 token 撐著 |

**八輪八個，沒有一個是「有人選錯了」** —— 全部是「沒有人選過，
而它剛好是對的」，然後在某個還沒發生的改動上會變成錯的。

那一圈最有用的一句：**「剛好相等」是最難發現的一種錯** ——
兩個定義外延相等的時候，任何測試、任何輸出、任何突變都分不出來。
以及：**理由跟程式做的事可以不是同一件，而輸出一模一樣。**

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十一圈走完了」。
