...
第三十九圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪把 `CoverImage` 的 `sizes` 量了一次 —— 原本寫的
`(max-width: 40rem) 100vw, ${width}px` **三個部分都不對**：
版面上沒有 40rem 這個斷點、圖填的是欄寬不是視窗、
而 `width` 是內在尺寸跟畫多寬無關。兩個用處的容器還不一樣
（文章 `.wrap--prose` 34rem、詩詞頁 `.wrap--content` 46rem，
而 `width` 傳的是 900／760）。改成從 `.wrap` 的 CSS 推出來的
`min(100vw - 2 * clamp(1.25rem, 5vw, 3rem), <欄寬>)`，
420px 視窗 DPR 2 實測從挑 1200w 變成挑 800w（少 37%）。

**但站上今天 0 張封面圖，所以這個改動現在改變不了任何一個位元組** ——
要到她第一次填 `cover` 那天才生效。這件事直接變成第四十圈的問題。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 314 筆逐輪紀錄都在
