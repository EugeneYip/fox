...
第四十一圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪驗兩課，**兩課都乾淨，所以什麼都沒改**：
「Astro 的 scoped style 不會穿進子元件」—— 16 個元件標籤帶 class，
父層有寫樣式的 2 個**都是誤報**（`Heading` 是 `const Heading = headingLevel`
的動態標籤名，不是元件），去產出上驗過選擇器與元素的 cid 對得上；
「直排的軸向是反的」—— `writing-mode` 只有一個檔案，裡面 3 處實體屬性
**全是 `@media (max-width: …)`**，媒體查詢本來就是實體的。

想過補一支掃描，沒有做 —— 16 個位置、0 個問題，而判準要分得出
「匯入的元件」與「動態標籤名」。第 6 輪那次的教訓還很新。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 330 筆逐輪紀錄都在
