...
**下一輪：第三十四圈第 8 輪 —— 視覺與排版。**

第 7 輪（建置與 CI）：網域有三份（`site.ts` 的 `url`、`astro.config.mjs` 的 `site`、
`public/CNAME`），今天一致，而**沒有任何東西比對它們** —— `deploy.yml` 只檢查
`dist/CNAME` 存在、不看值。而 `astro.config.mjs` 自己的註解就寫著「site 一定要
正確，否則 sitemap、RSS、og:image 的絕對網址會是錯的」：重要性寫下來了，
一致性沒人看。答案早就在手上——隔壁的 `locale-list-drift` 已經把那兩個檔案
讀進來了。加一條 `domain-drift`（主體 3）。四個突變都對，包括「三份一起換成
新網域 → 綠」：守的是一致，不是某個特定網域。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 273 筆逐輪紀錄都在
