...
**下一輪：第五十二圈第 8 輪 —— 視覺與版面**。

**第五十二圈開始了，問的是：這個東西的名字，跟它實際做的事一樣嗎？**
判準：把名字當成一句承諾（可及名稱、規則 id、規則自己印的訊息），
再對照它真正的行為，看**照著名字理解的人會不會判斷錯**。
理由：`skip-link` 這一條已經為了同一件事被修過兩次（第二十四圈、第四十七圈），
兩次都不是加一條新檢查，是**修一條說了謊的**。

**第 1 輪（無障礙）**：全站 899 個連結、95 個相異的可及名稱，對不上的只有一類 ——
同一個叫「English」的連結，在 12 個中文頁去的是這一頁的英文版，
在另外 19 個（＋404）去的是英文版**首頁**。去處是第十一圈刻意決定的，
而同一頁 `<head>` 的 `hreflang` **誠實地沒有宣告英文版** ——
給機器看的那一份說了實話，給人看的那一份沒有。
補了一段 `sr-only`（畫面上的字不動），它跟 `hreflang` 讀同一個
`availableLocales`，所以兩邊不可能分岔。

順帶修掉一條說了謊的規則：`unlabelled-cjk` 的樣式會把下一個元素的 `<`
一起吃掉，於是**任何緊接在前一段文字後面的元素都掃不到** ——
而這一輪新加的那個 `sr-only` 正好是那個形狀。

WCAG 2.5.3（可及名稱要包含畫面上的字）量了 127 個控制項，**0 個對不上** ——
不過那個 0 是判準改對兩次之後才出現的（箭頭算不算「標籤的字」決定了答案）。

**第 2 輪（效能）**：`requestParts()` 的第一句寫的是「這一頁**真正會發出的
請求數**」，而它數的三種全是**標記**宣告的。站上有一個不是 ——
`/search` 的內嵌腳本 `fetch('/search-index.json')`，帶著 `?q=` 進站時
一進來就抓。線上實測那個網址壓縮後共 16,602 B，而整支關卡唯一對讀者說的
那句話寫著「第一次到訪**最多**下載 14.1 KB」。預算不動（索引不阻塞算繪），
補的是那兩句話 —— 而且數字每次重算。

**第 3 輪（內容結構）**：`external` 的 frontmatter 寫著
「platform 必須對應 platforms.data.mjs 裡的 id」，而**沒有東西在守那個「必須」**
（同一份 schema 的 `related` 用的是 Astro 的 `reference()`，`sources.mjs`
那一半也有人驗）。把範本暫時打開、把 id 打錯一個字：建置離開碼 0、
`/elsewhere` 多一格假平臺、多產生兩頁、進 sitemap 與 feed，**七支檢查全綠**。
沒有當場加檢查（過去十輪沒有真事故），記進 TODO 並把範本那句「必須」
改成說得出後果的版本。同一次實測順手抓到兩件事：`external-missing`
的改法第一句指向一個不可能發生的原因（已改），以及 `SyndicationList`
的 `why` 少標 `lang` —— 而它的註解說「跟著頁面語言」，
同一個檔案上面卻寫著那條路刻意不依語言過濾（已修，同一份輸入一紅一綠）。

**第 4 輪（平臺 feed 實測）**：跑 `--patterns` 的那一刻，`✓ youtube` 與
`syndication.json` 裡 `youtube-foxpoetry: status error` **同時成立** ——
那一格打的是 Google 自家頻道的 probeHandle，不是這個站的來源。
`CLAUDE.md` 寫著這件事，而印出那格綠燈的腳本自己不說；補上了（判準算出來的，
不寫死平臺名）。順帶接完上一圈那次故障的下半場：feed 已復原（至少停了八小時），
`npm run sync` 把 status 翻回 `ok`，`/colophon` 從「1 個異常」變回「1 個正常」。
24 個平臺的樣板全打過，`confidence` 一個都不用動。

**第 5 輪（隱私與安全）**：規則叫 `csp-unsafe-inline`，而它只看 **11 條指令裡的
1 條**（`script-src`）。實測把 `style-src` 的雜湊鎖拿掉
（`styleDirective` 改成 `kind: 'element'`）：`audit:privacy`、`check:a11y`、
`check:content`、`check:perf` **四道全綠**，而且「CSP 雜湊數」那條預算
從 88% 掉到 29% —— 一個安全性的退步在螢幕上是一條變短的綠色長條。
改成掃每一條指令，唯一的例外是 `style-src-attr` 的 `'unsafe-inline'`
（刻意的，理由早就寫在 astro.config.mjs 上）。改完同一個實驗 exit 1。

**第 6 輪（文案與語氣）：沒發現問題。** 佔位符在兩種語言之間 98 組全部對得上；
`ui.ts` 的 90 組拿去比對產出，**12 組從來沒有人看過**，而那 12 組
一組一組都對應到一件已經記著的「這個功能還沒有真的跑過」（分頁沒畫過、
`ExternalLink` 沒人 import、手動登錄 0 筆、schema 欄位沒人用）——
**文案沒被看過跟功能沒被跑過是同一件事的兩面**。
唯一改的是關卡自己開口要的那一行：`EN_COVERAGE` 的基準線 147 → 148
（多的那一組就是這一圈第 1 輪加的）。

**第 7 輪（建置與 CI）**：`CLAUDE.md` 寫著排程的 `sync-feeds`
「一天跑兩次，而**最多 commit 一次**」—— 而那個判準的函式名字
（`sameAndAlreadyToday`）自己就說要**兩個條件同時成立**才不寫。
數版控裡每一筆 `chore(sync)`：**四個臺北日裡兩天是兩筆**。
兩個原因都遇到了 —— 內容真的變了（feed 壞掉那次），
以及 GitHub 的排程遲到五個多小時、跨到下一個臺北日。改掉那句話並附上表。
另外查了 `deploy.yml` 那一步「不需要 dist 的那些」今天是真的（`ci:sim` 每次都在驗），
而 `needs-dist-before-build` 抓不到當年那個形狀，所以 CLAUDE.md
那句「唯一會抓到它的是 `ci:sim`」**沒有過期**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 417 筆逐輪紀錄都在
