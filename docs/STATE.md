...
**下一輪：第五十一圈第 6 輪 —— 文案與語氣。**

**第五十一圈問的是：同一件事，現在有幾個地方各做一次？做得一樣嗎？**

第 5 輪（隱私與安全）先查了「外部連結要有 `rel`」那一對（`check-a11y` 的
`blank-rel` 是 error、`audit-privacy` 的 `target-blank-no-rel` 是 warn）——
**第十四圈就查過了，而且註解白紙黑字寫著「這一行寫下來，免得下一個人順手對齊」**。
這一格的成果是沒有動它。

真正值得量的是「**什麼算第三方請求**」，它有**三層**：名單（5 個主機、warn、
掃原始碼但跳過散文 `.md`）、`built-third-party-request`（反過來——不是
`bellafoxy.com` 就算，error、掃 `dist/`）、以及 CSP（瀏覽器執行時）。

拿同一個主機在三種位置各放一次：散文 `.md` **一聲都沒有**（`audit-privacy.mjs:1805`
刻意跳過）、原始碼檔 warn、建置產物 error。**三份不是同一件事做三次，
是同一件事在三個時間點各攔一次**——而 `third-party-cdn` 的 `whyWarn` 直接點名
另外兩層，所以「把主機加進名單就安全了」這個誤會有東西擋著。
