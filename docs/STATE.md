...
第四十圈第 1 輪（無障礙）問「站上真的跑過嗎？」——
31 條規則裡有 **3 條在站上一個主體都沒有**（`aria-ref`、`img-alt`、
`positive-tabindex`），而關卡自己每次都會說那件事。

做了一頁暫存的把那三種形狀擺出來，**三條都對** ——
訊息準確、`<img alt>`（無值）也沒有誤報（那是第八圈修過的）。
文件裡那份手抄的閒置名單也當場被抓到過期。

真正找到的一個錯是另一類：**跑過幾百次，而沒有人比對過它說的話。**
`check:a11y` 標題印「31 條規則」，四行之後印「上面那 **30** 條」——
寫死的，規則加到 31 時沒有人回來改。改成同一個來源，
並加一格比**兩者相等**（不是等於 31 —— 釘死數字會讓加規則的人要改兩個地方）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 315 筆逐輪紀錄都在
