...
**下一輪：第三十五圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）：同一份報告裡「191 個介面字串」與「108 組文案」
一直對不起來（108 組若都有兩種語言該是 216）。追下去**兩個都是對的** ——
語料是 `ui.ts` **加** `site.ts` 兩個檔案（我第一次只數了 `ui.ts`），
而 236 個字串值裡有 45 個（19%）被跳過（單字、帶 `{佔位符}` 的比不到），
剩 191；108 數的是「物件」不是字串，單位本來就不同。
兩行各補一句說明分母，跳過數是現算的。加 1 格測試 ——
**它第一次跑是紅的（0 個）而那是對的**：fixture 裡沒有可跳過的字串，
所以修的是語料不是判準。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 280 筆逐輪紀錄都在
