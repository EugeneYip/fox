...
**下一輪：第四十一圈第 6 輪 —— 文案與語氣。**

第四十一圈第 5 輪（隱私與安全）驗了兩課。

**「稽核腳本本身也不能有真值」（第二圈）—— 修乾淨了。** 拿稽核自己那 8 個
身分值去掃 9 個豁免檔案（只報數量不印值）：唯一命中的是 `identity.local.ts`
—— 那些值本來就住的地方，而且在 `.gitignore` 裡。

**「不比對的時候也要登記 0」—— 漏了三條。** `STRUCTURAL_IDS` 列了 20 條，
而 `saw()` 過的有 23 條。那三條（`cookie-promised-none`、
`csp-frame-host-unpromised`、`external-link-rel-broken-promise`）靠的是
所在區塊開頭的 `saw(id, 0)`，而那個區塊包在 `if (existsSync(dist))` 裡。
把 `dist/` 移走再跑：**規則數 31 變 28**，而「沒有東西可看」那份名單裡
一條都沒有它們，離開碼 0。

**而那正是 CI 上會發生的事** —— `verify:all` 把 `audit:privacy` 排在 `build`
**前面**，乾淨的 runner 跑到它時還沒有 `dist/`。那三條在 CI 上從來沒跑過，
而輸出連「沒跑」都沒說。補進清單，兩格測試，突變重現 31 vs 28。

**我自己第六次「判準被別的東西滿足」**：那兩格的第一版用 `build()` 做
「沒有 dist」的情境，而 `build()` 一律先鋪 `CLEAN`，裡面有兩份
`dist/*.html` —— 兩邊都有 dist，突變照樣全綠。六次裡三次是突變抓到的、
三次是數字不對勁，**兩種都不是我自己看出來的**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 327 筆逐輪紀錄都在
...
**第四十一圈（進行中）：這一課學過了，當時修乾淨了嗎？**
判準：**翻出 `REVIEW-LOG.md` 裡記過的一課，找出它今天在 repo 裡每一個
同形狀的地方 —— 有幾個當時沒修到？**
第四十圈撞到兩次，共同點很不舒服：**兩次都是有人認真學到了那一課、
認真寫下來了，然後只修了眼前那一處。**
「這個端點不只回 404，500 也算」修了 `verify-sources.mjs`，
而站主真正會看到的 `youtube-source.mjs` 與三份文件都是舊的；
「別借 `home.emptyBody`，在這一頁那句話是錯的」修了「各處」那一頁，
而 `ListPage.astro` 與 `archive.astro` 是同一個形狀。
`REVIEW-LOG.md` 現在有 322 筆、每一筆都在講一課 —— 如果多數的課
都只修了一處，那份紀錄就從資產變成帳單。所以下一圈不找新問題，
回頭把學過的課修乾淨，並且把「還有幾處」變成一個會自己說話的數字。
