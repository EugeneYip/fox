...
**下一輪：第三十一圈第 5 輪 —— 隱私與安全。**

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 246 筆逐輪紀錄都在
...
**第三十一圈（進行中）：這件事，是我們選的，還是它剛好長成這樣？**
第二十八圈問過「這件事是誰決定的」，那一圈假設**有人決定過**。
這一圈問更前面的一步：**有些事根本沒有被決定過** ——
`ui.ts` 的 `en` 是選填而 `site.ts` 是必填、`check.yml` 只在 PR 上觸發
而這個 repo 從不開 PR、圖片 `sizes` 用 40rem 而版面斷點是 34rem。
分辨的方法很具體：**說得出當初在哪兩個選項之間選了哪一個嗎？**
...
### 最近一圈（第三十圈：整個拿掉的話，誰會發現）

問法只有一種：**真的拿掉，重建，跑完全部關卡，看有沒有人喊。**

| 輪 | 拿掉什麼 | 誰發現了 |
|---|---|---|
| 1 無障礙 | 導覽列的 `aria-current`、`.sr-only` 的定義 | **沒有人**（八道全綠） |
| 2 效能 | `inlineStylesheets` 改成 `always` | **沒有預算超標**，而連讀 5 頁多 26% |
| 3 內容結構 | `ExternalLink.astro`（沒有人 import） | **沒有人** —— 而 9 處在手寫它要統一的事 |
| 4 平臺 feed | 那則「同步資料過期」的提醒 | **沒有人** —— 排程一活過來它就永遠不會響 |
| 5 隱私 | 16 條這次沒判斷過東西的規則 | **沒有人** —— 預設輸出一模一樣 |
| 6 文案 | `ui.ts` 裡任何一句 `en` | **沒有人** —— `pick()` 安靜退回中文 |
| 7 建置與 CI | `check.yml` | 沒有人（跑過 0 次）—— 但它被守得最緊 |
| 8 視覺 | 一個 `34rem` 改成 `32rem` | **沒有人**（八道全綠） |

**八輪問八次，七次的答案是「沒有人」。**

那一圈最有用的一句：**大部分「沒有人會發現」不是因為東西沒用，
是因為沒有人在看。** 以及：**鬧鐘的錶可能被它要監視的東西自己撥快**
（`generatedAt` 每跑一次就更新，所以「資料過期」的提醒永遠不會響）。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十圈走完了」。
