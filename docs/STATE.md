...
**下一輪：第四十二圈第 8 輪 —— 視覺與版面。**

第四十二圈第 7 輪（建置與 CI）驗的兩份清單，**都是自己記下來的**：
`TESTED_ELSEWHERE`（第 1 輪補 a11y 那份時就記著「這一支同樣沒有人守」）
與 `NEEDS_DIST_WHY`（上一圈第 7 輪加它的時候就寫著「這份表是手寫的」）。
今天量：兩份都乾淨。

`NEEDS_DIST_WHY` 有兩半，而**值那一半更安靜**：鍵打錯的話那一支就不在
「需要 dist」的清單裡（正是上一圈那個 bug 的形狀）；值打錯的話
`=== 'quiet'` 不成立，訊息**退回**「這一步在 CI 上一定會失敗」——
而那句話對 `audit:privacy` 是**假的**。**沒有任何徵兆，只是說了一件不對的事。**

補了三格守門，三個突變都驗過。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 337 筆逐輪紀錄都在
