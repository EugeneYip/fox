...
**下一輪：第三十七圈第 6 輪 —— 文案與語氣。**

第三十七圈第 5 輪（隱私與安全）的答案特別清楚：**要求這些規則的是 `/privacy`
那一頁**，而那一頁是寫給**讀者**的承諾書。把它的承諾逐條列出來對照，
八條裡**六條有人守，兩條沒有** —— 「不使用 cookie」與「影片框用
`youtube-nocookie.com`」。兩條當時都是真的，但沒有東西讓它們保持為真。
影片那一條特別值得守：`VideoFacade` 到今天**一次都沒有算繪過**，
真正擋著的是 CSP 的 `frame-src`，而之前沒有規則拿它跟承諾對。
補了兩條（`cookie-promised-none`、`csp-frame-host-unpromised`），
作法跟既有那條 rel 規則一樣：承諾印在頁面上，事實在隔壁的檔案裡。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 295 筆逐輪紀錄都在
