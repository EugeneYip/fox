...
**下一輪：第五十一圈第 8 輪 —— 視覺與版面。**

**第五十一圈問的是：同一件事，現在有幾個地方各做一次？做得一樣嗎？**

第 7 輪（建置與 CI）問「用 `&&` 串起來的那種 npm script 字串，有幾支在剖析」——
**兩支**：`run-steps.mjs` 的 `expand()`（`split('&&')` 再 `trim()`，遞迴）
與 `check-workflows.mjs` 的 gate 清單（`split(' && ')`，**要求兩側各一個空格**）。

**餵同一個合法輸入，答案不一樣**：`&&` 不留空格在 npm 裡合法（實測 `verify:all`
照樣離開碼 0），`run-steps` 兩種寫法都展成 2 步，而 `check-workflows` 把整條鏈
當成一個關卡的名字 —— `X [gate-missing-in-check] 部署路徑上會跑 npm run check&&npm run…`。
紅得很大聲，但點名的是一個不存在的關卡，讀的人會往錯的方向找。

改成 `split(/\s*&&\s*/)`。兩個方向都驗過：正常空格的輸出跟改之前**一字不差**，
少空格時假關卡名消失、跟 `run-steps` 一樣看到六道。

**為什麼不是兩支合成一支**：`run-steps` 要「展開成可逐步跑的名字」（遞迴），
`check-workflows` 要「`verify:all` 這一層有哪幾道」（**刻意只展一層** ——
第四十六圈量過那個決定的後果）。要的答案不同，所以是兩支；差別只在容忍度。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 409 筆逐輪紀錄都在
