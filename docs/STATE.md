...
**下一輪：第三十四圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed）：宣稱（`confidence`／`verifiedAt`）與實測那一組**早就接好了**，
而且接得比別處都細（宣稱 verified 卻實測失敗會指名道姓，沒打到的理由拆成三種
而且加得起來）。找到的是一個小地方：`gen-platform-docs.mjs --check` 失敗訊息裡
那句「那 23 個中文字串」是**寫死的**，而這支腳本 import 的就是 `PLATFORMS`，
走一遍就數得出來——而且那句話只有在檢查失敗時才會被看到。已改成自己數、
成功時也印，並拿去對 `CLAUDE.md` 那一份手抄的數字。三個突變都紅。

（這一輪最花時間的是確認「沒發現」：我第一次數得到 35，追下去才知道那是
掃原始碼把 `notes` 的多行串接算成十幾個片段，走資料結構才是 23。差一點就
把一個正確的數字改壞。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 270 筆逐輪紀錄都在
...
**第三十四圈（進行中）：這個答案，系統裡已經有了嗎？如果有，為什麼沒用上？**
第三十三圈八輪裡有五輪，答案早就在系統裡了 —— 印在同一張表往上兩行、
寫在一行註解裡、檔名印在輸出上卻沒讀過那個檔、被 pipe 吃掉、
在一段不擋的說明裡。不是「沒想到」，是**知道了而沒有接上**。
...
| 輪 | 那個人走到之後拿到什麼 |
|---|---|
| 1 無障礙 | 關卡把他送到 `A11Y.md`，而那份第一句寫「26 條規則」——他上一秒看到 28 |
| 2 效能 | 「讀者實際下載 10.5 KB」——那是 HTML 而已，真正是 14.1 |
| 3 內容結構 | 詩詞的 `title` 必填、**沒有讀者看得到**；指南說那是「這頁的標題」 |
| 4 平臺 feed | `npm run handle` 給的三段裡，一段貼下去會多一筆重複的來源 |
| 5 建置與 CI | `mutate` 印「已還原」而檔案還是髒的；含反斜線的 `--from` 永遠配不到 |
| 6 文案 | 六個空的英文頁都會指路，**只有搜尋不會**——那是他唯一動手的地方 |
| 7 隱私 | `ci:sim` 說「全部通過」，而身分規則一條都沒跑 |
| 8 視覺 | 為特定讀者寫的四樣都經得起量；洞在別處（見下） |

**八輪裡有五輪，答案早就在系統裡了** ——「首次造訪關鍵路徑 14.1 KB」印在
同一張表往上兩行、`check-content.mjs:169` 有一行註解寫著答案、
`check-handle.mjs` 把檔名印在輸出裡卻沒讀過那個檔、`audit:privacy` 印的 ⚠
被 `ci:sim` 的 pipe 吃掉、`check:contrast` 看到了 `--t-ys` 但那段不擋。
**不是「沒想到」，是知道了而沒有接上。**

第 8 輪那個是意外撞到的：我隨手跑 `mutate` 忘了還原，把 `--t-xs` 留成
`--t-ys`（17 個檔案在用），**`verify:all` 與 `test:tools` 兩套全綠** ——
`var()` 指不到東西時 CSS 不報錯，字級悄悄退回繼承值。現在會擋了。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十三圈走完了」。
