...
**下一輪：第三十四圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）：「可以寫哪些欄位」是 zod schema 說了算，而
`docs/CONTENT.md` 用範例又寫了一次。`field-undocumented` 早就在守這件事 ——
但**只守一個方向**（schema 有的，指南教過嗎）。反過來沒有人看：指南教一個
schema 已經沒有的欄位，zod 會把那個鍵**安靜丟掉**，建置不紅、畫面不變。
`declared` 與指南文字都已經在同一個區塊裡了，只是沒往回問一次。
已加 `guide-field-unknown`（主體 23，今天 0 個對不上），兩個突變都紅。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 269 筆逐輪紀錄都在
