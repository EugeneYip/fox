...
第三十七圈第 1 輪（無障礙）問「這一條規則是誰要求的」，答案不在數量上：
`check:a11y` 30 條規則、人會讀的文件提到 6 條，但 `docs/A11Y.md` 本來就不是
規則目錄。問題在它**怎麼**提規則 —— 它拿規則當證據（「焦點框本身是自動守住的：
`focus-outline-removed` 這條規則」），今天點名 5 個 id，
而那幾句是給**做人工走查的人**看的：他讀到就會跳過那一項。
實測把那條規則全部改名，`check:a11y` 離開碼 0、`check:doc-links` 0 ——
只有 `test:a11y-rules` 紅，而它紅是因為它自己的 fixture 也寫著那個名字。
新增 `doc-names-real-rule`：文件指到不存在的規則就擋，一個都沒點到也要說話。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 291 筆逐輪紀錄都在
...
**第三十七圈（進行中）：這一條規則，是誰要求的？寫在哪份文件裡？**
第三十六圈量出來的邊界，有好幾條的真正爭議不是「數字對不對」，而是
**這條規矩到底管到哪裡** —— 那 67 處程式碼註解算不算「網站上的中文」？
176 個 icon 連結要不要算進「單頁請求數」？feed 裡的外部網址是內容還是請求？
每一個都是**我的判斷**，不是哪份文件說的。所以往回追一步。
...
### 最近一圈（第三十六圈：這道檢查的邊界外面是什麼）

八輪**全部數得出一個數字**，沒有一輪是「大概還好」。

| 輪 | 邊界外面現在有幾個 |
|---|---|
| 1 無障礙 | `<svg>` **99**、`<details>`／`<summary>` **各 44**、`<time>` **82** —— 都沒有規則在看 |
| 2 效能 | `rel="icon"` 132 ＋ `manifest` 44 ＝ **176** 個請求沒算進「單頁請求數」 |
| 3 內容結構 | 反過來：邊界**裡面**多了 **9 支影片、1420 個字**，那是在 YouTube 上打的 |
| 4 平臺 feed | **9 筆 × 13 欄**，而那份 schema 從第一個 commit 就沒人執行過 |
| 5 隱私與安全 | 產出 61 個檔案只讀 **44** 個，**82 個內嵌 `<style>`** 完全看不到 |
| 6 文案與語氣 | 程式碼註解 **8,619 行**，是它掃的 2,571 行的 **3.4 倍** |
| 7 建置與 CI | `deploy.yml` 跑過 8 次、`sync-feeds.yml` 2 次、`check.yml` **0 次** |
| 8 視覺與排版 | 畫面上真的出現 10 種組合，**1 種**不在手寫的 `PAIRS` 表上 |

**兩輪找到「硬性限制完全沒有人在守」**：在讀者真的會下載的 CSS 裡加一行
Google Fonts 的 `@import`，七支檢查全部綠燈（第 5 輪）；資料少一個必填欄，
六道關卡全綠而產出多出 6 個沒有 href 的 `<a>`（第 4 輪）。
兩次的答案**早就在 repo 裡**，只是沒有人去執行它。

**「綠燈說了什麼」這件事這一圈補完了**：第 7 輪找到漏掉的第七支
（`check:workflows` 有 9 條規則卻只說「3 份 workflow」）。
現在七支都說得出判斷過幾個、哪幾條閒置、**以及看不到什麼**。

**一個新的形狀：假的 0 比不說更糟。** 第 5、6 輪的邊界說明第一版都印出
「跳過 0 個」「0 行含漢字」，而那兩個 0 都是走錯路徑，不是事實。
兩支現在都會說「這次沒有量到」並指出它找的是哪些路徑。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十六圈走完了」。
