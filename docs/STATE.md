...
**下一輪：第四十七圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed 實測）今天是 `200 Atom 9 筆`。24 個平臺裡 11 個重驗過，
**13 個沒有日期**（`manual`／`bridge`／`lookup-required`，沒有 `probeHandle`，
`verify -- --patterns` 從來不會碰到）—— 那 13 條宣稱從 2026-09-04 起沒被檢查過。
沒去驗：要有真實帳號，而規矩是不編造帳號。

**改問排程，找到一句抄進兩份文件的錯話**：CLAUDE.md 與 STATE.md 都寫
「排程每天自己 commit 兩次」。量了：**4 次排程、3 筆 commit**，
沒 commit 的那一次「觸發部署」是 skipped —— 那是第二十三圈刻意加的
「同一個臺北日、內容沒變就不寫檔」。**那句話從第二十三圈起就不對了。**

**順帶：那個間歇性的 404 在排程上真的發生過一次。** 09-05 04:08 那一筆
把 `status` 寫成 `error`、記下「RSS 失敗（HTTP 404（試了 7 次））」，
`lastSuccessAt` 停在 09-02（所以頁面誠實顯示 9月2日），11 小時後自己恢復。
降級的設計整條走完了，而且沒有人需要做任何事。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 374 筆逐輪紀錄都在
