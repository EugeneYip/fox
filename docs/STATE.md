...
第四十圈第 2 輪（效能）問「站上真的跑過嗎？」——
11 條預算的超標路徑，在測試裡 **11／11 都真的執行過**
（量法：暫時在 `check-perf` 的比較後面接一行記錄超標的 label，跑一次測試，量完還原）。

但其中一條**只在測試裡跑過**，而它說的話是錯的：
「最大單一檔案」超標且觸發 `_astro/*.webp` 時，改法叫人去改
`CoverImage.astro` 的 **`densities`** —— 那個 prop 早就不存在了
（第二十圈就改成 `widths` ＋ `sizes`，元件註解自己寫著）。

沒有人發現，是因為那句話要有內容圖才會印，而**站上 0 張內容圖**。
它唯一跑過的地方是測試，而測試斷言的正是 `densities` 那個字 ——
**把過期鎖住了**。改成不比字面，比「訊息點名的 prop 元件裡真的在用」，
兩個方向都突變驗過。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 316 筆逐輪紀錄都在
