...
**下一輪：第三十九圈第 5 輪 —— 隱私與安全。**

第三十九圈第 4 輪（平臺 feed 實測）把「11 個平臺的『N 筆』沒驗」那條**驗掉了**：
用一個獨立的算法（直接數 `<item>`／`<entry>` 標籤）對 `countItems()`，
**11／11 一模一樣**（連 `note` 的 0 也一樣，所以那個 feed 真的是空的）。
但那是一次性的，而它守的東西不是 —— 剖析器哪天開始漏項目，
數字只會**變小**，跟「今天少發幾篇」長得一樣。
所以把對照放進 `verify -- --patterns`，不一樣時印「9 筆（標籤數 10）」。
那一格的文字抽成純函式才測得到：那條路在真實執行時幾乎跑不到（11／11 都一樣）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 310 筆逐輪紀錄都在
