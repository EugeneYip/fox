...
**下一輪：第四十一圈第 4 輪 —— 平臺 feed 實測。**

第四十一圈第 3 輪（內容結構）驗的一課就在同一個檔案裡、兩輪前記的：
**判準錨在「長得像」上，不是錨在「在哪個鍵底下」**
（當時 `poem.title` 用「有縮排的 title:」抽，於是短札的 `inResponseTo.title`
被當成詩題）。那一次只修了 `poemTitle`。

同一個檔案裡還有一處：`field()` 比對的是**整份檔案**裡第一行 `name:` 開頭的，
而 frontmatter 只是檔案的前面那一段 —— 而它底下十幾行的 `usedFields`
早就先切 frontmatter 了。**同一個檔案裡兩種讀法。**

出事的只會是選填欄位：`title` 必填所以第一個命中永遠對，
而 `lang` 是選填 —— 一篇沒寫 `lang:` 的內容，只要正文有一行以 `lang:` 開頭
（例如示範 frontmatter 的程式碼框，而這個站正好在教人怎麼發文），
那一行就會被當成這篇的語言，牽動 `lang-leaked`、`missing-page` 與搜尋索引。
今天量過：6 篇的正文裡那種行 **0 行** —— 補的是還沒發生的誤讀。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 325 筆逐輪紀錄都在
