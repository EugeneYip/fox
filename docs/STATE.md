...
**下一輪：第三十八圈第 4 輪 —— 平臺 feed 實測。**

第三十八圈第 3 輪（內容結構）：內容管線裡「靠人記得」的那幾件前幾圈都接上了
（`related` 今天 0 筆、`translationKey` 早就在報、同步的新舊有兩個鬧鐘）。
沒人守的是另一種宣稱：這些文件除了連結，還會**叫人跑指令** ——
那跟連結是同一種宣稱（文件指著一個東西，那個東西要存在），而只有連結有人守。
量到 **94 處 `npm run`，一個都沒壞**，所以新加的 `doc-command-missing`
是預防性的。值得守的理由是**發現的時機最差**：改個 script 名字，
七份文件同時指到不存在的指令，而要等到有人真的去打它 ——
對站主來說那一刻多半是她想發文的時候。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 301 筆逐輪紀錄都在
