...
**下一輪：第四十三圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）問 `check:content` 第一行那個分母：「讀了產出裡 **50** 個
html／json／xml／txt」。又是一份手寫的副檔名清單。

第三十五圈追過這個數字，結論是「50 是對的，只是沒說是哪 50 個」。
這一輪把差的 11 個數出來：圖片 7、CSS 2、`CNAME` 1 —— **只有 10**。
第 11 個是 `site.webmanifest`（它是 JSON，只是副檔名不同）。

那個檔案是**手寫的**，裡面有站名、描述與主題色，四樣在 `site.ts` 都另有
一份。`#faf6ee` 寫在三個地方，`check:contrast` 在比前兩份，第三份誰都沒看。
實測把前兩份一起改成 `#faf6ef`：**六道關卡全綠**，manifest 還是舊的。

補了語料（50 → 51）與第 23 條規則 `manifest-drift`（比五項）。
突變掃描抓到我自己：三個比較裡只有顏色那一項真的被守著，另外兩項各補一格。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 341 筆逐輪紀錄都在
