...
第四十四圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪：「7 個沒人用的 token（→ 站主）」，輸出說「要刪還是要接上去，站主決定」
卻沒說哪一個是哪一種。逐個查值有沒有被寫死在別處，答案是 **6 比 1** ——
`--lh-loose: 2.1` 的值直接寫在 `PoemBlock.astro:277` 的 `line-height`
（直排時那就是每一句的寬度）。那不是「要不要刪」，是 **token 沒當成唯一來源**。
輸出改成說得出是哪一種，**一個 token 都沒動**。

那一格我自己錯了四次，全部是量測方法：子字串比對、行號用剝過註解的、
判準只認整行、測試的陷阱值選到真的會撞的。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 354 筆逐輪紀錄都在
