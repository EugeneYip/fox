...
**下一輪：第三十五圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed）：那個「9 筆」**四種算法都說 9**（verify 的解析器、
`syndication.json` 的三個欄位、我自己抓一次數 `<entry>`）；24 個平臺的
confidence 三分法與每一層子分割也都加得回總數，跟報告一字不差。

**這一輪第一次真的抓到東西**：「最舊的宣稱⋯N 天前」用 `toISOString()` 算今天，
那是 **UTC**，而這個專案把時區釘成 `Asia/Taipei`（`dates.ts` 兩處，有測試在守，
`sync-core.mjs` 也早就用對了）。當下 UTC 是 09-05、臺北已經 09-06 ——
報告說「0 天前」，照專案的時區是「1 天前」。已抽出 `projectDay()` 讓「今天」
只有一種算法，並加兩格測試（含一格掃 `scripts/` 不准有人再用 UTC 算今天）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 278 筆逐輪紀錄都在
...
**第三十五圈（進行中）：我怎麼知道這個數字是對的？有第二種算法算過嗎？**
第三十四圈連續四次證明「第二種算法」會給出不同的答案 —— 那四次都是我的算法錯，
但下一次可能是關卡的那一份。每一道關卡都在印數字（44 頁、28 條規則、58 個連結、
26 處動效、69 個 token⋯⋯），而多數只有一種算法算過。
...
| 輪 | 答案原本在哪 |
|---|---|
| 1 無障礙 | 閒置規則清單，**同一支腳本每跑一次就算一次**；文件抄了快照 |
| 2 效能 | 每條預算的真值，**同一個迴圈裡的 `b.value`**；抽取視窗只有 14 個字，配不到就靜靜跳過 |
| 3 內容結構 | schema 的欄位集合，**同一個 `if` 區塊上面幾行**；只比了一個方向 |
| 4 平臺 feed | `PLATFORMS`，**這支腳本 import 的就是它**；數字寫死在失敗訊息裡 |
| 5 隱私 | `externalLinkRel`，**那一頁第 14 行就 import 了**；承諾寫成不看開關的常數 |
| 6 文案 | `site.url`，**那一頁已經 import 了它的型別**；網域抄了兩次 |
| 7 建置與 CI | `site.ts` 與 `astro.config.mjs`，**隔壁那條規則已經讀進記憶體了** |
| 8 視覺 | 對比值 **完全正確** —— 唯一沒發現問題的一輪 |

**七個發現裡有七個，答案不只是「在系統裡」，而是已經被載入同一個作用域** ——
同一個迴圈、同一個 `if`、同一份 import、隔壁那條規則的讀檔。
沒有一次是「不知道去哪裡拿」。

共同原因只有一個：**修的人只看了眼前那一件。** 第 1 輪是上一圈修了那段話的
數字沒問「這一段還抄了什麼」；第 7 輪是寫 `locale-list-drift` 的人沒問
「這個檔案裡還有什麼是三份的」；第 2 輪是我自己上一圈改了一條措辭、
另外兩條一模一樣的沒改。

這一圈我自己也四次「以為找到 bug」其實是量法錯，**形狀跟在找的毛病一模一樣**：
重寫了一份系統裡已經有的計算，然後兩份分岔。能救回來的每一次，
都是因為去對了工具自己算的那一份。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十四圈走完了」。
