...
**下一輪：第四十一圈第 5 輪 —— 隱私與安全。**

第四十一圈第 4 輪（平臺 feed 實測）驗了兩課。

**「HTTP header 只能是 Latin-1」—— 修得比要求的還乾淨。** 值集中在
`lib/http.mjs`、測試用**真的 `new Headers()`**（檔案自己寫著正則掃原始碼
不可靠）、還有對照組。掃過 8 個 header 值，0 個非 Latin-1。
唯一沒守到的是**那份清單自己是手寫的** —— 加第四個 UA 忘了收進去，
上面照樣全綠。判準改用模組自己的 `UA_*` 匯出，突變驗過。

**「不只 404，500 也算」（兩輪前）—— 程式比每一份說明都寬。**
那一課寫在六個地方，五處寫「404、500」，而真正在判斷的那一份是
`404|500|502|503` —— **502 與 503 一個字都沒被寫進任何說明**。
清單改成只寫一次，訊息去讀它。

**我自己又一次「判準被別的東西滿足」**（這一圈第五次）：新加那一格原本用
整句訊息去 `includes(狀態碼)`，而錯誤文字裡本來就有那個碼 ——
突變套上去照樣全綠。改成只看「改法：」後面那一段才真的會紅。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 326 筆逐輪紀錄都在
