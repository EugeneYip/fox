...
第 2 輪（效能）：`astro.config.mjs` 與 `global.css` 都停在初始那一筆，
之後推了 **207 個 commit**。三個前提裡一個成立（sitemap 的 filter：41 筆／43 頁，
少的正是兩個搜尋頁）、兩個休眠（Shiki 兩套主題 —— 內容裡 0 個程式碼區塊；
`image.domains` —— 全站 0 個 `<img>`）。

**然後量到一件沒有人算過的事**：拿產出的 CSS 跟產出的 HTML 對一次，
**34 個 class 選擇器一頁都沒用到**。單頁的代價：`poems/index.html` 的內嵌 CSS
3,964 bytes 裡有 **1,487（37.5%）是 `.pager*`**，而那一頁沒有分頁 ——
全站 23 頁載著它、**0 頁畫出它**。原因是 `PAGE_SIZE = 30` 而站上只有 6 篇：
一個當時對、現在還沒到的前提。**沒有刪**（第 31 篇出現那天它就會活過來）。

**更正第 1 輪的一句話**：「分頁實際上有 22 頁在用」是錯的 ——
那個 22 是 grep 配到內嵌的 `<style>`，不是 markup。真的畫出分頁的是 0 頁。
同一天犯了兩次「grep 到的不是我以為的東西」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 372 筆逐輪紀錄都在
