...
**下一輪：第五十一圈第 4 輪 —— 平臺 feed 實測。**

**第五十一圈問的是：同一件事，現在有幾個地方各做一次？做得一樣嗎？**

第 3 輪（內容結構）先問「哪些內容算數」有幾個地方在判：

- **草稿過濾兩份**（`lib/content.ts`、`lib/syndication.ts`），逐字相同 ——
  而 `syndication.ts` 的註解**已經記著這件事**（第十四圈查過，結論還成立）。不是新發現。
- **排序三份**，而它們對「沒有日期」的立場不一樣：`byNewest` 假設有日期、
  `syndication.ts` 用 `?? 0` 容忍、`rss-all.xml.ts` **不容忍**。

先確認站上的列表跟 feed 會不會排出不同順序 —— **不會**（`rss.xml.ts` 走同一個
`getAllWriting()`）。把兩篇的日期改成同一天再建一次驗過，順序完全一樣。

**真正的分岔是 `rss-all.xml` 那一行 `.filter((item) => item.publishedAt)`** ——
它長得像編輯決定，旁邊一句說明都沒有。實測：把一筆的 `publishedAt` 設成 null
再拿掉那一行，建置當場死在 `Cannot read properties of null (reading 'getTime')`，
**噴在底下那個排序裡**。所以那一行擋的是自己的排序。
後果：沒有日期的同步項目 `/elsewhere` 看得到、`rss-all.xml` 看不到。
補了註解（是註解不是檢查）。

（又踩了一次 `grep -c`：那個 XML 是一行，`grep -c` 回 1 而實際 14 ——
第四十七圈第 1 輪同一個坑，只隔四圈又踩。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 405 筆逐輪紀錄都在
