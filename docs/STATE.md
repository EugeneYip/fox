...
**下一輪：第四十九圈第 1 輪 —— 無障礙**（新的一圈，問題還沒定）。

**第四十八圈走完了**，這一圈問的是「站上真的看得到的東西，我們量過幾成？」。
八輪全部從讀者那一端量：真的按 Tab、真的打線路、真的開瀏覽器讀 computed style。

**讀者拿到的東西這一圈一個 byte 都沒有變，而那是量出來的** —— 拿第 1 輪之前
那一版建一次，把 61 個檔案跟今天的 `bellafoxy.com` 逐 byte 比：61／61 一樣。
八輪動到的非文件檔案只有 6 個，全部是註解或去掉重複的第二份。

**站本身幾乎全對，錯的都是「講程式的那些話」**：`/privacy` 的八條承諾條條成立、
CSP 真的在 enforce、九個站外連結 9／9、瀏覽器算的對比跟 `check:contrast`
對到小數第二位、375px 五頁零溢出、直排門檻 767／769 一格不差。
找到的六個錯全是文字（`astro.config.mjs` 兩處、`sync-feeds.yml` 檔頭、
`audit-privacy.mjs` 的對照表、`check-perf.mjs` 一句推測、`rss-all.xml.ts` 的重複）。

**新規則 0 條，連續第二圈。** 四個真的找到的洞都寫得出判準，
而四個都說不出過去十輪裡會擋下哪一次 —— 所以都進了 [TODO.md](TODO.md)。

（第 8 輪也順手結掉一條掛很久的待辦：`aria-current` 有沒有看得見的對應 ——
有，而且有三種，導覽列是顏色＋2px 短線、頁尾是顏色＋底線、語言鈕是顏色＋字重。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 386 筆逐輪紀錄都在
