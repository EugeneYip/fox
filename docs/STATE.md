...
第 8 輪（視覺與排版）收掉這一圈：兩支探針有檔案，而「畫面上真的用到哪些
前景／背景組合」那一支**沒有** —— 留下來的只有一句描述
（「對每個有文字的元素取 color ＋ 第一個不透明背景」）。
照著它重寫的人要自己決定選取器與走法，而**那正是會出錯的地方**
（這一圈第 1 輪我就這樣得到 47 而基準是 45）。
寫成 `scripts/probe-contrast-pairs.js`，實測首頁 8 種組合、對照 `PAIRS` 一組都沒漏，
而且半透明與漸層會被標出來。這一圈**八輪全部走完**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 306 筆逐輪紀錄都在
