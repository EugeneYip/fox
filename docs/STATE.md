...
第 1 輪（無障礙）：三個「同一件事寫在兩邊」的候選裡，兩個已經接好了 ——
嚴重度雖然分寫在 `add()` 呼叫點與測試的 `SEVERITY` 表，但測試每一格都拿它去比，
還有兩格檢查漏登記與過期登記；頁數三支腳本各自數，都是 44。

第三個是真的：`docs/A11Y.md` 抄了一份「這次沒有東西可看的規則」的名單，
而那份名單 `check-a11y` **每跑一次就重算一次**。答案就在腳本手上，只是沒拿去對。
已就地比對（對不上就 exit 1），並加 `--doc=` 覆寫讓測試驗得到，4 格測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 267 筆逐輪紀錄都在
