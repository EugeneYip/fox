...
**下一輪：第四十三圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）問 `audit:privacy` 那句「掃了 **187** 個檔案
（另外 9 個在豁免名單上）、31 條規則」背後的三份清單。

豁免名單那一份**已經有人守**（`shadowedByAllowlist` 每一輪都說「9 條裡
只有 4 條真的擋住東西」）—— 我自己逐條拿掉量了一次，數字一字不差。
順手用**假的** needle 驗了「`identity-value` 穿得過豁免」那句話，仍然成立。

沒有人守的是 `SCAN_DIRS`。`.claude/launch.json` **被 git 追蹤**，
而 `.claude` 不在那份清單上；`unscanned-file-type` 也看不到它
（那條規則的迴圈本身就是 `for (const dir of SCAN_DIRS)`）。
實測把 Google Fonts 網址與看起來像 AWS 金鑰的字串放進 `.claude/`：
**必須修正 0、檔案沒被提到、連「掃了 187 個」都沒動。**

改了三件事：`.claude` 納入掃描（187 → 188）、加第 32 條規則
`unscanned-dir`（判準用 `git ls-files` —— repo 是公開的，
「在不在 repo 裡」才是該問的問題）、
`unscanned-file-type` 改成只看會進到公開 repo 的檔案。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 343 筆逐輪紀錄都在
...
第 5 輪是**少一個資料夾**。第 5 輪也第一次遇到「這一格已經有人守了」——
那份豁免名單每一輪都自己在說。站上這種句子還有很多
