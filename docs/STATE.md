...
判準：用 `git log` 找出每個面向裡最久沒動過的東西，回去問它當初的前提還成不成立。
理由：檢查工具每一圈都在動，而**網站本體幾乎沒有人碰** ——
最久沒動的地方，正是「當時的前提」與「現在的事實」最可能已經分家的地方。

第 1 輪（無障礙）量到：`Header`／`ThemeToggle`／`Base`／`PoemBlock`／`Pagination`
**206 個 commit 沒有人碰過**（都停在初始那一筆）。

三個前提裡兩個還成立（跳過連結的錨點 44／44 都在；分頁元件實際上有 22 頁在用）。
**第三個變了，而且那條規則說了謊**：`skip-link` 的判準是
`hasSkipClass || skipByShape`，而這個站自己是用 class 寫的 ——
所以它永遠走第一條，**錨點從來沒有被驗過**。
把 `<main id="main">` 的 id 拿掉，44 頁的跳過連結全部指空，關卡離開碼 0。
而規則自己的訊息寫著「指向後面真的存在的錨點」。

併成一個判準（不是加新檢查，是修一條說了謊的），並把兩種壞法分開。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 371 筆逐輪紀錄都在
