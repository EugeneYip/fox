...
第 1 輪（無障礙）拿第二種算法驗了 `--verbose` 的 6 個主體數：
heading 161、button 47、id 49、nav 91、input 2 **五個一模一樣**；
`link-name` 關卡說 899 而直接掃 HTML 是 901。追那 2 個：`strip()` 會先拿掉
`<script>`，而搜尋頁的前端會自己拼一個 `<a>` —— **899 才是對的**。
但那也表示搜尋結果的連結每一條 a11y 規則都沒看過，包括 `decorative-glyph-in-name`
（那條規則存在的理由正好是那個 ↗ 箭頭）。已把「掃不到幾個」印在報告上，加 2 格測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 275 筆逐輪紀錄都在
