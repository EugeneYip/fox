...
**下一輪：第三十六圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）問的是同一個邊界問題，答案在**產出那一端**：
每一條隱私規則看的都是 `src/`，而讀者下載的是 `dist/` —— 那一端只有一條規則，
它讀 61 個檔案裡的 **44 個**（只有 `.html`），而且只認標籤屬性。
那 44 個裡有 **82 個內嵌 `<style>`** —— 這個站的樣式就是內嵌的。
實測：在讀者真的會下載的那份 CSS 最前面加一行 Google Fonts 的 `@import`，
**七支檢查全部綠燈**。零第三方請求是這個專案的第一條硬性限制，
而它在產出那一端沒有人在守。現在讀 51／61，認得 `url()`、`@import`、
webmanifest 的圖示、以及 `//host`，並且說得出沒讀的 10 個是哪些。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 287 筆逐輪紀錄都在
