...
**下一輪：第三十八圈第 5 輪 —— 隱私與安全。**

第三十八圈第 4 輪（平臺 feed 實測）追出一條鏈子：來源**全部**失敗 →
`sync-feeds.mjs` 沿用快取、離開碼 0 → 資料沒變 → 不 commit →
**「觸發部署」那一步的 `if:` 不成立** → `deploy.yml` 不跑 → `check:content` 不跑
→ 「來源已經 N 天沒成功」那個鬧鐘**不會響**。
也就是說**來源死掉的時候，正好是那個鬧鐘走不到的時候** ——
排程那一次是綠的、而且安靜的，跟「今天沒有新影片」長得一模一樣。
判斷抽到 `lib/sync-health.mjs`（門檻只有一份），新增 `npm run sync:health`，
並在 `sync-feeds.yml` 加一步呼叫它 —— **刻意不加 `if:`**。
預設只說不擋，跟這個 repo 既有的立場一致。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 302 筆逐輪紀錄都在
