...
**下一輪：第四十九圈第 7 輪 —— 建置與 CI。**

**第四十九圈問的是：這道關卡如果安靜地失效了，我們會不會知道？**

第 6 輪（文案與語氣）：`check:copy` 跟 `audit:privacy` 是**一樣的架構**
（五條規則住在庫裡、五條住在腳本裡），所以先問上一輪同一件事。
**10／10 弄啞都會被抓到** —— 差別在測試怎麼跑：`test-copy-rules.mjs` 雖然也
import 庫，但它**同時會建假專案、用 `--root=` 真的去跑腳本**，所以接線有人守；
隱私那一支沒有。上一輪記的那條待辦，補法在這裡就有現成的樣子。

這一圈到目前為止：a11y 32／32、perf 11／11、content 24／24、
**privacy 27／33**、copy 10／10。

**往下一層問語料，三種縮水兩種會說話**：`ui.ts` 那條路讀不到會說
「一個介面字串都沒收到」；`dist/` 只剩一頁只有數字變小；
**`docs/` 整個不見最糟** —— `rule-not-documented` 的判準從「兩份文件都要有」
安靜地變成「一份就好」（那條規則存在的理由從此擋不住），
而反例那一格反過來指控文件缺反例，其實是檔案不見了。記進 [TODO.md](TODO.md)。

（「`ui.ts` 讀不到會怎樣」那一格我差點寫成「沒有人說話」——
去看對照組才發現那句話在對照組裡本來就有：假 root 裡那個動態 import
根本沒成功過。那一格什麼都證明不了，卻反過來證明了那條路是會說話的。）
