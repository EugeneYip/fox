...
**下一輪：第三十五圈第 8 輪 —— 視覺與排版。**

第 7 輪（建置與 CI）驗了 `ci:sim` 的六個數字：deploy.yml 10 步、
其中 5 步是 GitHub action、版控 191 個檔案、workflow 3 份 —— **五個一模一樣**。
第六個加不起來：`68 + 20 + 1 = 89`，而那一行寫「合計 90 秒」。
不是量錯，是**兩種算法**（每步先四捨五入再印，合計先加原始毫秒再四捨五入）。
差一秒不影響判斷，但讀的人會停下來重算。已改成合計與百分比都用螢幕上的數字算，
並加一格**直接驗加法**的測試 —— 那一格改之前根本沒辦法成立。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 281 筆逐輪紀錄都在
