...
**下一輪：第三十八圈第 7 輪 —— 建置與 CI。**

第三十八圈第 6 輪（文案與語氣）：這一支有一項**自己就寫著答案** ——
英文覆蓋率印「108 組全部都有 en（100%）」，然後說「這一項不擋⋯⋯
它從 100% 掉下來的時候，**要有人看得見**」。要有人看得見，也就是靠人。
而掉下來時印的「99%」**單看是一個現況**，要看出那是掉下來的，
得知道上一次是 100%，而那件事沒有寫在任何地方。
現在記了一個基準（`EN_COVERAGE`）：掉下來那一段會說「**這是回退**」
並附上上一次的數字。離開碼不改 —— 真要少一句英文就把那一行一起改，
**那一改就是「我知道我在改什麼」**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 304 筆逐輪紀錄都在
