...
**下一輪：第五十一圈第 7 輪 —— 建置與 CI。**

**第五十一圈問的是：同一件事，現在有幾個地方各做一次？做得一樣嗎？**

第 6 輪（文案與語氣）挑的是每一條文案規則的第一步：**這個字算不算漢字**。
那個判斷在檢查工具這一側寫了 **14 處、2 種寫法**（`[一-鿿]` 與 `[\u4e00-\u9fff]`）
—— 量過是**同一個範圍**（`一`＝U+4E00、`鿿`＝U+9FFF），五個字各餵一次答案完全一致，
沒有分岔。站的程式碼（`src/`）裡一份都沒有。

**但那個範圍本身漏掉擴充 A（U+3400 起）與擴充 B（U+20000 起）** ——
而那正是古典文本最常出現罕用字的地方。拿真的規則餵三句一模一樣的違規：
常用字命中 `halfwidth-punct`，**擴充 A 與擴充 B 一條都沒有**。
同一個錯、同一個半形逗號，換一個罕用字就沒有人管了。

**今天沒有踩到**：`src/content/`、`ui.ts`、`site.ts` 與 `dist/` 全掃一遍，
30,215 個漢字類字元裡落在範圍外的是 **0 次**。空著的洞。

沒有改任何東西 —— 要補會同時動到 14 處，而規則 10 說不出過去十輪會擋下哪一次。
記進 [TODO.md](TODO.md)，連同「那 14 處是同一個範圍的 14 份拷貝」——
真的要補的時候，這是一行還是十四行的差別。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 408 筆逐輪紀錄都在
