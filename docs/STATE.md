...
第 1 輪（無障礙）量到：`check-a11y.mjs` 綠燈時會把人送到 `docs/A11Y.md`，
而那份文件開頭寫「26 條規則」，關卡印的是 28 —— 第三十圈加了兩條，
文件沒跟上。已改正，並在 `test-a11y-rules.mjs` 加了兩格守住它。
另有一個「寫下來就會把自己弄錯」的數字（`VoiceOver` 的出現次數），
改成留指令、不留答案。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 259 筆逐輪紀錄都在
