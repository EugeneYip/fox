...
第 2 輪（效能）：`check:perf` 有一道專門在守「說明裡的數字過期」的檢查，
而它自己漏了一條 —— 11 條預算裡有 5 條的說明寫了現值，它的樣式只配得到 4 條
（「最大單一檔案」那句夾著一個很長的檔名，超出 14 個字的視窗），
而配不到時是安靜 `continue`。真正的問題是**從輸出上看不出來它比了幾條** ——
全部對得上時那一段一句話都不說。已放寬視窗（逐條驗過只多那一條），
並把涵蓋範圍印在綠燈上，加 2 格測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 268 筆逐輪紀錄都在
