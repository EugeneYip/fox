...
**下一輪：第三十七圈第 5 輪 —— 隱私與安全。**

第三十七圈第 4 輪（平臺 feed 實測）先照規矩打了一次網路：**11 個平臺全部 200**，
沒發現問題。問題在那份給人讀的表：資料裡每個 `verified` 都帶著 `verifiedAt`，
而 `docs/PLATFORMS.md` 的「可信度」欄**一個日期都沒有** ——
讀者看到「已實測」，分不出那是昨天還是半年前。
而那個差別是真的：`verifiedAt` 是**人手寫的**，真正在驗的那支腳本不會寫回去。
現在表上印日期、圖例說明那是人寫的，`--check` 也守住「別把日期弄不見」。
然後照那份文件自己說的程序做了一次：這一輪 11 個全過，
把日期更新成 2026-09-06 —— 最舊的宣稱從 1 天前變成 0 天前。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 294 筆逐輪紀錄都在
