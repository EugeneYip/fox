...
**下一輪：第三十三圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）量到：詩詞的 `title` **必填、而且沒有讀者看得到** ——
六條會顯示詩名的路徑（兩份 RSS、搜尋索引、`EntryCard`、`/archive`、
詩頁的 `<h1>` 與 `<title>`）全部挑 `poem.title`。`npm run write` 剛好把兩個
欄位填成同一個答案，所以差異永遠不出現；唯一手寫的那篇寫了「琵琶行（節錄）」，
「節錄」兩個字站上一次都沒出現。`docs/CONTENT.md` 對這個欄位的註解
（「這頁的標題」）也是錯的，已改。`check:content` 現在會印出不一致（只說不擋）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 261 筆逐輪紀錄都在
