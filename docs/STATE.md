...
**下一輪：第四十四圈第 8 輪 —— 視覺與版面。**

第 7 輪（建置與 CI）挑了「`check.yml` 永遠不會自己觸發（→ 站主）」。
那句話讀起來像在等他決定要不要改用 PR 流程 —— 而在那個決定之前，
有一件事誰都可以做，也沒有人做過：**那份 workflow 從來沒跑過，
所以沒有人知道它會不會過。**

它有 `workflow_dispatch`。觸發了：在那之前 `gh run list` 回的是 `[]`
（整個生命裡 0 次），這一次**12 個步驟全綠、88 秒**，
log 裡也讀得到「身分規則：8 個值，來自 PRIVACY_NEEDLES」。

順便比了一次：它跑的跟 deploy 是**同一套**（`verify:all` 的六個成員
加上兩套測試），連 secret 都一樣帶。所以「開 PR」買到的是
**部署前先看到結果**，不是多一層檢查。

**而那個「0 次」是寫死的，這一輪把它變成 1。** 同一段註解上面兩行才剛說過
「那種數字寫下來的那一刻就開始爛」—— 那個教訓沒有套用到 0 身上，
因為 0 看起來像常數。改成寫性質：它不會自己跑，要跑得手動觸發。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 353 筆逐輪紀錄都在
