...
第 2 輪（效能）量到：`check:perf` 整支只有一行是對著讀者說的
（「讀者實際下載 10.5 KB」），而那是 HTML 而已 —— 讀者第一次到訪還要抓
一支阻塞渲染的樣式表，真正是 14.1 KB，少講了 34%。答案本來就印在
同一張表上（「首次造訪關鍵路徑」那條預算），只是那一行沒有用它。
已改成兩句話分別說「第一次到訪」與「第二頁起」，並加了兩個斷言守住數字。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 260 筆逐輪紀錄都在
