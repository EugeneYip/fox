...
**下一輪：第五十一圈第 5 輪 —— 隱私與安全。**

**第五十一圈問的是：同一件事，現在有幾個地方各做一次？做得一樣嗎？**

第 4 輪（平臺 feed 實測）跑第一個指令就發現 **YouTube 的 feed 今天真的壞著**
（`✗ 404 HTML（不是 feed）`）。照 CLAUDE.md 的規矩連打三次：**404、500、404**
—— 跟文件裡記的那個樣子一字不差。再照它給的判準看頻道頁：`@FoxPoetry` 回 200、
canonical 指到 `sources.mjs` 裡那個 channelId。**頻道好好的，壞的是 feed 那一端。**

**而排程比我早一步，整條鏈已經自己跑完了。** 上一圈第 4 輪用合成資料驗的每一節，
這一次全部是真的：`status` 翻成 error → 寫檔（`3f31244`，04:15）→ commit →
部署 → **站上 `/colophon` 現在就寫著「0 個正常，1 個異常（youtube-foxpoetry）」**。
那一格上一輪是我把資料改壞才畫得出來的，今天它自己亮了。
`sync:health` 這時候說「全部都在 3 天內成功過 ✓」也是對的 ——
上次成功是十一小時前，三天的容忍正是為了這種一陣一陣的故障。

趁著它真的壞著跑了一次 `sync:dry`：**重試真的跑了 7 次、快取真的頂上、離開碼真的是 0**，
而它印的那段話就是那個判準本身。

回到這一圈的問題：判準有 **2 份**（`CLAUDE.md` 與 `lib/youtube-source.mjs` 的執行時訊息），
今天一字不差，而且那是**該有兩份的**（一份給規劃的人讀、一份在失敗當下印給撞到的人看）；
feed 剖析**不是重複** —— `parse-youtube-feed.mjs` 從 `parse-feed.mjs` import 了 8 個東西。

沒有改任何東西：feed 的故障不是這個 repo 的 bug，而整條降級路徑今天全部照著設計走完了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 406 筆逐輪紀錄都在
