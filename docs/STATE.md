...
**下一輪：第三十六圈第 8 輪 —— 視覺與排版。**

第 7 輪（建置與 CI）找到七支關卡裡漏掉的那一支：`check:workflows` 的綠燈
只有「3 份 workflow，沒有發現問題」—— 而它其實有 9 條規則、有 `saw()`、
有 `--verbose`，預設輸出一個字都沒提。它偏偏最需要講，因為它守的三份檔案
**沒有第二個人在看**（`ci:sim` 只模擬 `deploy.yml`）。
順帶用 `gh run list` 對了一次現實，更正兩處過期的宣稱：
`deploy.yml` 跑過 **8 次**、`sync-feeds.yml` **2 次**、`check.yml` **0 次**
—— 最後那份只在 `pull_request` 觸發，而這個專案是直接推 main，
所以它不是「還沒輪到」，是這個工作方式下永遠不會自己觸發。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 289 筆逐輪紀錄都在
