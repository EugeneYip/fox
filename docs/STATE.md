...
**下一輪：第五十圈第 4 輪 —— 平臺 feed 實測。**

**第五十圈問的是：照著文件做，做得完嗎？**

第 3 輪（內容結構）照著走的是 `npm run write` —— CLAUDE.md 說那是站主要的
「她自己就能發文」那條路，`docs/CONTENT.md` 第一節就叫「最快的開始方式」。

**照著做第二題就掛了**：問完第一題、印出第二題，然後 exit 13，
畫面上是 Node 的 `Detected unsettled top-level await` ——
看不出真正的原因是「stdin 沒有下一行了」。不經過 npm 直接跑，一模一樣。

那一節寫的是**問答式**的路，而問答需要一個真的終端機 ——
這一半跟第 1 輪的 VoiceOver 同一種：那條路需要一個人。
**但差別是：這個專案有另一條路，而且它是通的**（三個旗標給齊就不問問題，
產出的 frontmatter 正確、`check:content` 綠的）。

**而那條路一個字都沒有寫在文件裡** —— `--collection=` 只出現在
`docs/REVIEW-LOG.md` 的三處輪次紀錄裡，`CONTENT.md`／`CLAUDE.md`／`AGENTS.md`／
`README.md` 全都沒有。這個 repo 的第一句話是「隨時可能換人（或換 AI）接手」，
而接手的代理人照著文件走會撞上那個 exit 13、然後沒有任何線索。
補進 `docs/CONTENT.md` 了，連同那個離開碼長什麼樣。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 397 筆逐輪紀錄都在
