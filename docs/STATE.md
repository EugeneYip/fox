...
**一、「GitHub Pages 會不會壓 `.atom`／`.rss` 沒有人量過」** —— 站已上線，
所以那個前提量得到了。把每一種出貨的文字型別都打一次：
`.xml`／`.txt`／`.json`／`.webmanifest`／`.svg`／`.css` **全部 gzip**，
**只有 `CNAME` 沒壓** —— 它沒有副檔名，GitHub Pages 送
`application/octet-stream`，而它不壓那個型別。

而 `CNAME` 正好是第四十三圈第 2 輪從「量圖片的尺」搬到 gzip 那把的那一個。
**位元組判準答的是「這個檔案是不是文字」，伺服器問的是「副檔名對到哪個型別」。**
影響是零（14 bytes），所以不改判準，把量到的寫進註解。

**二、「`--w-prose`／`--w-content` 也是抄進 `sizes` 的」** —— 還成立。
`CoverImage.astro` 的 `sizes` 不能用 `var()`，所以那裡有**兩份**手抄值；
`--gutter` 那一份第三十九圈就有守衛，**同一個字串裡的第二份沒有**。
補了一格（`column` 改成 `44rem` → 離開碼 1），
並順手補上那兩格守衛自己從第三十九圈起就缺的測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 356 筆逐輪紀錄都在
