...
判準不是「規則抓不抓得到 bug」（那是突變掃描，每一圈都在做），
而是把規則自己弄成「不會失敗」，再看有沒有任何東西說話。
理由：新規則連續兩圈 0 條 —— 守衛的數量飽和之後，
風險就從「少一條規則」轉成「有一條規則安靜地不再守了」。

第 1 輪（無障礙）把 `check-a11y` 的 32 條規則**一條一條弄啞**
（在共用漏斗 `add()` 裡加一行，靠環境變數指定要悶哪一條）：
**32／32 都會讓測試變紅**。其中 `doc-names-real-rule` 一開始看起來是漏網的，
去看實作才發現它不走 `add()`（自己 `console.log` ＋ 一個 `docDrift` 旗標）——
打它真正那條路，測試照樣紅。

**唯一的洞是頁數。** `--dir=` 指到空目錄會 exit 1 並說「什麼都沒檢查」，
但 `dist/` 只剩 1 頁（其餘 43 頁刪掉）時它說「1 頁、32 條規則、沒有發現問題」
並 exit 0。同一份產出拿其他四支跑：`check:links` 與 `check:content` 都紅 ——
而那兩支**都住在 `test:built`**，`verify:all` 的六道一支都不在。
所以「產出少了 43 頁」只跑 `verify:all` 不會知道。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 387 筆逐輪紀錄都在
