...
**下一輪：第三十圈第 4 輪 —— 平臺 feed 實測。**

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 237 筆逐輪紀錄都在
...
**第三十圈（進行中）：這件事，如果現在整個拿掉，會有誰發現？**
這個 repo 只長不縮：`REVIEW-LOG.md` 三萬九千行、`check-contrast.mjs` 九百七十行、
5 個沒人 `var()` 的 token、4 條什麼都沒擋的隱私豁免、
一份開始偏長的 `CONTENT.md`。前九圈問的全是「這個東西夠不夠好」，
**沒有一圈問過「這個東西還需不需要在」**。
不是為了刪而刪 —— 是每一樣東西都要說得出**拿掉之後誰會踩到**。
...
### 最近一圈（第二十九圈：新手與老手看到的是同一份東西嗎）

| 輪 | 看的那份輸出 | 結果 |
|---|---|---|
| 1 無障礙 | 綠燈只說「44 頁」 | 沒說幾條規則；**六支關卡的 `--verbose` 沒人看得見** |
| 2 效能 | 11 個一模一樣的綠勾 | 加「最接近上限的是哪一條」 |
| 3 內容結構 | `check:links` 與 `check:content` | 前者本來就對（誠實寫沒發現），後者補規則數 |
| 4 平臺 feed | 「會 404」那句提醒 | 邏輯早就對，**措辭只講 404 而實測拿到 500** |
| 5 隱私 | 「必須修正 0」 | 0 個問題 —— 在什麼裡面的 0，沒說 |
| 6 文案 | 六支關卡的最後一支 | 說了掃多少東西，沒說用幾條規則 |
| 7 建置與 CI | 三個光禿禿的勾，跑十分鐘 | 加每一步的秒數與佔比 |
| 8 視覺 | `check:contrast` 的七段 | 六段報得出數字，**fallback 的兩個勾數不出東西** |

那一圈最有用的一句：**「綠燈」不是一件事，是兩件事迭在一起 ——
判準沒響，跟判準有東西可判。**
以及：**跨輪的待辦會過期，而過期的待辦看起來跟真的一樣**
（第 8 輪要修的那條，兩半都不成立，實際上是去改一個沒壞的東西）。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第二十九圈走完了」。
