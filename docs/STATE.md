...
找的不是重複的**事實**（那是第四十七、五十圈做的），是重複的**做法** ——
然後拿真的例子餵給它們，看答案一不一樣。理由：第四十九圈量到六支關卡對
「語料縮水」的反應不一致，那不是文件的事，是幾份各自為政的程式。

第 1 輪用行為當判準（「呼叫 `readdir` 的 async generator」）數了一次：
**走一棵目錄，這個 repo 裡有 13 份程式在做，11 種寫法**，
其中 `check-content`／`check-copy`／`check-links` 那三份**逐字一模一樣**。
而**其中一份已經住在 `lib/` 了**（`lib/dist-diff.mjs`）——**只差沒有 export**。

**今天行為一致**：加一個 `.well-known/index.html` 餵給三支，都看得到 45 頁。
兩處刻意的差別也說得出理由（`check-doc-links` 跳過 dot 目錄；
`audit-privacy` 反過來刻意收根目錄的 dotfile —— `.npmrc` 那次事故就是這樣抓到的）。

沒有改任何東西：合成一份要動 11 個檔案，而今天沒有一個 bug 是它造成的。

（這一輪我把同一種錯犯了兩次：先用「名字裡有 walk」找、再用「有沒有
`startsWith('.')`」判斷會不會跳過 dot —— 兩次都是拿好寫的近似當判準，
然後把它的答案當成問題的答案。第二次去讀那一段才發現它是相反的。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 403 筆逐輪紀錄都在
