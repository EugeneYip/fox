...
**第二十八圈（進行中）：這件事，是誰決定的？那個人還在嗎？**
不是「為什麼這樣做」（`ARCHITECTURE.md` 寫了很多），是**那個決定的有效期限**
—— 它當初是為了誰、為了什麼限制，那個人與那個限制現在還在不在。
第二十七圈撞到兩次（磁碟緊、站沒上線），兩次都是「理由不成立了而沒有人回頭」。
...
### 最近一圈（第二十七圈：換一個人來做，做得到嗎）

網站在這一圈的中途上線了，所以有一半的輪次量到的是**真的東西**。

| 輪 | 換誰來做 | 做得到嗎 |
|---|---|---|
| 1 無障礙 | 重現「最小餘裕 17.9px」 | 做得到，但要試三種解讀 —— 結論留著，量法沒留 |
| 2 效能 | 重量伺服器的壓縮率 | 做不到，而舊結論方向是反的 |
| 3 內容結構 | **她發一篇文** | 走到跑完關卡就沒有下一步了，而文件說那叫「發佈」 |
| 4 平臺 feed | 演練 `sync-feeds.yml` | 做不到 —— 兩個跨檔案的相依沒人看，壞了不會紅 |
| 5 隱私 | 設 `PRIVACY_NEEDLES` | **真人做錯了**，守門接住，但那要花掉一整輪 CI |
| 6 文案 | **她照著約定寫** | 做不到 —— 五條規則全部只寫在給 AI 看的那份文件裡 |
| 7 建置與 CI | clone、裝、跑 | 做得到（29 秒／235 MB），而代打的理由早就不成立 |
| 8 視覺 | 確認直排還在 | 做得到，但那條規則的判準有一個沒人問過的補集 |

那一圈留下：兩支貼上去就能跑的探針、一份新文件（`docs/A11Y.md`）、
`CONTENT.md` 的兩節新章、四條新規則、一個新檢查（`check:doc-links`）、
一個新模式（`ci:sim --real-install`），以及三條被收緊的判準。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第二十七圈走完了」。
