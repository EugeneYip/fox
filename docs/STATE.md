...
第四十圈第 4 輪（平臺 feed 實測）問「站上真的跑過嗎？」——
24 個平臺、四種 `feedKind`（`rss` 14、`bridge` 6、`manual` 3、`hybrid` 1），
而 `sources.mjs` 只有 YouTube 一筆，`hybrid` 的 API 那一半又要金鑰 ——
**真的在站上跑過的是四條路裡的半條。**

追下去撞到一件更要緊的事。今天 `sync:dry` 是紅的（她的 feed 回 404），
照 `CLAUDE.md` 說的 `verify -- --patterns` 重驗卻是綠的（Google 的頻道 200）——
那一課寫的判準是「連 Google 自家頻道都 404 才是端點問題」，
**照它推下去會得到「那就是她的頻道沒了」，而那是錯的**：

| 量什麼 | 結果 |
|---|---|
| `@FoxPoetry` 頁面 | 200，canonical 的 channelId 跟 `sources.mjs` 一模一樣 |
| 頻道上的公開影片 | 9 支（快取裡正好 9 筆） |
| 同一個網址連打三次 | **404、500、404** |
| 十分鐘後再跑 `sync:dry` | **✓ 抓到 9 筆** |

`verify-sources.mjs` 2026-09-05 就寫下「不只 404，**500 也算**」了，
而同一件事在另外四個地方還是舊版本 —— 其中一個是**站主真正會看到的那句話**，
它的判斷還寫著 `/404/.test(why)`，**剛好撞上 500 就會拿到最容易被讀成
「帳號沒了」的那一句**。判斷、訊息、四份文件都補齊了，突變驗過。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 318 筆逐輪紀錄都在
