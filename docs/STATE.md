...
第四十二圈第 1 輪（無障礙）把這一塊的五份清單逐份問過：
`RULE_IDS`（它就是來源）、`SEVERITY`（**兩個方向都有守**，連「表裡有已刪的
規則」都會擋）、`A11Y.md` 說的規則數、`A11Y.md` 的閒置名單 —— **四份都乾淨**。

只有 `TESTED_ELSEWHERE`（「這條做不出假頁面，在別處驗」的豁免清單）沒有人守。
實測兩種寫錯法：**打錯鍵**會被間接抓到（那條規則因此失去豁免，另一格響），
但上面那一行照樣印著不存在的名字；**多加一條**則**完全沒有人說話**，
離開碼 0，而輸出很有自信地說有兩條在別處驗過。

同一個守法這個 repo 已經有兩份（`check:copy`、`check:workflows`），
補上之後兩種寫錯法都會紅。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 331 筆逐輪紀錄都在
