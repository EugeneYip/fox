...
第 2 輪（效能）問了 `check:perf` 的分母。摘要那幾句都是算出來的
（「11 條預算裡 6 條說得出上限」「共 8 種：0 個」），沒人守的是
**決定一個檔案該用哪一把尺量的那份清單** —— `TEXT_EXT` 九個副檔名。

實測一份 104 KB（壓完約 5 KB）的重複性文字，叫 `feed.rss`：
**`X 最大單一檔案　173%`、離開碼 1**，改法還叫人「圖片改 WebP／AVIF」，
而同一次輸出說「最大的文字資源 —— dist 裡沒有純文字資源可量」。
`.xml`／`.txt` 在清單裡，所以第十六圈留下的誤報探針一直是綠的。

改成**問位元組**（沒有 NUL 且整份解得開 UTF-8 就是純文字），
這裡沒有清單可以漏。站上 `dist/CNAME` 因此從「量圖片的尺」換到 gzip 那把。

順帶：`TEXT_EXT` 這條待辦第十六圈就記了，抄到第二十一圈之後
**從清單上消失** —— 沒有一輪寫過它做完了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 340 筆逐輪紀錄都在
...
前兩輪的答案是：一支關卡可以有好幾個分母，而**推導出來的那幾個通常沒事，
出事的是決定語料的那一份** —— 第 1 輪是那句說明涵蓋範圍的話只認連結，
第 2 輪是決定「用哪一把尺量」的副檔名清單。站上這種句子還有很多
