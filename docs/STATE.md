...
**下一輪：第四十五圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）挑了五條待辦回去量，**三條還成立、兩條半錯**。

**還成立的三條都補上了守衛。** `manifest-drift` 原本只比五項**文字**，
現在多比三樣**指路的** —— 每個 `icons[].src` 要在 `dist/` 裡真的有、
`start_url` 要對得到一頁、`lang` 要等於那一頁的 `<html lang>`。
（順帶答了「`zh-Hant-TW` 還是 `zh-TW` 誰是準的」：出貨的那一頁寫的是
`zh-Hant-TW`，manifest 本來就對，是那條待辦拿錯了對照組。）

第三條是欄位抽取。`check:content` 靠兩條**我寫的**正則從 `content.config.ts`
抽欄位名，而兩條規則整個站在那個答案上。找到第二把尺：`astro sync` 產生的
`.astro/collections/*.schema.json` 是 **Astro 自己**從 zod 推出來的。
實測 Astro 說 26 個、正則抽到 33 個（多的是巢狀欄位）、**一個都沒漏**。

**兩條半錯的才是重點。** 「`CONTENT.md` 的指令沒有人在驗」—— 4 個
`npm run` 早就有人驗，沒人驗的是 5 個 `git`，而壞掉的那個正好在後面那一半；
但補「子指令名字存在」的規則**當初一格都擋不下來**（壞的是順序不是名字）。
「微網誌沒有標題、畫面沒處理」—— 沒處理是真的，但實測空標題會讓
`check:a11y` 離開碼 1，六道關卡擋得住，只是訊息講的是標題結構不是真正的原因。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 357 筆逐輪紀錄都在
