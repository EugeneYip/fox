...
第四十二圈第 2 輪（效能）：`check:perf` 的清單多半有人守（11 條預算都要
說得出 `basis`、`ICONISH` 是算出來的）。真正沒人守的是**一段註解** ——
`requestParts()` 上面那段把邊界列得很完整，但它有一半在重算、一半是快照：

| 哪一半 | 誰維護 |
|---|---|
| 「沒算的」（圖示、manifest） | `uncountedLinks`，**每次跑都重算** |
| 「一個都沒有的」（八種） | **寫死在註解裡，第三十六圈之後沒有人再數過** |

今天重量：44 頁，八種**確實還是 0**。而那正是問題 ——
**一個永遠對的句子跟一個沒有人在看的句子，讀起來一模一樣。**

改成跟「圖示」那一半同一個做法：八種每次重算，全是 0 也說出口。
做一頁暫存的（`<video>` ＋ `<link rel="preload">`）驗過，它會說
「**這條預算漏數了**：preload 1 個、media 1 個」。兩格測試跑在假 dist 上。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 332 筆逐輪紀錄都在
