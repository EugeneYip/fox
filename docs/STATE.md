...
**下一輪：第四十九圈第 4 輪 —— 平臺 feed 實測。**

**第四十九圈問的是：這道關卡如果安靜地失效了，我們會不會知道？**

第 3 輪（內容結構）把 `check:content` 的 24 條規則一條一條弄啞
（沒有共用的 `add()`，但都推進同一個 `problems` 陣列，那就是漏斗）：
**24／24 都會讓測試變紅**。這一圈到目前為止 **67 條規則，沒有一條弄啞了還會綠**
（a11y 32、perf 11、content 24）。

**往下一層問語料**：這一支讀七份不同的語料，逐一指到不存在的路徑 ——
`--content=` 會擋（exit 1），`--guide=`／`--syndication=`／`--astro=`／`--src=`
都會出聲（「不是『一致』，是**沒有比對到**」那種），
只有 **`--scripts=` 一句話都沒有**：數字從「137 個具名匯出裡 3 個沒有人用」
變成「74 個裡 5 個」，印出一個很有自信的錯數字。

**補了提示，然後還原了。** 那三格測試本來就刻意把 `--scripts=` 指到空目錄
（註解寫得很清楚為什麼），所以那句提示會在每次 fixture 跑的時候都響 ——
一個每次都響的警告等於沒有警告；而真的跑的時候兩個目錄一定都在。
記進 [TODO.md](TODO.md)。

（寫那句提示的時候把它放在 `else` 裡，而兩份語料都空的時候剛好走另一支、
一句都沒印 —— 這一圈在找的形狀，出現在我三分鐘前才寫的程式裡。）

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 389 筆逐輪紀錄都在
