...
**下一輪：第三十九圈第 7 輪 —— 建置與 CI。**

第三十九圈第 6 輪（文案與語氣）驗掉一條死的（`sync-feeds.yml` 的註解用錯那個字，今天 0 處），
另外兩條驗出來比原本記的更難看：

- **覆蓋率看不見分母縮小。** `check:copy` 只比 `pct`。實測把 `tags.count_one`
  整組刪掉 —— 型別 0 errors（`_one` 是動態查表）、`build` 過、`check:copy` 離開碼 0，
  而它印的是「107 組文案全部都有 en（**100%**）」。英文讀者從此看到「1 tags」，
  而六道關卡裡沒有一道說過話。補了組數比對。
- **「subject 比 bad 寬」那一格在 `bad` 壞掉的時候是空的。** 把 `taiwan-tai` 的
  `bad` 改成配不到任何東西，那一格與「每條規則都有違規樣本」**兩格都還是綠的** ——
  整套測試紅的是別的格子。補了更基本的一句：每個違規樣本，它自己那條規則抓得到嗎。

另外量到：那 67 處註解違規**數字沒動**，但註解裡的中文行從 8,619 長到 **9,299**，
倍數從 3.4× 變 **3.6×** —— 邊界沒移，長的是我每一輪寫的解釋。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 312 筆逐輪紀錄都在
