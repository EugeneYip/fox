...
>
> 最後更新：2026-09-08（**網站已上線：https://bellafoxy.com**，DNS 與 HTTPS 都好了；
> **第五十三圈開始了：動這一處，會牽動哪些地方？**）
...
**第五十三圈開始了，問的是：動這一處，會牽動哪些地方？那些牽連，有人寫下來嗎？**
判準：挑一個看起來只有單一用途的東西，真的改它一次，量出還有什麼跟著動。
理由：上一圈有兩輪是**撞上**這件事而不是去問它的。

**第 1 輪（無障礙）**：`aria-current` 在這個站身兼**無障礙狀態**與**樣式鉤子**
（Header／Footer／Pagination／LanguageSwitcher 四個元件），
而沒有一處註解寫下來。語言鈕那一個特別容易被「順手改對」——
它的值是 `true` 而別人都是 `page`。實測改成 `page`：產出裡
`aria-current="true"` 從 44 個變 0 個、目前語言的橘色粗體整站消失，
而**七道檢查全部離開碼 0**。而「改成 page」其實是錯的 ——
404 頁的「繁體中文」指向 `/`，根本不是當前頁。
改成讓樣式不看值（`[aria-current]`），畫面一模一樣，值換了樣式不會掉。

**第 2 輪（效能）**：在一個只產生 2 頁、原本沒有 `<style>` 的路由上加一個樣式區塊，
**44 頁每一頁**的 CSP 雜湊數都 +1、全站 HTML gzip **+1,813 B**、
「CSP 雜湊數」那條預算 88% → 90%。**決定是局部的，帳單是全站的**，
而付帳的是全站最接近上限的那一條。把「這條預算是全站共用的」與
「還剩幾個」（現算，目前 5 個）寫進那條預算的說明，並把上限提成常數。

**第 3 輪（內容結構）**：改一篇的 `publishedAt` 一個欄位，**61 個產出檔裡 13 個**
跟著變 —— 包括**另外兩首詩自己的頁面**（`getNeighbors()` 讓每一篇依賴它的鄰居）。
先做了對照組：什麼都不改重建一次是 **0 個**，所以那 13 個都是那一個欄位造成的。
量到的數字寫進 `getNeighbors()` 的註解（原本只有一行「上一篇／下一篇」）。
而有一個該變的沒變：`sitemap-0.xml` **一個 `<lastmod>` 都沒有** ——
內容動了，給爬蟲看的那一份完全沒有訊號（記進 TODO，`lastmod` 這個字
在這個 repo 的文件與紀錄裡一次都沒出現過）。

**第 4 輪（平臺 feed 實測）**：翻成站主聽得懂的話 ——「她在 YouTube 發一支影片，
站上會動到哪裡？」對照組 0 個，加一筆之後 **61 個產出檔裡 10 個**跟著變
（`/elsewhere` 與 `/elsewhere/youtube` 各兩語、首頁兩語、**`/about` 兩語**、
`rss-all.xml`、`search-index.json`）。`/about` 那兩個容易漏掉 ——
它不畫項目清單，它拿每個平臺的「共 N 篇」。
同一次量到：`search-index.json` 15 筆裡**站內只有 5 筆，YouTube 有 10 筆** ——
搜尋框回的東西三分之二會把人帶離這個站，而那些字是她在 YouTube 上打的。
（那條路是有人想過的：外連帶 rel、有 ↗、印平臺名、站內排序多 0.5 分。）
feed 本身連打兩次 200，狀態仍是 `ok`。

**第 5 輪（隱私與安全）**：`privacy.ts` 最不起眼的那個布林值
`strictReferrerPolicy`（說明只有一行）關掉之後：**61 個產出檔裡 14 個**變、
58 個外連的 rel 掉了 `noreferrer`（另外 2 個是搜尋頁執行時組的，也跟著變）、
而 **`/privacy` 那句話也跟著改** —— 於是 `external-link-rel-broken-promise`
（比的是承諾對事實）**不可能因為開關被關掉而響**，五道檢查全綠。
那不是漏洞（那條規則守的是「有人手寫 rel」），但做那個決定的人
看不到那 60 個連結，所以把量到的數字寫進開關旁邊。

**第 6 輪（文案與語氣）**：反過來問「改了站名，有哪一個地方**不會**跟著動？」——
答案是社群分享圖。`make-icons.mjs` 把站名、標語、題辭**手抄了一份**
（三個字串在 `site.ts` 裡都有，今天都一樣）。而它三重危險：
改站名時 44 頁、兩份 feed、manifest 都會動（manifest 還有規則在守），
只有這張 PNG 不會；它是二進位，掃文字的檢查都看不進去；
它連 npm script 都沒有。改成讀 `site.ts`，產出**逐位元組一樣**，
並補了 `npm run icons`。突變驗過：改站名 → PNG 真的跟著變。

**第 7 輪（建置與 CI）**：反過來用 ——「把一處拿掉，有誰會說話？」
把 `test:mutate` 從 `test:units` 的串裡拿掉（script 定義留著）：
走得到的測試檔 38 → 37，而 `check:workflows` **離開碼 0**、
輸出裡連 `test-file-not-run` 都沒出現。那條規則的改法早就寫著
「加一個 script，**並串進 test:units**」——**第二半句沒有人在守**。
補上（起點從三份 workflow 推，不寫死清單），id 與主體數都沒變，38／38 零誤報。
這一條四圈前就記在 TODO 並標著「→ 7 建置與 CI」，這一輪做掉、刪掉那一行。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 425 筆逐輪紀錄都在
