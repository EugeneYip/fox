...
**下一輪：第四十七圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）問了最值得問的那一個：`scripts/new-entry.mjs`
（**她自己發文的唯一一條路**）191 個 commit 沒有人碰過，而這段時間 schema 動過、
`check:content` 從十幾條長到 24 條。**它產出的東西今天還過得了關嗎？**

**過得了。** 三個 collection 都走得通；把產出改成 `draft: false` 之後，
七道檢查裡六道直接綠，`check:content` 報的是 `template-text-left`
—— 而那正是規則做對了（我留著範本文字沒改）。換掉之後**七道全綠**。

**但有一個數字過期了，而且就印在她眼前**：那支腳本最後一句寫「推完約 80 秒」，
而 `docs/CONTENT.md` 早就改成「實測 71～108 秒」（第四十四圈重量過六次部署）。
最好笑的是那一行上面三行的註解寫著「同一份步驟寫在兩個地方，遲早會有一邊過期」
—— **它預言了自己**。改成去讀文件，抽不到就只指路、不編數字。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 373 筆逐輪紀錄都在
