...
第 2 輪（效能）**打開真的網站**（`bellafoxy.com`）量瀏覽器實際抓了什麼：

| | `/` | `/poems/wu-yi-xiang` |
|---|---|---|
| 請求數（含文件） | **2** | **3** |
| 關鍵路徑 | **14,410 bytes** | 12,676 bytes |
| `load` | 54ms | 196ms |

**兩條預算拿真的線路對過了，一字不差**：「最大單頁 HTML 10.5 KB」對上 10,653 bytes、
「首次造訪關鍵路徑 14.1 KB」對上 14,410 bytes。第一次拿產品環境驗過。

**而有一句說明是推測**：`check:perf` 寫著那 176 個 `rel="icon"`／`rel="manifest"`
「也會發出請求」—— 兩頁都宣告了 4 個，而**一個都沒被抓**（兩個獨立來源都是）。
改成說得出來的「宣告了幾個」，抓不抓由瀏覽器決定。

量測條件的坑也先確認了：第一次量到主文件只有 300 bytes，那是快取命中；
加 `?cold=1` 才拿到真的 10,953。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 380 筆逐輪紀錄都在
