...
**下一輪：第四十八圈第 1 輪 —— 無障礙。**（第四十七圈八輪都走完了）

**第四十七圈問的是：這一段多久沒有人碰過了？那段時間裡，它的前提變了嗎？**

**一、網站本體 200 多個 commit 沒有人碰，而它幾乎全對。** 八輪查下來，
站上的東西**只有一處是壞的**（`skip-link` 那條規則的判準），其餘全部驗過還成立：
跳過連結的錨點 44／44、外連 58／58 帶 `rel`、`colophon` 的五句承諾五句都對、
直排的軸向與 48rem 門檻一格不差。

**二、會爛的不是程式，是「抄下來的數字」。** 三個錯全是同一種：
`new-entry.mjs` 說「約 80 秒」而文件早改成 71～108、`README` 說「四小段」
而實際五段、`CLAUDE.md` 說「每天 commit 兩次」而第二十三圈起就最多一次。
**而算出來的數字一個都沒錯**（`colophon` 的平臺數 211 個 commit 之後仍然對）。

**三、所以改法全部是「拿掉第二份」或「把語料補齊」，沒有一條新規則。**
規則數八輪一條都沒有增加 —— 這是第一次。

**四、我自己栽了四次，形狀完全一樣**：拿一個工具的輸出當答案，
而沒有先確認它答的是哪個問題（`grep -c` 數行、`grep -rl` 配到樣式、
不認得的旗標被當作沒看到、瀏覽器窗格只有 280px 寬）。第四次在寫下來之前抓到了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 378 筆逐輪紀錄都在
