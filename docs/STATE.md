...
**下一輪：第三十四圈第 6 輪 —— 文案與語氣。**

第 5 輪（隱私與安全）：上一圈我**用手**驗過 `/privacy` 每一句都是真的，
這一圈問「系統自己在驗嗎」。那句「站上的外部連結一律加上 `rel="noreferrer"`」
是**寫死的常數**，而真正決定 rel 的是 `privacy.strictReferrerPolicy` ——
實測把它關掉，58 個外部連結全部掉了 noreferrer，**而那一頁還是照樣承諾**。
答案就在手上：那個檔案第 14 行就 import 了 `privacy`，影片那一段早就在用它。
已改成跟著開關走（中英各一份），`search.astro` 那份寫死的 rel 也改用同一個常數，
並加一條稽核規則守「承諾＝事實」（主體 58）——它比對的是 `dist/` 裡的頁面
與連結，不讀 TypeScript。關掉開關時兩邊一起改口，檢查仍然綠。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 271 筆逐輪紀錄都在
