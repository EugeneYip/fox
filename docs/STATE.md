...
**下一輪：第五十二圈第 1 輪 —— 無障礙**（新的一圈，問題還沒定）。

**第五十一圈走完了**，這一圈問的是「同一件事，現在有幾個地方各做一次？做得一樣嗎？」
—— 找的不是重複的**事實**（第四十七、五十圈做過了），是重複的**做法**，
然後拿真的例子餵給它們看答案一不一樣。

**重複到處都是，而絕大多數是刻意的分工**：走一棵目錄 13 份、
「這個字算不算漢字」14 處、「什麼算第三方請求」3 層、「由新到舊」3 份⋯⋯
量下來今天的輸入上幾乎全部一致。

**真正的分岔只有兩處，而兩處都要「邊界外面」的輸入才看得到**：
`&&` 兩側不留空格（npm 合法）會讓 `check-workflows` 把整條鏈當成一個關卡名；
顏色比底色亮會讓 `test-contrast` 算出 0.93 而關卡算 1.08。
兩處都改成跟對方一致，而且都驗過**正常輸入的輸出一字不差**。

**第 8 輪那一處還有一層**：讓那個捷徑今天無害的，正是 `docs/TODO.md` 上
想拿掉的那個寫死值（`#faf6ee`）—— 兩條待辦互相牽制，而沒有人把它們放在一起看過。

**而最省事的兩輪，是因為有人先寫下了結論**（第 3、5 輪都在註解裡撞到
「第十四圈已經查過⋯這一行寫下來免得下一個人順手對齊」）。
這一圈的規則因此多了一條：**查一件事之前，先看看它是不是已經被查過了。**
