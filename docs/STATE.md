...
**下一輪：第四十圈第 4 輪 —— 平臺 feed 實測。**

第四十圈第 3 輪（內容結構）：`check:content` 早就在答這一題了 ——
它每次都印「schema 宣告了、但沒有任何一篇內容用過的欄位（**8／33**）⋯
畫面上讀這些欄位的程式碼從來沒有跟真資料跑過」。

所以這一輪把那 8 個填起來看（`cover`／`coverAlt` 上一輪做過）。
**六個都畫得出來**：`canonicalUrl`、`alsoOn`、`inResponseTo`、`updatedAt`、
`related`、`videoUrl`。其中 `videoUrl` 帶出 `VideoFacade` ——
待辦上「一次都沒算繪過」的那個元件，扛的是零第三方請求那條硬性限制。
真的畫出來之後量：載入時 **0 個 iframe**、0 個 preconnect，
`audit:privacy` 必須修正 0。**它第一次跟真資料跑，而它是對的。**

真正的錯在關卡自己：`check:content` 用「有縮排的 `title:`」抽 `poem.title`，
而短札的 `inResponseTo` 底下也有一個 `title`。於是一篇**根本沒有 `poem:`
的短札**被列進「這幾篇詩詞的 title 讀者看不到」，還說它顯示的是「某篇文章」——
那一頁的 `<h1>` 印的是它自己的 title。判準錨在縮排上，不是錨在 `poem:` 上。

沒有人發現，是因為 `inResponseTo` 在這之前一篇都沒用過 ——
**而它就寫在 `check:content` 自己那份「沒有內容用過」的名單上。**

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 317 筆逐輪紀錄都在
...
**第四十圈（進行中）：這段東西，站上真的跑過嗎？**
判準：**找出一個今天真的執行到它的地方。找不到的話，它的正確性是靠什麼保證的？**
第三十九圈第 8 輪撞出來的：`CoverImage` 的 `sizes` 三個部分都寫錯了，
而它活了二十圈沒有人發現 —— 站上 44 頁、**0 個 `<img>`**，
沒有任何一次瀏覽、任何一道關卡碰得到它。要量還得先自己做一頁出來。
這樣的東西不只一個：`VideoFacade` 一次都沒算繪過、`ExternalLink` 沒有人用、
4 個沒人用的匯出、7 個沒人用的 token、`check.yml` 在 GitHub 上跑過 0 次、
`note` 那個 feed 連續四圈 0 筆。它們都通過了型別、六道關卡、44 步測試 ——
**而通過的方式是沒有人踩到它們。**
