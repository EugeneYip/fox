...
第 2 輪（效能）用第二種算法驗了 9 個數字：頁數、HTML 合計、dist 總計、
圖片檔數與大小、CSP 雜湊、stylesheet 連結 **六個一模一樣**；兩個 gzip 數字差
約 0.9%（node 與 python 的 zlib 差異，第三十三圈已經記過同一件事）；
`最大單一檔案` 關卡說 24.9 KB 而 `dist` 裡真正最大的是 `index.html` 35.2 KB ——
差的不是數字是**範圍**（`assets` 濾掉 HTML 與文字資源，那兩類各有自己的預算）。
名字比量的東西大，但改名會弄斷十幾處引用，所以改成讓報告自己說出範圍，加 2 格測試。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 276 筆逐輪紀錄都在
