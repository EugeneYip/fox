...
**下一輪：第三十六圈第 4 輪 —— 平臺 feed 實測。**

第 3 輪（內容結構）撞到相反的一種邊界問題：**裡面多了不屬於這裡的東西**。
同步回來的 9 支影片、1420 個字是**在 YouTube 上打的**，而 `check:copy` 掃 `dist/`，
所以照樣被當成本站文案校對。實測把一支標題塞一個「台」→ `taiwan-tai` 紅 →
CI 紅 → **部署停住**，而那行字在這個 repo 裡改不了（`syndication.json` 是產生的）。
這個 repo 早就有這條界線（`QUOTED` 清單，原本只有 `poem__original`），
把 `synd__title`／`synd__summary` 加進去 —— 但 **`synd__why` 不能一起排除**，
那一句是站主自己寫的。兩個突變各釘住邊界的一側。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 285 筆逐輪紀錄都在
