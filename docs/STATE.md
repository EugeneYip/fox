...
**下一輪：第三十七圈第 4 輪 —— 平臺 feed 實測。**

第三十七圈第 3 輪（內容結構）把同一個問題問到**她**身上：`check:content`
20 條規則，三份文件加起來提到 0 條 —— 但多數規則報的是 `dist/` 或設定檔，
那是維護者的事。從程式碼機械地讀「這條規則報的是誰的檔案」：
**11 條會擋住建置並指名她寫的那一份**。
而 `docs/CONTENT.md` 有一節就叫「寫錯的時候會看到什麼」，
它只涵蓋 7 種 schema 錯誤，那 11 條一條都不在。
補進去了，而且分成兩半：6 條「你改得動的」、5 條「不是你寫錯了 ——
先 build，還在的話把訊息貼給站主」。新增 `rule-not-in-guide` 守著這份清單，
用的是 `check:copy` 那支既有的 `documentationDuty()`。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 293 筆逐輪紀錄都在
