...
**下一輪：第三十七圈第 7 輪 —— 建置與 CI。**

第三十七圈第 6 輪（文案與語氣）問的是後半題：文案這五條是七支關卡裡
**唯一**早就有文件義務的，所以「有沒有寫」早就答完了 —— 剩下「寫的跟做的一樣嗎」。
那兩份文件是用**正反例**教人的，而 `docs/CONTENT.md` 還特地說
「那種框不會被檢查，所以你可以**放心照著看**」—— 那句話是在請她信任那些例子。
用規則自己的正則跑一次那 12 個例子：**全部對得上，沒發現問題**。
所以新加的 `example-not-real` 是預防性的：`✗` 至少要有一條規則抓得到，
`✓` 一條都不能抓到。判準不綁哪一條規則，所以 `CLAUDE.md` 那一對
（沒有規則 id 的標題）也守到了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 296 筆逐輪紀錄都在
