...
第四十三圈第 8 輪（視覺與版面）走完，這一圈結束。

那一輪：`check:contrast` 印六個 ✓，五個的分母都是算出來的。有洞的是
「涵蓋率：前景 12 種、純色背景 7 種，都在 PAIRS 裡 ✓」—— 分桶那一段的
最後一行是 `if (!bucket) continue;`，**歸不到前景也歸不到背景的宣告直接消失**。
站上真的有兩種：`box-shadow: 0 0 0 1px var(--c-rule-strong)`，
以及 `--border-thin: 1px solid var(--c-rule)`（自訂屬性帶著顏色）。

改法跟這一圈其他幾支一樣：判準不動，把邊界外面數出來、印出來。
自己的測試當場抓到我兩次（gradient 混進名單、那句話寫在 `if` 裡面）。

**順帶抓到偶發紅燈的第三個原因**：`check-perf.mjs` 最後一行是
`process.exit()`，而它**不等 stdout 排空** —— 輸出接到管線時
30 次會斷 1 次，斷在第 74 行中間，讀起來像「只有 9 條有 basis」的內容錯誤。
改成 `process.exitCode` 之後 30 次都完整。**修完同一天又撞到第二支**：
`check-content` 的綠燈出口印的是最長的報告，同樣會斷（`test:content-rules`
紅在「那一行沒送到」，不是規則沒響）。兩支都改了；其餘六支同一個形狀，記在待辦。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 346 筆逐輪紀錄都在
...
**第四十四圈（進行中）：這件事，站主要自己做嗎？**
判準：**找一個「→ 站主」的待辦，問它為什麼還在那裡 —— 是真的需要他決定，
還是只是沒有人把它做完？**
第四十三圈結束時「→ 站主」的待辦有 **20 條**，最早的可以追到第二十三圈。
它們混著兩種完全不同的東西：真的需要站主判斷的（要不要刪那 7 個沒人用的
token、`ui.ts` 的 `en` 要不要改必填），以及**被標成「他的事」但其實是沒做完的**
（`check.yml` 永遠不會觸發、那 5 條的 `whyWarn` 還是空的）。
第二種每多待一圈就多一分「這是刻意的」的假象。
