...
**下一輪：第四十四圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）挑了「那 67 處註解要不要改（→ 站主）」。
那個問題在原本的形狀下**答不了** —— 輸出只說「命中 68 處」，
加一句「有一部分是刻意在引用違規本身」，卻沒說是哪一部分、有多少。

逐處看了一次（判準用**檔案的職責**：有沒有把 `lib/copy-rules.mjs` 讀進來，
不是逐行猜措辭 —— 第一版那樣猜，而且猜錯了）：
**27 處在解釋或測試規則的檔案裡，其餘 41 處只有 8 種不同的字，
而最多的那一個詞就佔了 31 處**（「平臺」被寫成半邊那個字）。

所以「67 處要不要改」的真面目是「**一個詞要不要統一**」。
而且約定自己先回答了一半：`CLAUDE.md` 寫的是「網站上的中文」，
註解不在網站上 —— 現況不是違規。要問他的其實是「要不要把約定延伸到註解」，
那需要的事實只有一個：**會動到一個詞、三十幾處、全在註解裡**。

改的是那句話說得出分家（兩個數字都是推導的），**一處註解都沒動**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 352 筆逐輪紀錄都在
