...
**下一輪：第三十六圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed 實測）先照規矩打了一次網路：**11 個平臺全部 200**
（youtube 124ms／15 筆，最慢的 github 870ms／30 筆），那一格沒問題。
真正的發現在那條邊界外面：`--patterns` 打的是**網路**，而站上算繪用的是
`src/data/syndication.json` —— 那個檔案第二行就寫著 `$schema`，
指的那份合約**從第一個 commit 就在，而沒有任何關卡執行過它**。
實測後果：少一個必填欄，`npm run build` 成功、六道關卡全綠，
而產出裡多出 **6 個沒有 href 的 `<a>`** —— 點不動也 tab 不到。
`check:a11y` 看不到是設計使然（每條連結規則都以 `href=` 開頭）。
補上的驗證器**讀那份 schema 檔本身**，不手寫第二份必填清單，
看不懂的關鍵字會說出來而不是安靜略過。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 286 筆逐輪紀錄都在
...
**第三十六圈（進行中）：這道檢查的邊界外面是什麼？那裡現在有幾個？**
第三十五圈拿第二種算法對了約 50 個數字，而兩種算法的差幾乎每次都落在
**涵蓋範圍的邊界上** —— `<script>` 裡的連結、非 HTML 的資源、`min-width` 的查詢、
豁免掉的檔案、帶佔位符的字串。每一個差都是真的東西，只是不在某一支的視野裡。
上一圈已經讓七支關卡各自說出範圍，這一圈往外一步。
...
| 輪 | 結果 |
|---|---|
| 1 無障礙 | 6 個裡 5 個一樣；`link-name` 899 vs 901 → `strip()` 先拿掉 `<script>`，**關卡對** |
| 2 效能 | 9 個裡 6 個一樣；2 個是 node／python 的 gzip 差 |
| 3 內容結構 | 5 個裡 3 個一樣；**兩次都是我的語料錯** |
| 4 平臺 feed | 「9 筆」四種算法都說 9；**「今天」用 UTC 而專案釘的是 Asia/Taipei** |
| 5 隱私 | 176 追到第三次才對上 —— 那是**豁免之後**的數字 |
| 6 文案 | 191 與 108 都對，但兩個的分母不同而報告沒說 |
| 7 建置與 CI | **68 ＋ 20 ＋ 1 = 89，而那一行寫「合計 90 秒」** |
| 8 視覺 | 6 個全對，結掉一條躺了好幾圈的待辦 |

**只有兩個是真的壞掉**（第 4 輪的時區、第 7 輪的四捨五入），
而且形狀一模一樣：**同一個量，在同一個地方，用了兩種算法**。
兩個都小到不會壞掉、剛好大到會讓人停下來 —— 那正是它們活得久的原因。

其餘六輪數字都對，改的都是同一件事：**把分母／語料／判準說出來**。
最值得記的是第 8 輪那條待辦 —— 它躺了好幾圈沒人驗，卡住的不是工夫，
是那句話沒說自己在數什麼；把判準寫出來，當場就結掉了。

**我自己八次「以為關卡算錯了」，八次都是我的語料或判準不對。**
救回來的動作每次一樣：拿關卡自己的常數、正則、語料去跑一次。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十五圈走完了」。
