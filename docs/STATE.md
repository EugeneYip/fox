...
**下一輪：第三十三圈第 6 輪 —— 文案與語氣。**

第 5 輪（建置與 CI）修的是 `npm run mutate` —— 這一層的讀者就是下一個
來跑輪次的人，而 CLAUDE.md 指名要他用這支工具，理由是「配不到就會停下來」。
這一圈前四輪我自己用了十幾次，踩到兩次它**安靜地騙人**：
連續突變兩次時 `--restore` 印「已還原」但留下髒污（備份被覆蓋），
以及 `--from` 含反斜線時永遠配不到（`npm run` 多經過一次 sh）。
兩個都修了，測試從 6 格變 12 格，CLAUDE.md 補上兩行。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 263 筆逐輪紀錄都在
