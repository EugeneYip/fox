...
**下一輪：第四十八圈第 8 輪 —— 視覺與版面。**

第 7 輪（建置與 CI）把整條管線的兩端第一次對起來。所有關卡量的都是本機的
`dist/`，`ci:sim` 比的是「本機的 dist」跟「版控建的 dist」——
**兩邊都沒有人去打 `bellafoxy.com`**。

**版控建出來的，跟站上出的，61 個檔案逐 byte 一模一樣。**
做法是 `git archive HEAD` 到一棵乾淨的樹（就是 CI 拿到的那 203 個檔案）、
建置、把每個檔案對到網址抓回來比。那把尺先失敗過才成功：拿**工作樹**的
`dist/` 跑，回報 59 一樣、2 不一樣，差的正是 `/about` 與 `/en/about` ——
本機多一段 `所在　臺灣`（`identity.local.ts` 在 `.gitignore` 裡，
`identity.local.example.ts` 的註解早就寫過這件事）。`ci:sim` 這次指到的也是同兩頁。

**順帶量到建置是可重現的**：同一份原始碼建兩次，整棵 `dist/` 一個字都沒差。
沒有這一格，上面那個 61／61 證明不了什麼。

**少部署那個 paths 濾網第一次有人驗。** 判準不是「看起來像不像」，
是「站的 import 圖有沒有踩到 paths 外面」：276 條本地 import、**0 條在外面**；
在 `src/` 裡加一條指到 `scripts/` 的 import，它就報 1 條並點出是哪一行。

**`sync-feeds.yml` 第 3 行跟同一個檔案第 107 行相反** —— 開頭寫「推到 main
就會觸發部署」，而底下正確地說明 GITHUB_TOKEN 的 push 不會觸發任何 workflow
（所以才有那一步 `gh workflow run`）。開頭那句改掉了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 385 筆逐輪紀錄都在
