...
第四十一圈第 1 輪（無障礙）翻了兩課出來驗：

**「算可及名稱之前先拿掉 `aria-hidden` 的子樹」（第十四圈）—— 修乾淨了。**
把每一個 `textOf(` 都翻出來逐一問，五個位置：三個算可及名稱的都有拿掉，
另外兩個一個只用在錯誤訊息裡、一個算的是可見文字（不該拿掉）。

**「判斷寫兩份遲早會分岔」（第十六圈）—— 這一課沒有。**
`svg-unnamed`（第三十六圈才加，比那一課**晚**）又寫了第二份「有沒有名字」。
**而它是對的** —— SVG 的 `title` **屬性**不算名字（要 `<title>` 子元素），
共用 `hasAccessibleName()` 會讓這一條變寬。**錯的不是那個決定，
是那個決定沒有說出口。** 補上理由，加兩格測試釘住差別；
其中「`title` 屬性不算名字」那一格擋的正是未來有人「順手統一一下」。

順著找到**第三份**（`same-name-different-target`），它比那一支窄。
量今天的產出：901 個連結，**0 個**受影響 —— 但那是因為站上 0 張 `<img>`、
99 個 SVG 全部 `aria-hidden`。記進待辦。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 323 筆逐輪紀錄都在
