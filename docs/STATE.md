...
**下一輪：第四十三圈第 8 輪 —— 視覺與版面。**

第 7 輪（建置與 CI）先數了幾個分母：`ci:sim`（從 deploy.yml 讀、自我驗證、
還會說涵蓋幾步）、`npm run check`（tsconfig 是 `**/*`）、
`test:tools` 的 44 步（49 支腳本每一支都有 npm script）—— **這一區大致健康**。

有洞的是「部署路徑上該跑哪幾道」：那一段展開 `test:tools` 時只認 `npm run X`，
而它現在是 `node scripts/run-steps.mjs test:units test:built` ——
**一個 `npm run` 都沒有**，必跑清單從 3 條縮成 1 條。
實測把 deploy.yml 的 `test:built` 拿掉：舊版**離開碼 0、一條規則都沒響**。

測試沒抓到，是因為 fixture 的 `package.json` 寫的是**舊寫法**。

順手：`*.orig`（`npm run mutate` 的備份）進 `.gitignore` ——
這一圈第 2 輪真的把一個 commit 進去過。

**更正第 4 輪的一句話，然後把那個偶發紅燈抓到了**：那筆寫「離開碼 1 而輸出說
44 步全過」，其實是**兩次不同的執行**（我把失敗那次的輸出丟掉了）。
這一輪改成把輸出寫進檔案再看 —— 第三次就紅了，而且留著證據：
`X Retry-After HTTP 日期 5907ms`，停在第 1 步 `test:throttle`。

那一格的下限寫死 `> 6000`，而 `toUTCString()` 只到秒（最多截 999ms）
再加上那幾行之間真的過了多久 —— 機器忙的時候就不夠。
改成拿同一個 `t0` 推下限，**斷言反而更緊**。

**再跑一次又紅一次，而這次更嚴重**：停在 `test:perf-budgets`，
而跑完 `git status` 看得到 `M scripts/check-perf.mjs` ——
那支測試會**把版控裡的檔案改壞再寫回去**（`現在約 10.4 KB` → `3.2 KB`），
中間一失敗就回不來，之後每一次都紅。看起來像偶發，其實是上一次留下來的。
量到的：修之前連五次紅一次、同時三份紅兩份；修之後連六次全綠。
改成改副本、`finally` 刪掉，並加一格斷言「跑完原檔一個字都沒變」。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 345 筆逐輪紀錄都在
...
**第四十三圈（進行中）：這個綠勾的分母是什麼？誰決定了它？**
判準：**找一個印出 ✓ 或百分比的地方，問它的分母怎麼來的 —— 是數出來的，
還是一份人挑的清單決定的？如果是後者，那個 ✓ 涵蓋了多少？**
前五輪的答案一樣：一支關卡可以有好幾個分母，而**推導出來的那幾個通常沒事，
出事的是決定語料的那一份** —— 第 1 輪是那句說明涵蓋範圍的話只認連結，
第 2 輪是決定「用哪一把尺量」的副檔名清單，第 3 輪是決定「看得到哪些檔案」的
另一份副檔名清單，第 4 輪是**分母整個取自產出而不是設定檔**，
第 5 輪是**少一個資料夾**，第 6 輪是**只數了兩個檔案**。
第 5 輪也第一次遇到「這一格已經有人守了」—— 那份豁免名單每一輪都自己在說。站上這種句子還有很多
（44 頁、31 條規則、108 組文案 100%、11 條預算、901 個連結⋯），
逐句問那個分母是誰決定的、它漏掉了什麼。
