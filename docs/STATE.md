...
第 5 輪（隱私與安全）走的是 `docs/PRIVACY.md` 的「要顯示本名該怎麼做 —— 兩道
關卡，缺一不可」。那一節從來沒有人照著走完過（走完就會有個資出現在產出裡）。
這一輪走了，然後拆回來。

**兩道關卡都走得完。** 把開關從 `false` 改成 `true` 再建置，稽核那一行從
「站上有幾個：**0 個**」變成「**2 個**出現在產出裡（2／51 個檔案）」——
**綠燈，而且它自己解釋為什麼綠**（「那是 privacy.ts 的開關決定的，不是問題；
這支稽核守的是『不進 repo』，而 dist/ 在 .gitignore 裡 —— 但那是這個站唯一
真的把個資送出去的路，所以把數字說出來」）。開關已還原、重建，回到 0 個。

**文件說值有兩個來源，量出來是替換不是相加**：設了 `PRIVACY_NEEDLES` 之後
「8 個值」變成「1 個值，來自 PRIVACY_NEEDLES」。跟文件那張「情境對來源」的表
一致，但以前沒量過 —— 在 CI 上，secret 裡漏掉一個值就等於那個值沒人守。

**擋人的那條路真的會擋，而且不必拿真的值去試**：把一個公開字串當假的個資餵進去
（`PRIVACY_NEEDLES='狐說八道'`），稽核 exit 1、59 條必須修正。
全程沒有任何真值離開 `identity.local.ts`。
