...
第 8 輪（視覺與排版）收掉這一圈：`check:contrast` 每個數字都算得準，
但算的是 **`PAIRS` 上有的那幾組**，而那是人手維護的清單。
用 `npm run preview` 起真的產出，在瀏覽器裡走 4 頁量每個文字元素的
`color` 與第一個不透明背景 —— 真的出現 **10 種**組合，**1 種不在表上**：
`--c-ink-faint` 畫在 `--c-bg-sunken` 上（`/colophon` 的定義清單標題）。
補上之後淺色 **4.52**、深色 5.05，門檻 4.5 —— 落在全站最緊的那一段
（最緊的是已經有人守的深色 4.51）。這一圈**八輪全部走完**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 290 筆逐輪紀錄都在
