...
**下一輪：第三十九圈第 8 輪 —— 視覺與版面。**

第三十九圈第 7 輪（建置與 CI）驗了八條，**沒有一條死掉**，修掉兩條：

- **十條規則的名字只活在歷史紀錄裡。** `check:workflows` 的 10 條 id
  逐一 grep 過整個 `docs/` 與根目錄 —— 十條**全部只出現在 `REVIEW-LOG.md`**，
  而那是歷史紀錄，不是給人查的文件。被擋下來的人查不到那個名字是什麼意思。
  `docs/DEPLOY.md` 補了一張表，並加第 11 條規則 `rule-undocumented` 守著它
  （`check:copy` 與 `check:content` 早就有同樣的義務，這一支是漏掉的那一支）。
- **「N 道關卡」不一定是在數關卡。** 上一輪寫的一句敘述被讀成宣稱擋了下來。
  量出來的分界線很乾淨：三處真的宣稱距離最近的 `verify:all` 是 13、14、15 個字元
  （都同一行），那句敘述是 **1,811** 個字元。判準改成「同一行要提到 `verify:all`」，
  沒過線的會說「沒有比」而不是安靜跳過。

其餘六條（`verify:all` 還是 `&&` 串、本機 Node 22.15.1 低於 `engines`、
`.nvmrc` 只寫主版本、`check.yml` 不會自己觸發、`sync:health` 沒進六道關卡、
`--real-install` 沒有成功路徑的測試）今天都還成立。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 313 筆逐輪紀錄都在
