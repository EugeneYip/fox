...
**下一輪：第四十六圈第 7 輪 —— 建置與 CI。**

第 6 輪（文案與語氣）**沒有改程式**，量到兩件以前只有猜測的事。

**英文介面是真的英文。** 12 頁英文全都有漢字，但拿 `ui.ts` 自己的 89 組去比對，
**沒有任何一句是退回的介面字串** —— 那些漢字是語言切換器的標籤「繁體中文」、
站徽那個「狐」字、以及內容本身（詩題、原文、影片說明）。

**少一句英文的代價也量出來了**：把 `nav.skipToContent` 的 `en` 拿掉，
那句中文出現在 **12／12** 頁英文上，`check:copy` 說「**這是回退**」
卻**離開碼 0**。不擋的理由是「缺 en 是合法的中間狀態」——
而量出來那個狀態在這個 repo 裡從來沒發生過（89 組全部都有）。
沒有自己改嚴重度：那是政策問題，待辦上本來就標著「→ 站主」，
這一輪把它變成有數字的決定。

**規則 9 的第一次真正使用**：「`EN_COVERAGE.pct` 仍然是手寫的 100」量過之後
不成立（`pct` 上限就是 100，不可能往上飄），從 `docs/TODO.md` **刪掉**了。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 368 筆逐輪紀錄都在
