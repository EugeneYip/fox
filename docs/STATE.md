...
**下一輪：第三十八圈第 8 輪 —— 視覺與排版。**

第三十八圈第 7 輪（建置與 CI）追那條「偶發紅燈沒有留下證據」的待辦，
發現**那不是偶發，是結構性的**：`test:tools` 是 `&&` 串起來的，
而那種串法的紅燈**輸出結尾是綠的**。實測：離開碼 1、輸出 919 行、
失敗在 802–834 行，**後面還有 85 行 `✓`** —— `tail` 看到的全是勾。
（那也正是第三十七圈那兩次當下看不出是哪一格的原因之一：我看的是結尾。）
新增 `scripts/run-steps.mjs`，紅的時候結尾直接說「停在第 23 步／共 42 步：
npm run test:copy-rules」加上證據與重跑指令。順序沒有抄第二份 ——
它讀 `package.json` 那兩串 `&&` 並遞迴展開。CI 不受影響。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 305 筆逐輪紀錄都在
...
**第三十八圈（進行中）：這件事現在靠誰記得？忘了會怎樣？**
第三十七圈八輪裡有兩輪的答案是「靠人記得回來寫」（平臺的 `verifiedAt`、
`tokens.css` 註解裡的對比值），而兩個都是在**別的事情上追下去才撞到的**。
待辦上還躺著好幾條同一個形狀：`--patterns` 不會把日期寫回去、
`check.yml` 永遠不會自己觸發、瀏覽器掃描沒有變成工具、
螢幕閱讀器「還沒有人做過」、`ci:sim` 只有人手動跑才會跑。
...
### 最近一圈（第三十七圈：這一條規則是誰要求的）

八輪找到的是**同一個形狀的八個實例**：一份文件對某件事做了**可以被機械
檢查的宣稱**，而檢查它的要嘛沒有，要嘛只做過一次、由人手做的。

| 輪 | 哪份文件的哪句話 | 給誰看 |
|---|---|---|
| 1 無障礙 | `A11Y.md` 拿 5 個規則 id 當「已經自動守住」的證據 | 做人工走查的人 |
| 2 效能 | 「全站的 JavaScript 只有**四小段**」 | 想理解取捨的人 |
| 3 內容結構 | 「寫錯的時候會看到什麼」只涵蓋 7 種 schema 錯 | **她** |
| 4 平臺 feed | 「已實測」**沒有日期** | 要不要相信這個樣板的人 |
| 5 隱私與安全 | `/privacy` 的 8 條承諾 | **讀者** |
| 6 文案與語氣 | 兩份文件的 12 個正反例 | 寫文案的人 |
| 7 建置與 CI | 「六道關卡」寫在 **4 份**文件裡 | 維護者 |
| 8 視覺與排版 | `tokens.css` 註解裡的對比值 | 下一個要改色的人 |

**只有一句是當場錯的**（第 2 輪：實際五段），而漏掉的那一段偏偏是
唯一會擋住第一次算繪的。其餘七輪當時都是對的 ——
**這一圈的價值不在抓錯，在讓它們保持對。**

兩個反覆出現的形狀：**同一個數字寫在 N 個地方**（「六道關卡」寫了 4 次，
而且已經漂過一次）；**「指得出誰能證實」與「真的被證實」是兩件事**
（`verifiedAt` 與對比值，兩處都指得出工具，而寫下結果的都是人）。

完整紀錄在 [REVIEW-LOG.md](REVIEW-LOG.md) 的「第三十七圈走完了」。
