...
**下一輪：第五十一圈第 1 輪 —— 無障礙**（新的一圈，問題還沒定）。

**第五十圈走完了**，這一圈問的是「照著文件做，做得完嗎？」——
挑一件文件宣稱可以這樣做的事，一個字一個字照著做，不用已經知道的捷徑。

**照著走了九件事，七件走得完，兩件卡住。** 卡住的是 `docs/A11Y.md` 的
VoiceOver 那一節（第一步 `⌘F5` 按不了）與 `npm run write` 的互動問答
（第二題 exit 13）。**差別在有沒有第二條路**：VoiceOver 需要一雙耳朵，沒有；
`npm run write` 有旗標那條路，**而它唯一寫下來的地方是六萬三千行的輪次紀錄**。補進去了。

**文件裡的數字抓到三處錯的，都在最前面**：`README.md`「一次跑完五道關卡」
（同一行括號裡列了六個）、`docs/DEPLOY.md`「守的十一件事」（表格 12 列），
以及我把 README 加進守衛之後那條守衛自己訊息裡寫死的「四份」。
沒人擋是因為 `gate-count-stale` 的語料只有四份文件，**`README.md` 不在裡面**。

**規則本身幾乎沒有錯，錯的是講規則的話。** 八輪下來程式沒有一處是壞的，
改的六處全部是文件與註解 —— 跟第四十八圈從讀者那一端看的結論同一句。
