...
第 1 輪（無障礙）逐條驗了七條被抄了很多圈的待辦，**沒有改任何一支關卡** ——
量出來的東西本身就是產出：

- **一條不成立了**：「`role="status"` 沒有被檢查」。站上 44 個
  `role="status"` 每一個都同時帶 `aria-live`，而 `live-region-value`
  數的正是 `aria-live`；而且那個角色本身沒有值可以打錯。
  （加那條規則的那一輪只結掉待辦的前半，後半其實同時失效，卻被抄了六圈。）
- **三個數字都不對**，而其中兩條是同一件事：「5 條」實際 6、「7 條」實際 8、
  「15 條」跟「7 條」是同一件事也是 8 —— 而 `test:a11y-rules` 每次都自己印。
- **一條要分兩半**：搜尋結果那 2 個連結「沒有規則看過」還成立，
  「沒有人看過」不成立了（第四十四圈第 1 輪在瀏覽器裡量過）。

所以「每一輪的規則」多了第 7 條：**待辦裡不要寫死工具算得出來的數字**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 355 筆逐輪紀錄都在
