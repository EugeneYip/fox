...
**第四十八圈問的是：站上真的看得到的東西，我們量過幾成？**
判準：不從 repo 裡的檔案出發 —— 打開頁面，把讀者實際會經歷的東西量一次。
理由：第四十七圈查完很清楚 —— 幾乎每一條檢查問的都是「repo 裡的東西對不對」，
而不是「讀者經歷到什麼」。

第 1 輪（無障礙）**沒有改任何東西**。真的按 12 次 Tab：
**12／12 都有可見的焦點框**（`outline 2px rgb(176,77,28)`）、12／12 符合
`:focus-visible` —— 焦點框本身從來沒有人真的按 Tab 看過。
跳過連結也有效：`activeElement` 雖然停在 `BODY`（`<main>` 沒有 `tabindex="-1"`），
但接下來三次 Tab 全部落在主要內容裡。

**兩個看起來像 bug 的發現都是工具的限制**：Enter／空白鍵在這個環境按不動
任何原生控制項（兩個獨立控制項都不動，而 `.click()` 都動）；
無障礙樹對「內容是巢狀區塊」的連結不顯示名字（DOM 裡其實有文字）。
兩次都在寫下來之前抓到 —— 做法只有一個：**量到不對勁時先找第二個獨立的例子試一次**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 379 筆逐輪紀錄都在
