...
**下一輪：第五十圈第 1 輪 —— 無障礙**（新的一圈，問題還沒定）。

**第四十九圈走完了**，這一圈問的是「這道關卡如果安靜地失效了，我們會不會知道？」
—— 不是問規則抓不抓得到 bug（那是突變掃描），而是把規則自己弄成不會失敗，
再看有沒有任何東西說話。

**110 條規則一條一條弄啞，104 條會被抓到**（a11y 32／32、perf 11／11、
content 24／24、**privacy 27／33**、copy 10／10；contrast 沒有規則 id，
四種弄壞法全部會擋）。沒被抓到的六條全是同一個原因：隱私那一支的測試
import 庫、不執行腳本，所以正則有人守、**接線沒有** ——
而同樣切法的 `check:copy` 會建假專案真的去跑，補法是現成的。

**真正沒有人守的是「一整支測試從套件裡消失」**：把 `test:a11y-rules` 從
`test:units` 拿掉，`test:tools` 印「43 步全部通過」並 exit 0。

**抓到「整段東西不見了」的，往往是看起來只在守文件的檢查** —— 三次：
`gz()` 回傳常數（「說明裡的現值」出聲）、`verify:all` 少一道（`gate-count-stale`）、
`PAIRS` 少一組（文件裡引的對比值算不出來）。把量到的數字寫進散文再回頭比對，
在這一圈裡是最有效的完整性檢查。

**語料空掉會大聲擋，縮水但沒空掉幾乎都是安靜的** —— 三處都只發生在測試專用的
旗標或不可能的狀態上，所以都進了 [TODO.md](TODO.md)。

**新規則 0 條，連續第三圈。** 這一圈唯一的改動是 `MEASURED.date`
（因為真的跑了一次 `probe:served`）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 394 筆逐輪紀錄都在
