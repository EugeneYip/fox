...
**第五十圈問的是：照著文件做，做得完嗎？**
判準：挑一件文件宣稱「可以這樣做」的事，一個字一個字照著做 ——
不用已經知道的捷徑、不補文件沒寫的步驟。中途卡住的地方，
就是接手的人會卡住的地方。理由：這個 repo 的前提是「隨時可能換人接手」，
而四十九圈裡沒有一圈真的照著文件走過一次。

第 1 輪照著走的是 `docs/A11Y.md` 的「目標尺寸與 Tab 順序」四個步驟
（走的是第二條路：直接開 bellafoxy.com，不起 dev server）。
**八頁 40 個數字一個都沒有變**，最擠的一對仍然是頁尾的「搜尋 ↔ RSS」。
這是那張表第三次重驗，也是第一次**因為文件說要這樣做**而跑的。
日期照那一節自己的慣例記進去，`check:a11y` 現在印「上一次量是 2026-09-07（0 天前）」。

**卡住的只有一處**：步驟 4 說「每一頁各跑一次」但沒說是哪幾頁 ——
要往下捲過三段散文才看得到基準表。那一句補上了。

**第二節「螢幕閱讀器」照著做做不完**：它寫得很完整、標題還寫著「不用裝東西」，
但第一步 `⌘F5` 就按不了。那一格需要的不是相依套件，是**一雙耳朵**（產出是聲音，
不是回傳值）—— 跟上一節那種「靠人記得貼」不是同一種卡住，而文件把兩件事
寫得很像。記進 [TODO.md](TODO.md)。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 395 筆逐輪紀錄都在
