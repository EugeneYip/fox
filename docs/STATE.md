...
**下一輪：第四十二圈第 6 輪 —— 文案與語氣。**

第四十二圈第 5 輪（隱私與安全）的目標是**上一圈自己記下的那句話**：

> `STRUCTURAL_IDS` 是手寫的。這一輪是靠比對 `saw()` 才發現漏三條，
> 而那個比對是臨時寫的 —— **下一條漏掉的一樣不會有人知道。**

那支測試早就有一格「每一條結構規則都有呼叫 `saw()`」，但那問的是別的方向。
上一圈那個 bug 是：一個 id 可以**有** severity、**也有** `saw()`，
卻不在 `STRUCTURAL_IDS` 裡 —— 而那份名單是「補 0」用的，
不在裡面的規則只要所在區塊沒跑到（例如沒有 `dist/`）就整條消失。

把那個臨時比對變成常設的一格，**兩個方向都比**（用到卻沒登記的、
登記了卻沒有人用的），突變兩邊都驗過。順手數了 `ALLOWLIST` 的 9 條路徑
——**9／9 都指到真的存在的檔案**，沒有為它補守門（0 件的問題不值得
讓別的地方付代價，跟上一圈第 6 輪同一個判斷）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 335 筆逐輪紀錄都在
