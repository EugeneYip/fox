...
第 1 輪（無障礙）問了 `check:a11y` 的三個分母。「44 頁」與「31 條規則」
都是推導出來的，有人守；沒人守的是第三個 —— 那句
「另有 2 個 `<a href=…>` 寫在 `<script>` 裡」。

**它宣稱的是「上面每一條規則都沒有看過它們」，而它只認連結。**
實測把 `<button>` 與 `<img>` 藏進一頁的 script 裡：報告一個字都沒說，
還把 `button-name`、`img-alt` 列進「這次沒有東西可看的規則」，
底下寫著「**站上沒有這種元素**」—— 那一頁明明有。

改成由**規則自己的樣式**推導（同一個正則數兩次，差額就是掃不到的量）。
站上的輸出從一句話變成四條規則，其中 `blank-rel` **是三十五圈以來
第一次被提到**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 339 筆逐輪紀錄都在
...
第 1 輪的答案是：一支關卡可以有好幾個分母，而**推導出來的那幾個通常沒事，
出事的是那句用來說明涵蓋範圍的話自己**。站上這種句子還有很多
