...
第 8 輪（視覺與排版）收掉這一圈：顏色的來歷寫在 `tokens.css` 自己的註解裡
（「已調深到 4.94:1」、「rule-strong 只有 1.55:1」）—— **那些數字是選這個
顏色的理由**。逐個對過關卡算的，**全部一模一樣**。問題在那段註解的下一句：
「不必相信這行註解，`--verbose` 每次都會算一次（第三十四圈逐個對過）」——
它指得出誰能證實，而那次證實是**人手做的**。現在每次都做。
這一圈**八輪全部走完**。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 298 筆逐輪紀錄都在
