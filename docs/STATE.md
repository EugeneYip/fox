...
第三十九圈第 2 輪（效能）逐條驗效能的待辦，其中一條是唯一**會說謊**的：
輸出裡那句「全是 favicon、PWA 圖示與 og:image」今天是真的
（7 個檔案逐個看過），**但那是巧合** —— 它印出來的條件只有
「有圖片檔、而且頁面一個都沒載入」。往 `public/` 丟一張沒人引用的內容圖，
那個條件仍然成立，於是它照樣說「全是 favicon」，
還**剛好蓋掉唯一值得注意的東西**。改成照檔名算、判準印出來、
認不出來的逐個列名。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 308 筆逐輪紀錄都在
