...
**下一輪：第四十六圈第 5 輪 —— 隱私與安全。**

第 4 輪（平臺 feed 實測）**沒有改任何東西** —— 量完找不到值得改的，照規則 4 誠實寫下來。

先量今天：`verify` 是 `200 Atom 9 筆`，`sync` 9 筆、2.5 秒。
然後把 feed 管線的零件一個一個拿掉再對真的網路跑一次：
**五個零件（三條策略、節流、重試）沒有一個拿掉會改到輸出**，
也沒有任何一道關卡說話。原因量得出來：站上只有一個來源、`feedKind` 是
`hybrid`，所以另外三條策略（68 行）今天一次都不會被執行到。

**沒有補檢查**，理由是規則 10 —— 說不出它在過去十輪會擋下哪一次真的事故。
為休眠的程式碼加檢查正是這一圈想停下來的那件事。

這一輪也是第一輪照新規則跑的：紀錄裡不再結轉待辦，新發現的兩條寫進 TODO.md。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 366 筆逐輪紀錄都在
