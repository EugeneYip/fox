...
**下一輪：第三十八圈第 6 輪 —— 文案與語氣。**

第三十八圈第 5 輪（隱私與安全）實測：把 `identity.local.ts` 拿掉、
也不設 `PRIVACY_NEEDLES` —— `audit:privacy` 印一行警告，然後結尾寫
「必須修正 0」、**離開碼 0**，六道關卡全綠，
而**唯一保護本名與校名的那幾條一次都沒跑**。
後果特別重的理由：**這個 repo 是公開的** —— 本機這一次是推上去之前
唯一會跑的檢查，CI 紅燈救不回一個已經公開的字串。
而「乾淨」那條結尾的註解**自己就寫著它是為了這個情況存在的**，
卻算的是 `notices`，而身分那一行是單獨印的、不在計數裡。
兩條結尾現在都會把缺口說出來；離開碼不改（那是既有的決定）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 303 筆逐輪紀錄都在
