...
第 7 輪（建置與 CI）讀的是 `docs/DEPLOY.md` 那一節「改 workflow 之前：
`check:workflows` 守的**十一件**事」——照著讀，然後數。

**那一節裡同一個數字寫了四次，兩次十一、兩次十二，而真的是 12。**
沒有人擋是因為 `rule-undocumented` 比的是每條規則的 **id** 有沒有寫進文件，
不是那個中文數字。兩處改掉了。

**然後在旁邊撞到一個更大的**：`README.md` 寫著
「`npm run verify:all` —— 一次跑完**五道**關卡（型別／隱私／對比／建置／無障礙／效能）」
**同一行的括號裡就列了六個**，而 `verify:all` 真的有六步。

**而這個 repo 有一條規則的整個用途就是抓這件事** —— `gate-count-stale`。
它今天綠的，因為 `CLAIM_DOCS` 只有四份，**`README.md` 不在裡面**。
（同一個檔案第 5 輪〔第四十七圈〕也過期過一次：「四小段」而實際五段。）
加進去了，突變驗過會紅。

**補完之後那條規則自己的訊息也過期了**：前面的份數是算出來的，
後面兩個「四份」是寫死的 —— 在修這個問題的同一分鐘裡同一種問題又出現一次。
兩處都改成算出來的。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
**還沒做的事在 [TODO.md](TODO.md)**（一份會被編輯的清單，不是結轉的）。

輪替順序、每一輪的規則、以及全部 401 筆逐輪紀錄都在
