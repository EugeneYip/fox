...
第 1 輪（無障礙）把產出裡的元素種類數出來（45 種），對照規則的視野：
**`<svg>` 99 個、`<details>`／`<summary>` 各 44 個，一條規則都沒看過**，
而 `img-alt` 的主體**一直是 0** —— 站上一張 `<img>` 都沒有，圖示全是內嵌 SVG。
「圖有沒有替代文字」這件事，真正的那 99 個整批落在規則外面。
實際狀態是乾淨的（99／99 都有 `aria-hidden`），所以加的 `svg-unnamed`
今天不會響，它守的是下一個加圖示的人。突變第一次沒紅**而那是規則對**
（那些 SVG 還有 `role="presentation"`，我的規則正確地也當成藏起來）。
規則 28 → 29，兩個既有守衛（文件規則數、`SEVERITY` 表）當場各說話一次。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 283 筆逐輪紀錄都在
