...
第四十一圈第 2 輪（效能）驗的是兩輪前自己記下的一課：
`check:perf` 有一句改法叫人去改一個**早就不存在的 prop**，
當時修好了那一句，也加了測試 —— 但那一格是**寫死在那一句上的**，
而那一輪的待辦自己寫著「其他關卡的『改法』也可能點名不存在的東西」。

掃九支關卡：105 段「改法：」＋ 11 個 `fix:` 區塊，其中 **31 個檔案引用**，
**指不到東西的 0 個**。這一課今天是乾淨的 —— 但沒有東西在守那 31 個。
所以加了第 4 條規則 `advice-target-missing`，放在 `check:doc-links` 裡
（那一支開頭就說「接手的人是照著文件的連結走的」——
被紅燈擋下來的人是照著改法走的，同一件事換一個讀者）。突變驗過。

**我自己這一輪錯了三次，三次都是同一個形狀**：語料定義成「含『改法：』」而
漏掉了 `fix:`（也就是漏掉這一課的來源本身）、路徑用猜的（10 個誤報）、
借了一個只吐 `.md` 的 `walk()` 來建檔名清單（26 個誤報）。
三次都是**數字不對勁**讓我回頭去讀定義的。另外還犯了第十二次
「東西放在消費者後面」（新規則的 `saw()` 排在 `total` 後面，
摘要的「50 個連結」變成 19）。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 324 筆逐輪紀錄都在
