...
第 2 輪（效能）把建置層的設定一個一個換掉再量：

| 換掉什麼 | 誰說話 |
|---|---|
| `inlineStylesheets: 'never'` | 單頁請求數（每頁 4.7 個 stylesheet，上限 4）＋ 說明數字過期 |
| `inlineStylesheets: 'always'` | **只有**說明數字過期 —— 13.9 KB 是上限 14.0 KB 的 **99%** |
| `compressHTML: false` | **沒有人說話**（gzip 後全站 +3.2%） |

**而更安靜的是另一件事**：`inlineStylesheets: 'auto'` 不只是效能設定，
它是 `audit-privacy`／`check-a11y`／`check-content`／`check-perf` 四支的**前提**
—— 那四支的掃描範圍就是照那句話決定的（樣式在 HTML 裡還是在 `.css` 檔裡）。
設定改掉的話它們會安靜地少看或多看一整類東西，而**沒有東西在比那個值**。

補了一條檢查（住在 `check:perf`）：判準不寫死 `'auto'`，
去問那四支腳本自己的註解怎麼寫，跟 `astro.config.mjs` 真正的值比。
三個方向都突變過。

**順手抓到那個掛了三圈的偶發紅燈。** 寫紀錄時 `test:tools` 紅了一次，
這一次**留了檔**，看得到是 `test-ci-sim`：慢的那一步靠忙等 700ms 造出來，
而兩步都要先經過一次 `npm run`（約 1.3 秒），機器一有負載就併成同一個秒數。
改成 3000ms，連跑三次都綠。查得到是因為第四十五圈第 7 輪加的反貧化訊息
把紅燈變成一句說得出原因的話 —— 而**留輸出**這件事現在寫成規則 8。

做法：一次只深入一個面向（八個面向輪流），每一圈換一個**問題**去問全站。
輪替順序、每一輪的規則、以及全部 364 筆逐輪紀錄都在
