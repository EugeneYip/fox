...
await check(
  ...
  { tokens: realTokens.replaceAll('--c-ink-faint:', '--c-ink-dim:') },
  {
    ...
    missing: [
      ...
      '深色／.faint（日期、註記）', '深色／浮起表面上的 .faint',
 ...
 */
{
   ...
   * 這裡**不寫死「比了幾個」**。
   *
   * 第一版寫的是 `/^\s*2 個對比值/`，而那個 2 只是當時 tokens.css 裡
   * 剛好有兩個現值宣稱。2026-09-11 在註解裡寫下印章那兩組對比
   * （6.14:1／6.35:1）之後它變成 4，測試就紅了 —— 紅的原因是
   * **有人多寫了一句正確的文件**，不是有人弄壞了什麼。
   *
   * 這一格真正要守的行為只有兩件：
   *   1. 標了「舊值／原本」的那一個要跳過（剛好 1 個）
   *   2. 跳過是**跟著數字**，不是跟著整行 —— 所以同一行的現值還在比，
   *      而「還在比」的證據是比到的個數**大於 1**
   *      （第一版的 bug 是整行跳掉，五個宣稱只剩一個在比）
   */
  const comparedM = /^\s*(\d+) 個對比值/m.exec(clean.out);
  const compared = comparedM ? Number(comparedM[1]) : 0;
  const okSkip = /另有 1 個標了「舊值／原本」的，刻意不比/.test(clean.out) && compared > 1;
  if (!okSkip) failed++;
  console.log(
    `  ${okSkip ? '✓' : 'X'} 舊值跟著數字跳過，同一行的現值照樣比（跳 1 個、比 ${compared} 個）`,
  );
