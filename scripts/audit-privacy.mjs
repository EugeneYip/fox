...
if (identity.source === 'none') {
  ...
  console.log('  而**這個 repo 是公開的**：本機這一次是推上去之前唯一會跑的檢查。');
  console.log('  CI 上少了 secret 確實會紅，但那時候東西已經公開了 —— 紅燈救不回來。');
  if (CI) {
    /*
     * ── 這一段是照著一次真的失敗改的 ──────────
     *
...
if (findings.length === 0) {
   ...
   */
  /*
   * ── 那個 notices 的計數**不包含身分規則** ────────────
   *
   * 第 5 輪（第三十八圈）量到的：「身分規則沒有執行」那一行是**單獨印的**
   * （在 `for (const n of notices)` 之外），所以上面那句
   * 「有 N 項這次沒有檢查」**沒有把它算進去**。
   *
   * 而這一段的註解自己寫著它就是為了那個情況存在的：
   * 「一個『身分規則沒有執行』的環境會拿到跟『全部查過而且乾淨』
   * 一模一樣的結尾」。那句話當時只擋住了別的 notice，
   * 偏偏漏掉它自己點名的那一個。
   *
   * 所以這裡把它**單獨說出來**，不混進那個數字裡 ——
   * 那不是「少查了一項」，那是「唯一保護本名的那幾條沒有跑」。
   */
  console.log(
    '乾淨。沒有硬編碼的個資，也沒有第三方資源。' +
      (notices.length > 0
      ...
      (identity.source === 'none'
        ? '\n  —— **而身分規則這次沒有執行**：本名／校名有沒有被貼進頁面，這一次沒有查。'
        : '') +
      '\n',
  );
  /*
...
console.log('\n' + '─'.repeat(52));
/*
 * ── 身分規則沒跑的時候，判決那一行不能只說「必須修正 0」────────────
 *
 * 第 5 輪（第三十八圈）加的。這一圈問「這件事現在靠誰記得？忘了會怎樣？」
 *
 * 實測：把 `identity.local.ts` 拿掉、也不設 `PRIVACY_NEEDLES`，
 * 這一支印一行 `⚠ 身分規則沒有執行`，然後結尾寫
 * **「必須修正 0」、離開碼 0**。`verify:all` 是靠離開碼串起來的，
 * 所以六道關卡全綠 —— 而**唯一保護本名與校名的那幾條規則一次都沒跑**。
 *
 * 後果為什麼特別重：**這個 repo 是公開的**。
 * 本機這一次是**發佈之前唯一會跑的檢查** ——
 * CI 上少了那個 secret 確實會紅，但那時東西已經推上去、已經公開了。
 * 紅燈救不回一個已經公開的字串。
 *
 * 這裡**不改離開碼**（那是既有的決定：沒有值的人也要跑得動這支）。
 * 改的是判決那一行不准讀起來像「都查過了」——
 * 跟這個 repo 一貫的作法一樣：綠燈要說出自己沒涵蓋什麼。
 */
const identitySkipped = identity.source === 'none';
console.log(
  `必須修正 ${errors.length}、請確認 ${warns.length}` +
    (identitySkipped ? '　—— **但身分規則這次沒有執行**（見上面那一段）' : '') +
    '\n',
);
