...
const budgets = [
  ...
  },
  {
    ...
    why:
      ...
      '目前最重的一頁約 15.8 KB（本機 level 9）。' +
      '上限 21 KB 是 2026-09-05 訂的，取當時那個數字（14.2 KB）的 1.5 倍；' +
      '2026-09-11 的視覺升級讓 CSS 長了 0.4 KB、HTML 長了 0.2 KB，' +
      '所以現值往上走了一格，離上限還有四分之一。' +
...
if (textFiles.length > 0) {
  ...
  budgets.push({
     ...
     * ── 這條預算量的東西**換人了** ──────────────────────
     *
     * 訂的時候心裡想的是 feed（所以底下那個 40 KB 是從「feed 滿載」推出來的），
     * 而說明裡一直寫著「rss-all.xml 目前 13 筆、gzip 3.6 KB」。
     * 2026-09-11 加了紙紋與捲動動畫之後這一條紅了，才發現：
     * **最大的那個文字檔早就不是 feed 了，是全站樣式表**
     * （rss-all.xml 現在 gzip 2.0 KB，樣式表 4.5 KB）。
     * 那個 3.6 也不是 feed 的尺寸，是**當時樣式表的尺寸** ——
     * 說明裡的數字一直對得上，而它指的東西一直是錯的。
     *
     * 上限維持 40 KB：那個推導問的是「哪一個檔案**可能**長最大」，
     * 而答案仍然是 feed（滿載約 27 KB），不是樣式表。
     *
     * 註解放在 `why:` **外面**是必要的：底下那段比對現值的程式碼用
     * `/why:\s*((?:'[^']*'\s*\+?\s*)+)/` 去抽字串，中間夾一個註解就會
     * 只抽到第一段，於是這一條**安靜地不再被比對**
     * （第一版就是這樣，比對條數從 4 掉到 3 而沒有人出聲）。
     */
    why:
      ...
      '碰到時先看是哪一種：feed 的上限被拿掉了、單筆變肥了（現在單筆 488 bytes）、' +
      '還是樣式表長大了。不要直接調高門檻。',
  });
} else {
...
if (searchIndex) {
  budgets.push({
    ...
    why:
      '目前 13 筆約 9.3 KB。**筆數少了不代表變小** —— 2026-09-11 把九支影片' +
      '併進對應的詩頁（同一首詩不要在搜尋裡出現兩次），筆數 22 → 13，' +
      '而大小幾乎沒變：影片的標題與說明搬進了詩頁那一筆的內文，字還在。' +
      '這個索引仍然**不需要任何人寫東西就會自己長**：她發一支還沒有詩頁的新影片，' +
      '就會多一筆。碰到時不要只是調高門檻，選一個：' +
