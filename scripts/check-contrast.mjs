...
const PAIRS = [
   ...
   * ── 這一組是量出來的，不是設計時想到的 ──────────
   *
   * 第 8 輪（第三十六圈）在瀏覽器裡把每一頁**真的出現過**的
   * 前景／背景組合數了一遍，拿去跟這份表對。
   * 只有這一組對不上：定義清單的標題（曾經是 `/colophon` 的「上次同步」、
   * 「來源狀態」，14.8px、weight 400 的正文）就是這個組合。
   *
   * 而它落在全站最緊的那一段：淺色 **4.52**、深色 5.05，門檻 4.5。
   * （最緊的其實是已經有人守的「卡片上的 .faint」深色 **4.51** ——
   * 這一組是第二緊的。兩組都只差 0.01～0.02，
   * 也就是說這份表漏掉的那一組，正好落在最需要有人看的那一段。）
   */
 ...
 *
 * 第 8 輪（第三十六圈）問「這道檢查的邊界外面是什麼？那裡現在有幾個？」
 *
 * 上面所有數字都算得很準 —— 但**算的是 PAIRS 上有的那幾組**，
 * 而 PAIRS 是人手維護的清單。加一個元件、換一個 class，
 * 畫面上就會多一組沒有人算過的組合，而這一支不會知道。
 *
 * 那一輪在瀏覽器裡實測（`npm run preview`，走 4 個頁面，
 * 對每一個有文字的元素取 `color` 與往上找到的第一個不透明背景）：
 * 真的出現過 **10 種**用得到 token 名字的組合，其中 **1 種不在表上** ——
 * `--c-ink-faint` 畫在 `--c-bg-sunken` 上（籤條；2026-09-11 之前是 `/colophon` 的定義清單標題）。
 * 補上之後量出來是淺色 4.52、深色 5.05，門檻 4.5。
 *
 * 沒有把那個瀏覽器掃描做成常設檢查，理由是它要真的起一個瀏覽器 ——
 * 六道關卡都是離線、無相依的。所以這裡只把邊界說出口：
 * 表上的組合算得準，表外的組合這一支看不見。
 */
console.log(
  '\n這份表是**手寫**的（PAIRS）—— 上面的數字對表上的組合算得準，\n' +
 ...
 */
process.exitCode = failures > 0 || staleTokenDocs > 0 ? 1 : 0;
