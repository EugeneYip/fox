...
export async function fetchYouTubeSource(source, { fetchRss, fetchApi, apiKey = '', log = { warn: () => {} } }) {
  ...
  try {
    ...
    if (!apiKey) {
       ...
       * ── 清單只寫一次，訊息自己去讀它 ────────────────
       *
       * 第 4 輪（第四十一圈）改的。第 4 輪（第四十圈）把判斷從只認 404
       * 放寬成 `404|500|502|503`，然後在**六個地方**寫下那一課 ——
       * 而那六處寫的都是「不只 404，**500** 也算」。
       * 也就是說程式比每一份描述都寬，`502`／`503` 一個字都沒被寫進去。
       *
       * 描述跟清單分開放，就一定會漂。所以清單只寫一次，訊息去讀它。
       */
      const FLAKY_STATUSES = [404, 500, 502, 503];
      const flaky = new RegExp(`\\b(${FLAKY_STATUSES.join('|')})\\b`).test(why);
      throw new Error(
        `RSS 失敗（${why}），而且沒有 YOUTUBE_API_KEY 可以退回。` +
          (flaky
            ? '　改法：**先不要當成帳號沒了** —— 這個端點會一陣一陣地壞掉，' +
              `而且**不只 404**，${FLAKY_STATUSES.slice(1).join('、')} 都算` +
              '（2026-09-06 一分鐘內連打三次：404、500、404）。' +
